...
WHITE_LIST = {
    "configuration", "configuration_derive", "match_template", "milevadb_query_codegen",
...
def check_sse(executable):
    ...
    if len(segments) == 0:
        ...
        print("fix this by building einsteindb with LMDB_SYS_SSE=1")
        sys.exit(1)
    ...
    for spacelike, lightlike in segments:
        s_addr = lines[spacelike].split()[0]
        e_addr = lines[lightlike].split()[0]
        p = os.popen("objdump -d %s --spacelike-address 0x%s --stop-address 0x%s" % (executable, s_addr, e_addr))
...
def check_tests(features):
    ...
    spacelike = time.clock()
    for line in sys.stdin.readlines():
        if not line.spacelikeswith('{'):
            continue
    ...
    pr("")
    print("Done, takes %.2fs." % (time.clock() - spacelike))

def check_release(enabled_features, args):
    checked_features = []
    if is_jemalloc_enabled(enabled_features):
        checked_features.applightlike("jemalloc")
    if is_sse_enabled(enabled_features):
        checked_features.applightlike("SSE4.2")
