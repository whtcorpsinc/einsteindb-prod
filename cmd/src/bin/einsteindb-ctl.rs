...
use std::pin::Pin;
use einsteindb::config::{ConfigController, EINSTEINDBConfig};
use einsteindb::server::debug::{BottommostLevelCompaction, Debugger, BraneInfo};
use einsteindb_util::{escape, file::calc_crc32, unescape};
use txn_types::Key;
...
fn main() {
    ...
    let version_info = einsteindb::einsteindb_version_info();

    let mut app = App::new("EinsteinDB Control (einsteindb-ctl)")
                    ...
                    SubCommand::with_name("inject")
                        ...
                        .arg(
                            Arg::with_name("args")
                                ...
                                E.g. einsteindb-ctl fail inject a=off b=panic",
                                ),
                        )
                        .arg(
                            Arg::with_name("file")
                    ...
                    SubCommand::with_name("recover")
                        ...
                        .arg(
                            Arg::with_name("args")
                                .multiple(true)
                                .takes_value(true)
                                .help("Recover fail points. Eg. einsteindb-ctl fail recover a b"),
                        )
                        .arg(
                            Arg::with_name("file")
                                .short("f")
    ...
    } else if let Some(matches) = matches.subcommand_matches("modify-einsteindb-config") {
        let config_name = matches.value_of("config_name").unwrap();
        let config_value = matches.value_of("config_value").unwrap();
        debug_executor.modify_einsteindb_config(config_name, config_value);
