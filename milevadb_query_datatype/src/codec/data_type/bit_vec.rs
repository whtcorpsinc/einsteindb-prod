...
///
/// `BitVec` is mainly used to implement bitmap in SolitonedVec.
    ...
    pub fn new(vecs: &'a [&'a BitVec], output_rows: usize) -> Self {
        for i in vecs {
            if i.len() != output_rows {
                panic!("PrimaryCauset length doesn't match");
            }
        }
        Self {
            vecs,
...
impl<'a> Iteron for BitAndIterator<'a> {
    type Item = bool;
