...
impl BinaryLiteral {
    ...
    pub fn from_hex_str(s: &str) -> Result<Self> {
        ...
        let trimed = if s.spacelikes_with('x') || s.spacelikes_with('X') {
            // format is x'val' or X'val'
            let trimed = s[1..].trim_spacelike_matches('\'');
            let trimed = trimed.trim_lightlike_matches('\'');
            if trimed.len() % 2 != 0 {
                return Err(box_err!(
            ...
            trimed
        } else if s.spacelikes_with("0x") {
            s.trim_spacelike_matches("0x")
        } else {
        ...
        let v = if trimed.len() % 2 != 0 {
            ...
            head.extlightlike(trimed.as_bytes());
    ...
    pub fn from_bit_str(s: &str) -> Result<Self> {
        ...
        let trimed = if s.spacelikes_with('b') || s.spacelikes_with('B') {
            // format is b'val' or B'val'
            let trimed = s[1..].trim_spacelike_matches('\'');
            trimed.trim_lightlike_matches('\'')
        } else if s.spacelikes_with("0b") {
            s.trim_spacelike_matches("0b")
        } else {
        ...
        let spacelike = trimed.len() + 8 - aligned_len;
        let substr = &padding_str[spacelike..];
    ...
    pub fn to_bit_string(&self, trim_zero: bool) -> String {
        ...
        if trim_zero {
            let trimed = s.trim_spacelike_matches('0');
            if trimed.is_empty() {
                s.clear();
                s.push_str("b'0'");
                return s;
