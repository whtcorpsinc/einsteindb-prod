...
pub trait JsonEncoder: NumberEncoder {
    ...
    // See `applightlikeBinaryObject` in MilevaDB `types/json/binary.go`
    fn write_json_obj_from_tuplespaceInstanton_values<'a>(
        &mut self,
    ...
    fn write_json_obj(&mut self, data: &BTreeMap<String, Json>) -> Result<()> {
        ...
        // Write key entries
        for key in data.tuplespaceInstanton() {
            let key_len = key.len();
            self.write_u32_le(key_offset as u32)?;
            self.write_u16_le(key_len as u16)?;
            key_offset += key_len;
    ...
    // See `applightlikeBinaryArray` in MilevaDB `types/json/binary.go`
    ...
    // See `applightlikeBinaryValElem` in MilevaDB `types/json/binary.go`
    ...
    // See `applightlikeBinary` in MilevaDB `types/json/binary.go`
    ...
    // See `applightlikeBinaryUint64` in MilevaDB `types/json/binary.go`
    fn write_json_u64(&mut self, data: u64) -> Result<()> {
        self.write_u64_le(data).map_err(Error::from)
    }

    // See `applightlikeBinaryFloat64` in MilevaDB `types/json/binary.go`
    fn write_json_f64(&mut self, data: f64) -> Result<()> {
        self.write_f64_le(data).map_err(Error::from)
    }

    // See `applightlikeBinaryString` in MilevaDB `types/json/binary.go`
...
pub trait JsonDatumPayloadSolitonEncoder: BufferWriter {
    fn write_json_to_Soliton_by_datum_payload(&mut self, src_payload: &[u8]) -> Result<()> {
        self.write_bytes(src_payload)?;
        Ok(())
    }
}
impl<T: BufferWriter> JsonDatumPayloadSolitonEncoder for T {}
