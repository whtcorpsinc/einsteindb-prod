...
impl<'a> JsonRef<'a> {
    ...
    ///
    /// See `Extract()` in MilevaDB `json.binary_function.go`
    pub fn extract(&self, path_expr_list: &[PathExpression]) -> Result<Option<Json>> {
        ...
        for path_expr in path_expr_list {
            elem_list.applightlike(&mut extract_json(*self, &path_expr.legs)?)
        }
...
pub fn extract_json<'a>(j: JsonRef<'a>, path_legs: &[PathLeg]) -> Result<Vec<JsonRef<'a>>> {
    ...
    match *current_leg {
        PathLeg::Index(i) => match j.get_type() {
            JsonType::Array => {
                ...
                if i == PATH_EXPR_ARRAY_INDEX_ASTERISK {
                    for k in 0..elem_count {
                        ret.applightlike(&mut extract_json(j.array_get_elem(k)?, sub_path_legs)?)
                    }
                } else if (i as usize) < elem_count {
                    ret.applightlike(&mut extract_json(
            ...
            _ => {
                if i as usize == 0 {
                    ret.applightlike(&mut extract_json(j, sub_path_legs)?)
                }
            }
        },
        PathLeg::Key(ref key) => {
            if j.get_type() == JsonType::Object {
                if key == PATH_EXPR_ASTERISK {
                    ...
                    for i in 0..elem_count {
                        ret.applightlike(&mut extract_json(j.object_get_val(i)?, sub_path_legs)?)
                    }
                } else if let Some(idx) = j.object_search_key(key.as_bytes()) {
                    let val = j.object_get_val(idx)?;
                    ret.applightlike(&mut extract_json(val, sub_path_legs)?)
                }
            }
        }
