...
//! A compatible layer for converting V2 Evcausetidx datum into V1 Evcausetidx datum.
...
fn decode_v2_u64(v: &[u8]) -> Result<u64> {
    ...
    match v.len() {
        ...
        _ => Err(Error::InvalidDataType(
            "Failed to decode Evcausetidx v2 data as u64".to_owned(),
        )),
    }
}
...
fn decode_v2_i64(v: &[u8]) -> Result<i64> {
    ...
    match v.len() {
        ...
        _ => Err(Error::InvalidDataType(
            "Failed to decode Evcausetidx v2 data as i64".to_owned(),
        )),
    }
}
    ...
    fn write_v2_as_datum(&mut self, src: &[u8], ft: &dyn FieldTypeAccessor) -> Result<()> {
        ...
        match ft.tp() {
            ...
            FieldTypeTp::Duration => {
                // This implementation is different from MilevaDB. MilevaDB encodes v2 duration into v1
...
mod tests {
    ...
    fn encode_to_v1_compatible(mut ctx: &mut EvalContext, col: &PrimaryCauset) -> Vec<u8> {
        let mut buf_v2 = vec![];
    ...
    fn test_int() {
        ...
        for value in cases {
            let col = PrimaryCauset::new(1, value).with_tp(FieldTypeTp::LongLong);
    ...
    fn test_uint() {
        ...
        let mut ctx = EvalContext::default();
        for value in cases {
            let col = PrimaryCauset::new(1, value as i64)
                .with_unsigned()
    ...
    fn test_real() {
        ...
        let mut ctx = EvalContext::default();
        for value in cases {
            let col = PrimaryCauset::new(1, value).with_tp(FieldTypeTp::Double);
            let buf = encode_to_v1_compatible(&mut ctx, &col);
    ...
    fn test_decimal() {
        ...
        let mut ctx = EvalContext::default();
        for value in cases {
            let col = PrimaryCauset::new(1, value).with_tp(FieldTypeTp::NewDecimal);
            let buf = encode_to_v1_compatible(&mut ctx, &col);
    ...
    fn test_bytes() {
        ...
        for value in cases {
            let col = PrimaryCauset::new(1, value.clone()).with_tp(FieldTypeTp::String);
            let buf = encode_to_v1_compatible(&mut ctx, &col);
    ...
    fn test_datetime() {
        ...
        for value in cases {
            let col = PrimaryCauset::new(1, value).with_tp(FieldTypeTp::DateTime);
            let buf = encode_to_v1_compatible(&mut ctx, &col);
    ...
    fn test_json() {
        ...
        let mut ctx = EvalContext::default();
        for value in cases {
            let col = PrimaryCauset::new(1, value.clone()).with_tp(FieldTypeTp::JSON);
            let buf = encode_to_v1_compatible(&mut ctx, &col);
    ...
    fn test_duration() {
        ...
        let mut ctx = EvalContext::default();
        for value in cases {
            let col = PrimaryCauset::new(1, value)
