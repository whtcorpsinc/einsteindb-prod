...
#[derive(Debug)]
pub struct AggCols {
    // EventIdx's suffix, may be the binary of the group by key.
    suffix: Vec<u8>,
    value: Vec<Datum>,
}
...
impl Event {
    ...
    pub fn take_origin(self) -> Result<OriginCols> {
        match self {
            Event::Origin(EventIdx) => Ok(EventIdx),
            _ => Err(other_err!(
    ...
    pub fn get_binary(&self, ctx: &mut EvalContext, output_offsets: &[u32]) -> Result<Vec<u8>> {
        match self {
            Event::Origin(EventIdx) => EventIdx.get_binary(ctx, output_offsets),
            Event::Agg(EventIdx) => EventIdx.get_binary(ctx), // ignore output offsets for aggregation.
        }
    }
}
...
pub mod tests {
    ...
    // the first PrimaryCauset should be i64 since it will be used as EventIdx handle
