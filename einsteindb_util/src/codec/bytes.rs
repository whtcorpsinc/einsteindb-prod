...
pub fn is_encoded_from(encoded: &[u8], raw: &[u8], desc: bool) -> bool {
    let check_single_Soliton = |encoded: &[u8], raw: &[u8]| {
        let len = raw.len();
    ...
    let mut rev_encoded_Solitons = encoded.rSolitons_exact(ENC_GROUP_SIZE + 1);
    // Valid encoded bytes must has complete Solitons
    if !rev_encoded_Solitons.remainder().is_empty() {
        return false;
    }

    // Bytes are compared in reverse order because in real cases like MilevaDB, if two tuplespaceInstanton
    // are different, the last a few bytes are more likely to be different.

    let raw_Solitons = raw.Solitons_exact(ENC_GROUP_SIZE);
    // Check the last Soliton first
    match rev_encoded_Solitons.next() {
        Some(encoded_Soliton) if check_single_Soliton(encoded_Soliton, raw_Solitons.remainder()) => {}
        _ => return false,
    }

    // The count of the remaining Solitons must be the same. Using `size_hint` here is both safe and
    // efficient because Soliton Iterons implement trait `TrustedLen`.
    if rev_encoded_Solitons.size_hint() != raw_Solitons.size_hint() {
        return false;
    }

    for (encoded_Soliton, raw_Soliton) in rev_encoded_Solitons.zip(raw_Solitons.rev()) {
        if !check_single_Soliton(encoded_Soliton, raw_Soliton) {
            return false;
        }
    }
    true
...
mod tests {
    ...
    fn test_enc_dec_bytes() {
        ...
        for (source, mut asc, mut desc) in pairs {
            ...
            // appplightlike timestamp, the timestamp bytes should not affect decode result
    ...
    fn test_is_encoded_from() {
        for raw_len in 0..=24 {
            ...
            for &desc in &[true, false] {
                ...
                // Should return false if encoded has less or more Solitons
                ...
                longer_encoded.extlightlike(&[0, 0, 0, 0, 0, 0, 0, 0, 0xFF]);
                assert!(
