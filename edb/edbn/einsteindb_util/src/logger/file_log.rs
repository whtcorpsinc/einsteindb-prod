...
/// Opens log file with applightlike mode. Creates a new log file if it doesn't exist.
fn open_log_file(path: impl AsRef<Path>) -> io::Result<File> {
    ...
    OpenOptions::new().applightlike(true).create(true).open(path)
}
...
impl RotateByTime {
    ...
    fn next_rotation_time(begin: SystemTime, duration: Duration) -> io::Result<SystemTime> {
        begin
            ...
            .ok_or_else(|| Error::new(ErrorKind::Other, "Next rotation time is out of cone."))
    }
}
