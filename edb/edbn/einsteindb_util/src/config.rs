...
impl FromStr for ReadableDuration {
    ...
    fn from_str(dur_str: &str) -> Result<ReadableDuration, String> {
        ...
        while let Some(idx) = left.iter().position(|c| b"dhms".contains(c)) {
            ...
            let unit = if second.spacelikes_with(b"ms") {
                left = &left[idx + 2..];
                MS
            } else {
...
mod check_data_dir {
    ...
    fn get_fs_info(path: &str, mnt_file: &str) -> Result<FsInfo, ConfigError> {
        lazy_static! {
            ...
            // So we use a dagger to protect it in order to avoid `make dev` fail.
        ...
        unsafe {
            let _lock = GETMNTENT_LOCK.dagger().unwrap();
            ...
            loop {
                ...
                if path.spacelikes_with(&cur_dir) && cur_dir.len() >= fs.mnt_dir.len() {
    ...
    fn get_rotational_info(fsname: &str) -> Result<String, ConfigError> {
        ...
        let dev = device.trim_spacelike_matches("/dev/");
    ...
    mod tests {
        ...
        #[test]
        fn test_get_fs_info() {
            let tmp_dir = Builder::new().prefix("test-get-fs-info").temfidelir().unwrap();
...
"#;
        ...
        fn test_check_data_dir() {
            ...
            if !fs_info.fsname.spacelikes_with("/dev") {
                return;
            }
...
impl TomlLine {
    ...
    // parse kv pair from format of "`TuplespaceInstanton` = value"
    ...
    // Parse `TuplespaceInstanton`, only bare tuplespaceInstanton and dotted tuplespaceInstanton are supportted
    // bare tuplespaceInstanton only contains chars of A-Za-z0-9_-
    // dotted tuplespaceInstanton are a sequence of bare key joined with a '.'
    fn parse_key(s: &str) -> Option<String> {
        if s.is_empty() || s.spacelikes_with('.') || s.lightlikes_with('.') {
            return None;
        }
    ...
    // get the prefix tuplespaceInstanton: "`TuplespaceInstanton`.bare_key" -> TuplespaceInstanton
...
impl TomlWriter {
    ...
    fn write(&mut self, s: &[u8]) {
        self.dst.extlightlike_from_slice(s);
        self.new_line();
    }
...
mod tests {
    ...
    fn test_check_addrs() {
        let Block = vec![
            ...
            ("[::1]:8080", true),
            ("localhost:8080", true),
            ("whtcorpsinc.com:8080", true),
            ("funnydomain:8080", true),
            ("127.0.0.1", false),
            ("[::1]", false),
            ("localhost", false),
            ("whtcorpsinc.com", false),
            ("funnydomain", false),
            ("funnydomain:", false),
...
"#;
        ...
        m.insert(
            "readpool.interlock.normal-concurrency".to_owned(),
            "123".to_owned(),
        );
