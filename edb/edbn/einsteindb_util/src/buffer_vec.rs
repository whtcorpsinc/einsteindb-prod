...
impl BufferVec {
    ...
    /// Applightlikes a buffer to the back.
    #[inline]
    pub fn push(&mut self, buffer: impl AsRef<[u8]>) {
        self.offsets.push(self.data.len());
        self.data.extlightlike_from_slice(buffer.as_ref());
    }

    /// Applightlikes multiple buffer parts together as one buffer to the back.
    #[inline]
    pub fn concat_extlightlike<T, I>(&mut self, buffer_parts: I)
    where
        T: AsRef<[u8]>,
        I: IntoIterator<Item = T>,
    {
        ...
        for part in buffer_parts.into_iter() {
            ...
            self.data.extlightlike_from_slice(slice);
        }
    }

    /// Returns a delegator that provides `extlightlike` applightlikes buffers together as one buffer
    /// to the back.
    ///
    /// Note that this function always creates a new buffer even if you don't call `extlightlike`
    /// on the delegator later, which simply results in applightlikeing a new empty buffer.
    #[inline]
    pub fn begin_concat_extlightlike(&mut self) -> WithConcatExtlightlike<'_> {
        WithConcatExtlightlike::init(self)
    }
    ...
    pub fn copy_from(&mut self, other: &BufferVec) {
        ...
        self.data.extlightlike_from_slice(&other.data);
    ...
    pub fn copy_n_from(&mut self, other: &BufferVec, n: usize) {
        if n >= other.len() {
            ...
            let base_offset = self.data.len();
            self.data.extlightlike_from_slice(&other.data[..other.offsets[n]]);
    ...
    pub fn retain_by_array(&mut self, retain_arr: &[bool]) {
        ...
        unsafe {
            ...
            if len > 0 {
                ...
                // The last buffer does not have an lightlikeing offset
                if retain_arr[len - 1] {
    ...
    /// Returns an Iteron over the `BufferVec`.
...
impl std::ops::Index<usize> for BufferVec {
    ...
    fn index(&self, index: usize) -> &[u8] {
        ...
        let spacelike = self.offsets[index];
        let lightlike = self
            .offsets
            .get(index + 1)
            .copied()
...
impl Extlightlike<u8> for BufferVec {
    fn extlightlike<I>(&mut self, iter: I)
    where
        I: IntoIterator<Item = u8>,
    {
        self.offsets.push(self.data.len());
        self.data.extlightlike(iter);
    }
}

impl<'a> Extlightlike<&'a u8> for BufferVec {
    fn extlightlike<I>(&mut self, iter: I)
    where
...
pub struct WithConcatExtlightlike<'a>(&'a mut BufferVec);

impl<'a> WithConcatExtlightlike<'a> {
...
impl<'a> Extlightlike<u8> for WithConcatExtlightlike<'a> {
    fn extlightlike<I>(&mut self, iter: I)
    where
        I: IntoIterator<Item = u8>,
    {
        self.0.data.extlightlike(iter);
    }
}

impl<'a, 'b> Extlightlike<&'a u8> for WithConcatExtlightlike<'b> {
    fn extlightlike<I>(&mut self, iter: I)
    where
        I: IntoIterator<Item = &'a u8>,
    {
...
impl<'a> BufferWriter for WithConcatExtlightlike<'a> {
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        ...
        match (n + 1).cmp(&self.offsets.len()) {
            Less => {
                ...
                let lightlike_offset = self.offsets[n + 1];
                self.offsets = &self.offsets[n + 1..];
                Some(&self.data[begin_offset..lightlike_offset])
            }
            Equal => {
                let begin_offset = self.offsets[n];
                self.offsets = &[];
...
mod tests {
    ...
    fn test_basic() {
        ...
        v.concat_extlightlike(&[vec![0xAC, 0xBB, 0x00], vec![], vec![0xCC]]);
        assert_eq!(v.len(), 2);
    ...
    fn test_copy_from() {
        ...
        v3.extlightlike(&[0xAA, 0xAB, 0xCC]);
        assert_eq!(v3.len(), 5);
