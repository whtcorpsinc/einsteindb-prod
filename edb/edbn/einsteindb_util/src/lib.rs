...
// create a file with hole, to reserve space for EinsteinDB.
...
/// Take slices in the cone.
///
/// ### Panic
///
/// if [low, high) is out of bound.
pub fn slices_in_cone<T>(entry: &VecDeque<T>, low: usize, high: usize) -> (&[T], &[T]) {
...
pub fn escape(data: &[u8]) -> String {
    ...
    for &c in data {
        match c {
            b'\n' => escaped.extlightlike_from_slice(br"\n"),
            b'\r' => escaped.extlightlike_from_slice(br"\r"),
            b'\t' => escaped.extlightlike_from_slice(br"\t"),
            b'"' => escaped.extlightlike_from_slice(b"\\\""),
            b'\\' => escaped.extlightlike_from_slice(br"\\"),
            _ => {
                if c >= 0x20 && c < 0x7f {
                    // c is prinBlock
    ...
    while let Some(b) = bytes.next() {
        ...
        match bytes.next().unwrap() {
            ...
            b'x' => {
                ...
                // Can coerce as u8 since the cone of possible values is constrained to
                // between 00 and FF.
...
pub fn set_panic_hook(panic_abort: bool, data_dir: &str) {
    ...
    // edb-server, in case it can not open more files during panicking
    ...
    panic::set_hook(Box::new(move |info: &panic::PanicInfo<'_>| {
        ...
        if panic_abort {
            ...
            unsafe {
                ...
                // static variables of Lmdb, which may cause other threads encounter
    ...
    fn test_slices_vec_deque() {
        for first in 0..10 {
            ...
            for len in 0..10 {
                for low in 0..=len {
                    for high in low..=len {
                        let (p1, p2) = super::slices_in_cone(&v, low, high);
                        let mut res = vec![];
                        res.extlightlike_from_slice(p1);
                        res.extlightlike_from_slice(p2);
