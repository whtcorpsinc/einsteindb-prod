...
impl ThreadsCollector {
    fn new<S: Into<String>>(pid: pid_t, namespace: S) -> ThreadsCollector {
        ...
        .unwrap();
        descs.extlightlike(io_totals.desc().into_iter().cloned());
...
impl Collector for ThreadsCollector {
    ...
    fn collect(&self) -> Vec<proto::MetricNetwork> {
        ...
        let metrics = self.metrics.dagger().unwrap();
        // Clean previous threads state.
        metrics.threads_state.reset();

        let mut tid_retriever = self.tid_retriever.dagger().unwrap();
        ...
        mfs.extlightlike(metrics.threads_state.collect());
        mfs.extlightlike(metrics.io_totals.collect());
        mfs.extlightlike(metrics.voluntary_ctxt_switches.collect());
        mfs.extlightlike(metrics.nonvoluntary_ctxt_switches.collect());
        mfs
    }
}
...
mod tests {
    ...
    fn test_thread_io_statistics() {
        ...
        for tid in tids {
            if let Ok(stat) = pid::stat_task(pid, tid) {
                if stat.command.spacelikes_with(s1) {
                    rx1.recv().unwrap();
                    thread_info.record();
                    {
    ...
    fn high_cpu_thread(
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                loop {
                    if (Instant::now() - spacelike).as_millis() > duration_ms.into() {
                        break;
                    }
                }
    ...
    fn test_thread_cpu_statistics() {
        ...
        for tid in tids {
            if let Ok(stat) = pid::stat_task(pid, tid) {
                if stat.command.spacelikes_with(tn) {
                    rx.recv().unwrap();
                    thread_info.record();
    ...
    fn get_thread_name(stat: &str) -> Result<(&str, usize)> {
        let spacelike = stat.find('(');
        let lightlike = stat.rfind(')');
        if let (Some(spacelike), Some(lightlike)) = (spacelike, lightlike) {
            return Ok((&stat[spacelike + 1..lightlike], lightlike));
        }
    ...
    fn test_sanitize_thread_name() {
        ...
        for &(i, o, idx) in &cases {
            let (raw_name, lightlike) = get_thread_name(i).unwrap();
            let name = sanitize_thread_name(1, raw_name);
            assert_eq!(name, o);
            assert_eq!(lightlike, idx);
        }
