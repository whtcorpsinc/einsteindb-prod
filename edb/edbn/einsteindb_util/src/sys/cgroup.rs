...
const CPU_QUOTA: &str = "cpu.causets_quota_us";
const CPU_PERIOD: &str = "cpu.causets_period_us";
...
pub fn parse_mount_point_from_line(line: &str) -> Result<MountPoint, String> {
    ...
    let mount_point = MountPoint {
        ...
        options: fields[MountInfoFieldPart1::Options as usize]
            ...
            .map(|s| s.to_string())
            .collect(),
        optional_fields: fields[MountInfoFieldPart1::OptionalFields as usize..fs_spacelike - 1]
            .to_vec(),
        fs_type: fields[fs_spacelike + MountInfoFieldPart2::FSType as usize].clone(),
        mount_source: fields[fs_spacelike + MountInfoFieldPart2::MountSource as usize].clone(),
        super_options: fields[fs_spacelike + MountInfoFieldPart2::SuperOptions as usize]
            .split(OPTIONS_SEP)
            .map(|s| s.to_string())
            .collect(),
    };
...
mod tests {
    ...
    fn test_read_num() {
        ...
        // Read -1 from file `cpu.causets_quota_us`
        let path = tmp_dir.path().join("cpu.causets_quota_us");
        let mut f2 = File::create(path).unwrap();
        f2.write_all(b"-1").unwrap();
        f2.sync_all().unwrap();
        assert_eq!(-1, cgroup.read_num("cpu.causets_quota_us").unwrap());

        // Read invalid number
        let path = tmp_dir.path().join("cpu.causets_period_us");
        let mut f3 = File::create(path).unwrap();
        f3.write_all(b"abc").unwrap();
        f3.sync_all().unwrap();
        assert!(cgroup.read_num("cpu.causets_period_us").is_err());
