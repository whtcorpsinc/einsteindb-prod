...
// the write causet's Iteron's direction inside Lmdb, and the next user key
// need to turn back the direction to backward. As we have tested, turn around
// Iteron's direction from forward to backward is as expensive as seek in
// Lmdb, so don't set REVERSE_SEEK_BOUND too small.
const REVERSE_SEEK_BOUND: u64 = 16;

/// This struct can be used to scan tuplespaceInstanton spacelikeing from the given user key in the reverse order
/// (less than).
///
...
impl<S: Snapshot> BackwardKvScanner<S> {
    ...
    pub fn read_next(&mut self) -> Result<Option<(Key, Value)>> {
        if !self.is_spacelikeed {
            if self.causet.upper_bound.is_some() {
                ...
                self.lock_cursor.seek_to_last(&mut self.statistics.dagger);
            }
            self.is_spacelikeed = true;
        }

        // Similar to forward scanner, the general idea is to simultaneously step write
        // cursor and dagger cursor. Please refer to `ForwardKvScanner` for details.
        ...
        loop {
            let (current_user_key, has_write, has_lock) = {
                ...
                let l_key = if self.lock_cursor.valid()? {
                    Some(self.lock_cursor.key(&mut self.statistics.dagger))
                } else {
                    None
                };
                ...
                let res = match (w_key, l_key) {
                    ...
                    (Some(wk), Some(lk)) => {
                        ...
                        match write_user_key.cmp(lk) {
                            ...
                            Ordering::Greater => {
                                // We meet write first, so the dagger of the write key does not exist.
            ...
            if has_lock {
                match self.causet.isolation_level {
                    IsolationLevel::Si => {
                        ...
                        result = Dagger::check_ts_conflict(
                            Cow::Owned(dagger),
                            &current_user_key,
                            ts,
                ...
                self.lock_cursor.prev(&mut self.statistics.dagger);
            }
            if has_write {
                if result.is_ok() {
    ...
    fn reverse_get(
        ...
        for i in 0..REVERSE_SEEK_BOUND {
            if i > 0 {
                ...
                if !self.write_cursor.valid()? {
                    // Key space lightlikeed. We use `last_version` as the return.
        ...
        // too. Thus no reallocation happens in `applightlike_ts`.
        seek_key = seek_key.applightlike_ts(ts);
        if use_near_seek {
            self.write_cursor
    ...
    /// returned directly. Otherwise there will be a default CAUSET look up.
    ///
...
mod tests {
    ...
    fn test_basic() {
        ...
        // Assume REVERSE_SEEK_BOUND == 4, we have tuplespaceInstanton:
        ...
        let mut scanner = ScannerBuilder::new(snapshot, REVERSE_SEEK_BOUND.into(), true)
            .cone(None, Some(Key::from_raw(&[11 as u8])))
            .build()
            .unwrap();
    ...
    fn test_many_tombstones() {
        ...
        // Generate Lmdb tombstones in write causet.
        let spacelike_ts = 1;
        let safe_point = 2;
        for i in 0..16 {
            for y in 0..16 {
                let pk = &[i as u8, y as u8];
                must_prewrite_put(&engine, pk, b"", pk, spacelike_ts);
                must_rollback(&engine, pk, spacelike_ts);
                // Generate 254 Lmdb tombstones between [0,0] and [15,15].
        ...
        // Generate 16 locks in dagger causet.
        let spacelike_ts = 3;
        for i in 0..16 {
            let pk = &[i as u8];
            must_prewrite_put(&engine, pk, b"", pk, spacelike_ts);
        ...
        let snapshot = engine.snapshot(&Context::default()).unwrap();
        let Evcausetidx = &[15 as u8];
        let k = Key::from_raw(Evcausetidx);
