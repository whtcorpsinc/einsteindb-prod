...
pub use milevadb_query_datatype::codec::data_type::{
    LogicalEvents, BATCH_MAX_SIZE, IDENTICAL_LOGICAL_ROWS,
};
use milevadb_query_datatype::codec::data_type::{ScalarValue, ScalarValueRef, VectorValue};
...
///
/// When node comes from a PrimaryCauset reference, it is a reference node (both value and field_type
/// are references).
///
...
impl<'a> RpnStackNodeVectorValue<'a> {
    ...
    /// Gets a reference to the logical events.
    pub fn logical_rows_struct(&self) -> LogicalEvents {
        match self {
            RpnStackNodeVectorValue::Generated { physical_value } => LogicalEvents::Ref {
                logical_rows: &IDENTICAL_LOGICAL_ROWS[0..physical_value.len()],
            },

            RpnStackNodeVectorValue::Ref { logical_rows, .. } => LogicalEvents::Ref { logical_rows },
        }
    }
...
pub enum RpnStackNode<'a> {
    ...
    /// Represents a vector value. Comes from a PrimaryCauset reference or evaluated result.
    Vector {
...
impl<'a> RpnStackNode<'a> {
    ...
    /// # Panics
    ///
    /// Panics if index is out of cone and this is a `Vector` variant.
    #[inline]
...
impl RpnExpression {
    ...
    ///
    /// Panics when referenced PrimaryCauset does not have equal length as specified in `events`.
    pub fn eval<'a>(
        &'a self,
        ctx: &mut EvalContext,
    ...
    /// Decodes all referred PrimaryCausets which are not decoded. Then we ensure
    /// all referred PrimaryCausets are decoded.
    ...
    /// Panics if the expression is not valid.
    ///
    /// Panics if referred PrimaryCausets are not decoded.
    ///
    /// Panics when referenced PrimaryCauset does not have equal length as specified in `events`.
                ...
                RpnExpressionNode::PrimaryCausetRef { offset } => {
                    ...
                    stack.push(RpnStackNode::Vector {
                        value: RpnStackNodeVectorValue::Ref {
                            physical_value: &decoded_physical_PrimaryCauset,
...
mod tests {
    ...
    use milevadb_query_common::Result;
    use milevadb_query_datatype::codec::batch::LazyBatchPrimaryCauset;
    ...
    fn test_eval_single_constant_node() {
        ...
        let result = exp.eval(&mut ctx, &[], &mut PrimaryCausets, &[], 10);
    ...
    /// Creates fixture to be used in `test_eval_single_PrimaryCauset_node_xxx`.
    fn new_single_PrimaryCauset_node_fixture() -> (LazyBatchPrimaryCausetVec, Vec<usize>, [FieldType; 2]) {
        let physical_PrimaryCausets = LazyBatchPrimaryCausetVec::from(vec![
            {
                // this PrimaryCauset is not referenced
                let mut col = LazyBatchPrimaryCauset::decoded_with_capacity_and_tp(5, EvalType::Real);
            ...
            {
                let mut col = LazyBatchPrimaryCauset::decoded_with_capacity_and_tp(5, EvalType::Int);
    ...
    fn test_eval_single_fn_call_node() {
        ...
        let mut ctx = EvalContext::default();
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::empty();
        let result = exp.eval(&mut ctx, &[], &mut PrimaryCausets, &[], 4);
        let val = result.unwrap();
        assert!(val.is_vector());
        assert_eq!(
            val.vector_value().unwrap().as_ref().to_int_vec(),
    ...
    fn test_eval_unary_function_scalar() {
        ...
        let mut ctx = EvalContext::default();
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::empty();
        let result = exp.eval(&mut ctx, &[], &mut PrimaryCausets, &[], 3);
        let val = result.unwrap();
        assert!(val.is_vector());
        assert_eq!(
            val.vector_value().unwrap().as_ref().to_real_vec(),
    ...
    /// Unary function (argument is raw PrimaryCauset). The PrimaryCauset should be decoded.
    #[test]
    fn test_eval_unary_function_raw_PrimaryCauset() {
        /// foo(v) performs v + 5.
        #[rpn_fn(nullable)]
        fn foo(v: Option<&i64>) -> Result<Option<i64>> {
            Ok(Some(v.unwrap() + 5))
        ...
        let mut ctx = EvalContext::default();
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::from(vec![{
            let mut col = LazyBatchPrimaryCauset::raw_with_capacity(3);
    ...
    fn test_eval_binary_function_vector_scalar() {
        ...
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::from(vec![{
            let mut col = LazyBatchPrimaryCauset::decoded_with_capacity_and_tp(3, EvalType::Real);
            col.mut_decoded().push_real(Real::new(1.0).ok());
    ...
    /// Binary function (arguments are both raw PrimaryCausets). The same PrimaryCauset is referred multiple times
    /// and it should be Ok.
    #[test]
    fn test_eval_binary_function_raw_PrimaryCauset() {
    ...
    fn test_eval_fail_1() {
        ...
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::empty();
        let hooked_eval = panic_hook::recover_safe(|| {
            let _ = exp.eval(&mut ctx, &[], &mut PrimaryCausets, &[], 3);
        });
        assert!(hooked_eval.is_err());
    }
    ...
    fn test_merge_nulls_constant_null() {
        ...
        let mut ctx = EvalContext::default();
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::empty();
        let val = exp.eval(&mut ctx, &[], &mut PrimaryCausets, &[], 10).unwrap();
        assert!(val.is_vector());
        assert_eq!(
            val.vector_value().unwrap().as_ref().to_real_vec(),
    ...
    fn bench_eval_plus_1024_rows(b: &mut Bencher) {
        ...
        profiler::spacelike("./bench_eval_plus_1024_rows.profile");
        b.iter(|| {
    ...
    fn bench_eval_compare_1024_rows(b: &mut Bencher) {
        ...
        profiler::spacelike("./eval_compare_1024_rows.profile");
        b.iter(|| {
            let result = black_box(&exp).eval(
                black_box(&mut ctx),
                black_box(schemaReplicant),
    ...
    fn bench_eval_compare_5_rows(b: &mut Bencher) {
        ...
        profiler::spacelike("./bench_eval_compare_5_rows.profile");
        b.iter(|| {
            let result = black_box(&exp).eval(
                black_box(&mut ctx),
                black_box(schemaReplicant),
...
mod benches {
    ...
    use milevadb_query_datatype::codec::data_type::*;
    use milevadb_query_datatype::expr::EvalContext;
    use milevadb_query_datatype::{EvalType, FieldTypeTp};
    ...
    fn bench_int_eval(b: &mut test::Bencher) {
        ...
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::from(vec![
            {
                let mut col = LazyBatchPrimaryCauset::decoded_with_capacity_and_tp(1024, EvalType::Real);
                for _i in 0..256 {
    ...
    fn bench_bytes_eval(b: &mut test::Bencher) {
        ...
        let mut PrimaryCausets = LazyBatchPrimaryCausetVec::from(vec![
            {
                let mut col = LazyBatchPrimaryCauset::decoded_with_capacity_and_tp(1024, EvalType::Bytes);
                for _i in 0..256 {
