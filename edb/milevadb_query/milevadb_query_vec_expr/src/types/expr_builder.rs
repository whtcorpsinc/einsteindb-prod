...
use milevadb_query_datatype::codec::mysql::{JsonDecoder, MAX_FSP};
...
impl RpnExpressionBuilder {
    ...
    pub fn check_expr_tree_supported(c: &Expr) -> Result<()> {
        ...
        // it can be different from the one we calculated (e.g. pass a PrimaryCauset / fn with different
        // type).
    ...
    pub fn is_expr_eval_to_scalar(c: &Expr) -> Result<bool> {
        match c.get_tp() {
            ...
            ExprType::PrimaryCausetRef => Ok(false),
    ...
    pub fn build_from_expr_tree(
        ...
        applightlike_rpn_nodes_recursively(
            tree_node,
            &mut expr_nodes,
            ctx,
    ...
    /// Pushes a `PrimaryCausetRef` node.
    pub fn push_PrimaryCauset_ref_for_test(mut self, offset: usize) -> Self {
        let node = RpnExpressionNode::PrimaryCausetRef { offset };
        self.0.push(node);
        self
    }
...
/// recursively.
fn applightlike_rpn_nodes_recursively<F>(
    tree_node: Expr,
    ...
    F: Fn(&Expr) -> Result<RpnFnMeta> + Copy,
{
    match tree_node.get_tp() {
        ExprType::ScalarFunc => {
            handle_node_fn_call(tree_node, rpn_nodes, ctx, fn_mapper, max_PrimaryCausets)
        }
        ExprType::PrimaryCausetRef => handle_node_PrimaryCauset_ref(tree_node, rpn_nodes, max_PrimaryCausets),
        _ => handle_node_constant(tree_node, rpn_nodes, ctx),
    }
}

#[inline]
fn handle_node_PrimaryCauset_ref(
    tree_node: Expr,
    rpn_nodes: &mut Vec<RpnExpressionNode>,
    max_PrimaryCausets: usize,
) -> Result<()> {
    let offset = tree_node
        .get_val()
        .read_i64()
        .map_err(|_| other_err!("Unable to decode PrimaryCauset reference offset from the request"))?
        as usize;
    ...
    rpn_nodes.push(RpnExpressionNode::PrimaryCausetRef { offset });
    Ok(())
}
    ...
    F: Fn(&Expr) -> Result<RpnFnMeta> + Copy,
{
    ...
    for arg in args {
        applightlike_rpn_nodes_recursively(arg, rpn_nodes, ctx, fn_mapper, max_PrimaryCausets)?;
    }
...
mod tests {
    ...
    /// For testing `applightlike_rpn_nodes_recursively`. It accepts protobuf function sig enum, which
    ...
    fn test_applightlike_rpn_nodes_recursively() {
        // Input:
    ...
    fn test_max_PrimaryCausets_check() {
        // Col offset = 0. The minimum success max_PrimaryCausets is 1.
        ...
        // Col offset = 3. The minimum success max_PrimaryCausets is 4.
        ...
        // Col offset = 1, 2, 5. The minimum success max_PrimaryCausets is 6.
        let node =
