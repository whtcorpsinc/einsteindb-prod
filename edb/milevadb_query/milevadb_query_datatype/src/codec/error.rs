...
quick_error! {
    ...
    pub enum Error {
        ...
        PrimaryCausetOffset(offset: usize) {
            display("illegal PrimaryCauset offset: {}", offset)
        }
...
impl Error {
    pub fn overflow(data: impl Display, expr: impl Display) -> Error {
        let msg = format!("{} value is out of cone in '{}'", data, expr);
    ...
    pub fn m_bigger_than_d(PrimaryCauset: impl Display) -> Error {
        let msg = format!(
            "For float(M,D), double(M,D) or decimal(M,D), M must be >= D (PrimaryCauset {}').",
            PrimaryCauset
        );
    ...
    pub fn cast_as_signed_overflow() -> Error {
        let msg =
            "Cast to signed converted positive out-of-cone integer to it's negative complement";
        Error::Eval(msg.into(), ERR_UNKNOWN)
    }
...
impl ErrorCodeExt for Error {
    fn error_code(&self) -> ErrorCode {
        match self {
            Error::InvalidDataType(_) => error_code::interlock::INVALID_DATA_TYPE,
            Error::Encoding(_) => error_code::interlock::ENCODING,
            Error::PrimaryCausetOffset(_) => error_code::interlock::PrimaryCauset_OFFSET,
            Error::UnknownSignature(_) => error_code::interlock::UNKNOWN_SIGNATURE,
            Error::Eval(_, _) => error_code::interlock::EVAL,
