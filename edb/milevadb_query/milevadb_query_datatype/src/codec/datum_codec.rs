...
/// An encoder to encode a datum storing PrimaryCauset id.
pub trait PrimaryCausetIdDatumEncoder: DatumFlagAndPayloadEncoder {
    #[inline]
    fn write_PrimaryCauset_id_datum(&mut self, col_id: i64) -> Result<()> {
        self.write_datum_var_i64(col_id)
    }
}

impl<T: BufferWriter> PrimaryCausetIdDatumEncoder for T {}
