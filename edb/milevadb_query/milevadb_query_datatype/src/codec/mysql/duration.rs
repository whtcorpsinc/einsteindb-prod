...
pub trait DurationEncoder: NumberEncoder {
    #[inline]
    fn write_duration_to_Soliton(&mut self, val: Duration) -> Result<()> {
        self.write_i64_le(val.to_nanos())?;
        Ok(())
    }
}

pub trait DurationDatumPayloadSolitonEncoder: NumberEncoder {
    #[inline]
    fn write_duration_to_Soliton_by_datum_payload_int(
        &mut self,
        mut src_payload: &[u8],
    ) -> Result<()> {
    ...
    #[inline]
    fn write_duration_to_Soliton_by_datum_payload_varint(
        &mut self,
        mut src_payload: &[u8],
    ) -> Result<()> {
...
impl<T: BufferWriter> DurationDatumPayloadSolitonEncoder for T {}
...
pub trait DurationDecoder: NumberDecoder {
    ...
    #[inline]
    fn read_duration_from_Soliton(&mut self, fsp: isize) -> Result<Duration> {
...
mod tests {
    ...
    fn test_codec() {
        ...
        for (input, fsp) in cases {
            let t = Duration::parse(&mut EvalContext::default(), input.as_bytes(), fsp).unwrap();
            let mut buf = vec![];
            buf.write_duration_to_Soliton(t).unwrap();
            let got = buf
                .as_slice()
                .read_duration_from_Soliton(fsp as isize)
                .unwrap();
            assert_eq!(t, got);
        }
    }
        ...
        b.iter(|| {
            ...
            for &duration in cases {
                ...
                let got = test::black_box(
                    buf.as_slice()
                        .read_duration_from_Soliton(t.fsp() as isize)
                        .unwrap(),
                );
                assert_eq!(t, got);
            }
