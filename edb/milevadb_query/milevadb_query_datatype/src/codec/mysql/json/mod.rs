...
//!   number ::=  ....    // little-lightlikeian format for [u]int(16|32|64), whereas
//!                       // double is stored in a platform-indeplightlikeent, eight-byte
...
pub use self::jcodec::{JsonDatumPayloadSolitonEncoder, JsonDecoder, JsonEncoder};
...
use std::str;
use einsteindb-prod_util::is_even;
...
impl TryFrom<u8> for JsonType {
    ...
    fn try_from(src: u8) -> Result<JsonType> {
        num_promises::FromPrimitive::from_u8(src)
...
/// Json implements type json used in einsteindb-prod by Binary Json.
...
impl Json {
    ...
    pub fn from_kv_pairs<'a>(entries: Vec<(&[u8], JsonRef<'a>)>) -> Result<Self> {
        let mut value = vec![];
        value.write_json_obj_from_tuplespaceInstanton_values(entries)?;
    ...
    pub fn from_object(map: BTreeMap<String, Json>) -> Result<Self> {
        let mut value = vec![];
        // TODO(fullstop000): use write_json_obj_from_tuplespaceInstanton_values instead
    ...
    /// Returns a `JsonRef` points to the spacelikeing of encoded bytes
