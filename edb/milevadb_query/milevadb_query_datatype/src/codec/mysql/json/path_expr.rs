...
//     scope ::= [ PrimaryCausetReference ] '$'
//     PrimaryCausetReference ::= // omit...
...
//     1) PrimaryCausetReference in scope must be empty now;
...
pub fn parse_json_path_expr(path_expr: &str) -> Result<PathExpression> {
    ...
    let expr = path_expr.index(dollar_index + 1..).trim_spacelike();
    ...
    let mut last_lightlike = 0;
    for m in RE.find_iter(expr) {
        let (spacelike, lightlike) = (m.spacelike(), m.lightlike());
        // Check all characters between two legs are blank.
        if expr
            .index(last_lightlike..spacelike)
            .char_indices()
            .any(|(_, c)| !c.is_ascii_whitespace())
        {
            return Err(box_err!("Invalid JSON path: {}", path_expr));
        }
        last_lightlike = lightlike;

        let next_char = expr.index(spacelike..).chars().next().unwrap();
        if next_char == '[' {
            // The leg is an index of a JSON array.
            let leg = expr[spacelike + 1..lightlike].trim();
            ...
            let mut key = expr[spacelike + 1..lightlike].trim().to_owned();
            if key == PATH_EXPR_ASTERISK {
                flags |= PATH_EXPRESSION_CONTAINS_ASTERISK;
            } else if key.spacelikes_with('"') {
    ...
    if (last_lightlike == 0) && (!expr.is_empty()) {
        return Err(box_err!("Invalid JSON path: {}", path_expr));
    }
    if !legs.is_empty() {
