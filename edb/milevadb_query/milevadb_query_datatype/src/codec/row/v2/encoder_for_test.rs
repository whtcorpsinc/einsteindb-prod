...
//! * non-null PrimaryCauset ids: when flag == 1 (big), id is 4 bytes, otherwise 1 byte
//! * null PrimaryCauset ids: when flag == 1 (big), id is 4 bytes, otherwise 1 byte
...
pub trait EventEncoder: NumberEncoder {
    fn write_row(&mut self, ctx: &mut EvalContext, PrimaryCausets: Vec<PrimaryCauset>) -> Result<()> {
        ...
        let mut non_null_ids = Vec::with_capacity(PrimaryCausets.len());
        let mut non_null_cols = Vec::with_capacity(PrimaryCausets.len());

        for col in PrimaryCausets {
            if col.id > 255 {
                is_big = true;
            }
...
pub trait ScalarValueEncoder: NumberEncoder + DecimalEncoder + JsonEncoder {
    #[inline]
    fn write_value(&mut self, ctx: &mut EvalContext, col: &PrimaryCauset) -> Result<()> {
        match &col.value {
...
mod tests {
    ...
    fn test_encode_unsigned() {
        let cols = vec![
            PrimaryCauset::new(1, std::u64::MAX as i64).with_unsigned(),
            PrimaryCauset::new(2, -1),
        ];
        let exp: Vec<u8> = vec![
    ...
    fn test_encode() {
        let cols = vec![
            ...
            PrimaryCauset::new(12, 2),
            PrimaryCauset::new(33, ScalarValue::Int(None)),
            PrimaryCauset::new(3, 3).with_unsigned(),
            PrimaryCauset::new(8, 32767),
            PrimaryCauset::new(7, b"abc".to_vec()),
            PrimaryCauset::new(9, 1.8),
            PrimaryCauset::new(6, -1.8),
            PrimaryCauset::new(
                13,
                Time::parse_datetime(&mut EvalContext::default(), "2018-01-19 03:14:07", 0, false)
                    .unwrap(),
            ),
            PrimaryCauset::new(14, Decimal::from(1i64)),
            PrimaryCauset::new(15, Json::from_str(r#"{"key":"value"}"#).unwrap()),
            PrimaryCauset::new(16, Duration::from_nanos(NANOS_PER_SEC, 0).unwrap()),
        ];
    ...
    fn test_encode_big() {
        let cols = vec![
            PrimaryCauset::new(1, 1000),
            PrimaryCauset::new(12, 2),
            PrimaryCauset::new(335, ScalarValue::Int(None)),
