...
impl Datum {
    ...
    fn cmp_dec(&self, ctx: &mut EvalContext, dec: &Decimal) -> Result<Ordering> {
        match *self {
            ...
            _ => {
                // FIXME: here is not same as MilevaDB's
    ...
    fn cmp_json(&self, ctx: &mut EvalContext, json: &Json) -> Result<Ordering> {
        let order = match *self {
            ...
            Datum::Dec(ref d) => {
                // FIXME: it this same as MilevaDB's?
    ...
    pub fn into_i64(self, ctx: &mut EvalContext) -> Result<i64> {
        ...
        match self {
            ...
            // FIXME: in Datum::Dur, to_int's error handle is not same as MilevaDB's
            Datum::Dur(d) => d.to_int(ctx, tp),
            // FIXME: in Datum::Dec, to_int's error handle is not same as MilevaDB's
    ...
    /// Keep compatible with MilevaDB's `GetFloat64` function.
    #[inline]
    pub fn f64(&self) -> f64 {
        let i = self.i64();
    ...
    /// Keep compatible with MilevaDB's `GetInt64` function.
    #[inline]
    pub fn i64(&self) -> i64 {
        match *self {
            Datum::I64(i) => i,
    ...
    /// Keep compatible with MilevaDB's `GetUint64` function.
    #[inline]
    pub fn u64(&self) -> u64 {
        self.i64() as u64
    }
    ...
    /// Keep compatible with MilevaDB's `CoerceArithmetic` function.
    ...
    /// Keep compatible with MilevaDB's `ToDecimal` function.
    ...
    /// Keep compatible with MilevaDB's `CoerceDatum` function.
    ...
    /// Keep compatible with MilevaDB's `ComputePlus` function.
...
{
    /// `read_datum` decodes on a datum from a byte slice generated by MilevaDB.
...
/// `decode` decodes all datum from a byte slice generated by milevadb.
...
/// `encode_to` encodes a datum slice and applightlikes the buffer to a vector.
