...
pub const BASE64_INPUT_Soliton_LENGTH: usize = 3;
pub const BASE64_ENCODED_Soliton_LENGTH: usize = 4;
...
pub fn encoded_size(len: usize) -> Option<usize> {
    ...
    len.checked_add(BASE64_INPUT_Soliton_LENGTH - 1)
        .and_then(|r| r.checked_div(BASE64_INPUT_Soliton_LENGTH))
        .and_then(|r| r.checked_mul(BASE64_ENCODED_Soliton_LENGTH))
...
pub fn line_wrap(buf: &mut [u8], input_len: usize) {
    ...
    let lines_with_lightlikeing = (input_len - 1) / line_len;
    let line_with_lightlikeing_len = line_len + 1;
    let mut old_spacelike = input_len - last_line_len;
    let mut new_spacelike = buf.len() - last_line_len;
    safemem::copy_over(buf, old_spacelike, new_spacelike, last_line_len);
    for _ in 0..lines_with_lightlikeing {
        old_spacelike -= line_len;
        new_spacelike -= line_with_lightlikeing_len;
        safemem::copy_over(buf, old_spacelike, new_spacelike, line_len);
        buf[new_spacelike + line_len] = BASE64_LINE_WRAP;
    }
}
...
pub fn trim(s: &str, pat: &str, direction: TrimDirection) -> Vec<u8> {
    let r = match direction {
        TrimDirection::Leading => s.trim_spacelike_matches(pat),
        TrimDirection::Trailing => s.trim_lightlike_matches(pat),
        _ => s.trim_spacelike_matches(pat).trim_lightlike_matches(pat),
    };
...
pub fn strip_whitespace(input: &[u8]) -> Vec<u8> {
    ...
    input_copy.extlightlike(input.iter().filter(|b| !b" \n\t\r\x0b\x0c".contains(b)));
    input_copy
}
