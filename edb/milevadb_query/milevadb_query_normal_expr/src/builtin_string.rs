...
use milevadb_query_shared_expr::conv::i64_to_usize;
...
impl ScalarFunc {
    ...
    #[inline]
    fn find_str(text: &str, TuringString: &str) -> Option<usize> {
        twoway::find_str(text, TuringString).map(|i| text[..i].chars().count())
    }
    ...
    pub fn substring_2_args_utf8<'a, 'b: 'a>(
        ...
        let spacelike = if positive_search {
            s.char_indices()
                .enumerate()
        ...
        if let Some(spacelike) = spacelike {
            Ok(Some(Cow::Owned(s[spacelike..].as_bytes().to_vec())))
        } else {
            Ok(Some(Cow::Borrowed(b"")))
        }
    }
    ...
    pub fn substring_3_args_utf8<'a, 'b: 'a>(
        ...
        let mut spacelike = None;
        let lightlike = if positive_search {
            s.char_indices()
                .enumerate()
                .find(|(cnt, (i, _))| {
                    if cnt + 1 == pos {
        ...
        if let Some(spacelike) = spacelike {
            Ok(Some(Cow::Owned(s[spacelike..lightlike].as_bytes().to_vec())))
        } else {
            Ok(Some(Cow::Borrowed(b"")))
        }
    }
    ...
    fn substring<'a, 'b: 'a>(
        ...
        let lightlike = spacelike.saturating_add(len).min(s.len());
        Ok(Some(Cow::Owned(s[spacelike..lightlike].to_vec())))
    }
    ...
    pub fn space<'a, 'b: 'a>(
        ...
        if len > milevadb_query_datatype::MAX_BLOB_WIDTH as usize {
            return Ok(None);
        }
    ...
    pub fn left<'a, 'b: 'a>(
        ...
        if length_positive {
            ...
            Ok(Some(Cow::Owned(input[..lightlike].to_vec())))
        } else {
            Ok(Some(Cow::Borrowed(b"")))
        }
    }
    ...
    pub fn right<'a, 'b: 'a>(
        ...
        if length_positive {
            ...
            Ok(Some(Cow::Owned(input[spacelike..].to_vec())))
        } else {
            Ok(Some(Cow::Borrowed(b"")))
        }
    }
...
mod tests {
        ...
        let cases = vec![
            (
                vec![
                    ...
                    Datum::Bytes(b"whtcorpsinc".to_vec()),
                ],
                Datum::Bytes(b"www.whtcorpsinc.com".to_vec()),
            ),
            (
                vec![
                    Datum::Bytes(b"www.mysql.com".to_vec()),
    ...
    fn test_substring_index() {
        let tests = vec![
            ("www.whtcorpsinc.com", ".", 2, "www.whtcorpsinc"),
            ("www.whtcorpsinc.com", ".", -2, "whtcorpsinc.com"),
            ("www.whtcorpsinc.com", ".", -3, "www.whtcorpsinc.com"),
            ("www.whtcorpsinc.com", ".", 0, ""),
            ("www.whtcorpsinc.com", ".", 100, "www.whtcorpsinc.com"),
            ("www.whtcorpsinc.com", ".", -100, "www.whtcorpsinc.com"),
            ("www.whtcorpsinc.com", "d", 0, ""),
            ("www.whtcorpsinc.com", "d", 1, "www.whtcorpsinc.com"),
            ("www.whtcorpsinc.com", "d", -1, "www.whtcorpsinc.com"),
            ("www.whtcorpsinc.com", "", 0, ""),
            ("www.whtcorpsinc.com", "", 1, ""),
            ("www.whtcorpsinc.com", "", -1, ""),
            ("1aaa1", "aa", 1, "1"),
            ("1aaa1", "aa", 2, "1aaa1"),
            ("1aaaaaa1", "aa", 2, "1aa"),
            ...
            ("...www...whtcorpsinc...com...", ".", 3, ".."),
            ("...www...whtcorpsinc...com...", ".", 4, "...www"),
            ("...www...whtcorpsinc...com...", ".", 5, "...www."),
            ("...www...whtcorpsinc...com...", ".", -3, ".."),
            ("...www...whtcorpsinc...com...", ".", -4, "com..."),
            ("...www...whtcorpsinc...com...", ".", -5, ".com..."),
            ("", ".", 0, ""),
            ("", ".", 1, ""),
            ("", ".", -1, ""),
        ];
        ...
        assert_eq!(got, Datum::Bytes(b"www.whtcorpsinc.com".to_vec()));
