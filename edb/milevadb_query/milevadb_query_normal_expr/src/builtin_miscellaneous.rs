...
use milevadb_query_datatype::codec::data_type::Duration;
use milevadb_query_datatype::codec::mysql::{Decimal, Json, Time};
...
impl ScalarFunc {
    ...
    pub fn is_ipv4_compat<'a, 'b: 'a>(
        ...
        if !input.spacelikes_with(&PREFIX_COMPAT) {
            return Ok(Some(0));
        }
        Ok(Some(1))
    }
    ...
    pub fn is_ipv4_mapped<'a, 'b: 'a>(
        ...
        if !input.spacelikes_with(&prefix_mapped) {
            return Ok(Some(0));
        }
        Ok(Some(1))
    }
...
mod tests {
    ...
    use milevadb_query_datatype::codec::data_type::Duration;
    use milevadb_query_datatype::codec::mysql::json::Json;
