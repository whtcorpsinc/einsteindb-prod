...
impl ScalarFunc {
    pub fn check_args(sig: ScalarFuncSig, args: usize) -> Result<()> {
        let (min_args, max_args) = match sig {
            ...
            | ScalarFuncSig::JsonTuplespaceInstantonSig => (1, 1),

            ScalarFuncSig::JsonTuplespaceInstanton2ArgsSig | ScalarFuncSig::JsonLengthSig => (1, 2),
            ...
            ScalarFuncSig::Unspecified => {
                return Err(box_err!("MilevaDB internal error (unspecified PbCode)"));
            }
...
macro_rules! dispatch_call {
    (
        ...
        impl ScalarFunc {
            pub fn eval_int(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<i64>> {
                match self.sig {
                    $(ScalarFuncSig::$i_sig => self.$i_func(ctx, Evcausetidx, $($i_arg),*)),*,
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }

            pub fn eval_real(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<f64>> {
                match self.sig {
                    $(ScalarFuncSig::$r_sig => self.$r_func(ctx, Evcausetidx, $($r_arg),*),)*
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }
            ...
            pub fn eval_decimal<'a, 'b: 'a>(
                ...
                match self.sig {
                    $(ScalarFuncSig::$d_sig => self.$d_func(ctx, Evcausetidx, $($d_arg),*),)*
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }
            ...
            pub fn eval_bytes<'a, 'b: 'a>(
                ...
                match self.sig {
                    $(ScalarFuncSig::$b_sig => self.$b_func(ctx, Evcausetidx, $($b_arg),*),)*
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }
            ...
            pub fn eval_time<'a, 'b: 'a>(
                ...
                match self.sig {
                    $(ScalarFuncSig::$t_sig => self.$t_func(ctx, Evcausetidx, $($t_arg),*),)*
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }
            ...
            pub fn eval_duration<'a, 'b: 'a>(
                ...
                match self.sig {
                    $(ScalarFuncSig::$u_sig => self.$u_func(ctx, Evcausetidx, $($u_arg),*),)*
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }
            ...
            pub fn eval_json<'a, 'b: 'a>(
                ...
                match self.sig {
                    $(ScalarFuncSig::$j_sig => self.$j_func(ctx, Evcausetidx, $($j_arg),*),)*
                    _ => Err(Error::UnknownSignature(self.sig))
                }
            }
            ...
            pub fn eval(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Datum> {
                match self.sig {
                    $(ScalarFuncSig::$i_sig => {
                        match self.$i_func(ctx, Evcausetidx, $($i_arg)*) {
                            Ok(Some(i)) => {
                    ...
                    $(ScalarFuncSig::$r_sig => {
                        self.$r_func(ctx, Evcausetidx, $($r_arg)*).map(Datum::from)
                    })*
                    $(ScalarFuncSig::$d_sig => {
                        self.$d_func(ctx, Evcausetidx, $($d_arg)*).map(Datum::from)
                    })*
                    $(ScalarFuncSig::$b_sig => {
                        self.$b_func(ctx, Evcausetidx, $($b_arg)*).map(Datum::from)
                    })*
                    $(ScalarFuncSig::$t_sig => {
                        self.$t_func(ctx, Evcausetidx, $($t_arg)*).map(Datum::from)
                    })*
                    $(ScalarFuncSig::$u_sig => {
                        self.$u_func(ctx, Evcausetidx, $($u_arg)*).map(Datum::from)
                    })*
                    $(ScalarFuncSig::$j_sig => {
                        self.$j_func(ctx, Evcausetidx, $($j_arg)*).map(Datum::from)
                    })*
                    _ => unimplemented!(),
                }
            }
...
dispatch_call! {
    ...
    JSON_CALLS {
        ...
        JsonTuplespaceInstantonSig => json_tuplespaceInstanton,
        JsonTuplespaceInstanton2ArgsSig => json_tuplespaceInstanton,
