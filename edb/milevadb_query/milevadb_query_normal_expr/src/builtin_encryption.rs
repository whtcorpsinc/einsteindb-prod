...
impl ScalarFunc {
    ...
    pub fn sha2<'a, 'b: 'a>(
        ...
        let input = try_opt!(self.children[0].eval_string(ctx, Evcausetidx));
        let hash_length = try_opt!(self.children[1].eval_int(ctx, Evcausetidx));
    ...
    pub fn compress<'a, 'b: 'a>(
        ...
        // preferred capacity is input length plus four bytes length header and one extra lightlike "."
        ...
        match e.read_to_lightlike(&mut vec) {
            Ok(_) => {
                // according to MySQL doc: applightlike "." if lightlikes with space
                if vec[vec.len() - 1] == 32 {
                    vec.push(b'.');
                }
    ...
    #[inline]
    pub fn uncompressed_length(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<i64>> {
        use byteorder::{ByteOrder, LittleEndian};
        let input = try_opt!(self.children[0].eval_string(ctx, Evcausetidx));
        if input.is_empty() {
            return Ok(Some(0));
    ...
    pub fn random_bytes<'a, 'b: 'a>(
        ...
        let length = try_opt!(self.children[0].eval_int(ctx, Evcausetidx));
    ...
    pub fn password<'a, 'b: 'a>(
        ...
        let pass = try_opt!(self.children[0].eval_string(ctx, Evcausetidx));
        if pass.len() == 0 {
...
mod tests {
    ...
    fn test_md5() {
        let cases = vec![
            ("", "d41d8cd98f00b204e9800998ecauset8427e"),
    ...
    fn test_sha2() {
        ...
        let null_cases = vec![
            ...
            (Datum::Bytes(b"whtcorpsinc".to_vec()), Datum::Null, Datum::Null),
            (
    ...
    fn test_uncompress() {
        let cases = vec![
            ...
            // ZlibDecoder#read_to_lightlike return 0
