...
/// An in-memory backlightlike, it saves master key in memory.
pub(crate) struct MemAesGcmBacklightlike {
    pub key: Vec<u8>,
}

impl MemAesGcmBacklightlike {
    pub fn new(key: Vec<u8>) -> Result<MemAesGcmBacklightlike> {
    ...
    pub fn decrypt_content(&self, content: &EncryptedContent) -> Result<Vec<u8>> {
        ...
        if method.as_slice() != MetadataMethod::Aes256Gcm.as_slice() {
            ...
            // content is written by a future version of EinsteinDB, and we don't know how to handle it.
...
mod tests {
    ...
    #[test]
    fn test_mem_backlightlike_ase_256_gcm() {
        ...
        let backlightlike = MemAesGcmBacklightlike::new(key).unwrap();
    ...
    #[test]
    fn test_mem_backlightlike_authenticate() {
        let pt = vec![1u8, 2, 3];
        let key = Vec::from_hex("603deb1015ca71be2b73aef0857d77811f352c073b6108d72d9810a30914dff4")
            .unwrap();

        let backlightlike = MemAesGcmBacklightlike::new(key).unwrap();
        let encrypted_content = backlightlike.encrypt_content(&pt, Iv::new_gcm()).unwrap();
        let plaintext = backlightlike.decrypt_content(&encrypted_content).unwrap();
        assert_eq!(plaintext, pt);
