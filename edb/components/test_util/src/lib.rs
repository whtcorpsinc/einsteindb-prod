...
pub fn setup_for_ci() {
    ...
    if env::var("PANIC_ABORT").is_ok() {
        ...
        einsteindb-prod_util::set_panic_hook(true, "./");
    }

    einsteindb-prod_util::check_environment_variables();

    if let Err(e) = einsteindb-prod_util::config::check_max_open_fds(4096) {
        panic!(
