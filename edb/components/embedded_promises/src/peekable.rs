...
///
/// Method variants here allow for specifying `ReadOptions`, the PrimaryCauset family
...
pub trait Peekable {
    ...
    ///
    /// Reads from the default PrimaryCauset family.
    ///
    /// Returns `None` if they key does not exist.
    fn get_value_opt(&self, opts: &ReadOptions, key: &[u8]) -> Result<Option<Self::DBVector>>;

    /// Read a value for a key from a given PrimaryCauset family, given a set of options.
    ///
    ...
    ///
    /// Uses the default options and PrimaryCauset family.
    ///
    /// Returns `None` if the key does not exist.
    ...
    /// Read a value for a key from a given PrimaryCauset family.
    ///
    /// Uses the default options.
    ///
    /// Returns `None` if the key does not exist.
    fn get_value_causet(&self, causet: &str, key: &[u8]) -> Result<Option<Self::DBVector>> {
        self.get_value_causet_opt(&ReadOptions::default(), causet, key)
    }
    ...
    /// Read a value and return it as a protobuf message.
    fn get_msg_causet<M: protobuf::Message + Default>(
        &self,
        causet: &str,
        key: &[u8],
    ) -> Result<Option<M>> {
        let value = self.get_value_causet(causet, key)?;
        if value.is_none() {
            return Ok(None);
        }
