...
{
    pub id: Uuid,
    pub cmds: MustConsumeVec<(VioletaBftCmdRequest, Callback<S>, Option<u64>)>,
...
where
    S: Snapshot,
{
    /// Check it's necessary to retry plightlikeing read requests or not.
    ...
    /// Clear all commands in the queue. if `notify_removed` contains an `brane_id`,
    /// notify the request's callback that the brane is removed.
    pub fn clear_all(&mut self, notify_removed: Option<u64>) {
        let mut removed = 0;
        for mut read in self.reads.drain(..) {
            removed += read.cmds.len();
            if let Some(brane_id) = notify_removed {
                for (_, cb, _) in read.cmds.drain(..) {
                    apply::notify_req_brane_removed(brane_id, cb);
                }
            } else {
                read.cmds.clear();
            }
        ...
        T: IntoIterator<Item = (Uuid, u64)>,
    {
        ...
        for (uuid, index) in states {
            ...
            debug!(
                "cannot find corresponding read from plightlikeing reads";
    ...
    /// VioletaBft could have not been ready to handle the poped task. So put it back into the queue.
...
mod tests {
    ...
    fn test_read_queue_fold() {
        let mut queue = ReadIndexQueue::<LmdbSnapshot>::default();
    ...
    fn test_become_leader_then_become_follower() {
        ...
        // Push a plightlikeing comand when the peer is follower.
        let id = Uuid::new_v4();
    ...
    fn test_retake_leadership() {
        ...
        // Push a plightlikeing read comand when the peer is leader.
        let id = Uuid::new_v4();
        let req = ReadIndexRequest::with_command(
            id,
            VioletaBftCmdRequest::default(),
    ...
    fn test_advance_replica_reads_out_of_order() {
        ...
        for i in 0..2 {
            // Push a plightlikeing read comand when the peer is follower.
            let req = ReadIndexRequest::with_command(
                ids[i],
                VioletaBftCmdRequest::default(),
                Callback::None,
