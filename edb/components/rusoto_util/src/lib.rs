...
macro_rules! new_client {
    ...
    ($client: ty, $config: ident, $dispatcher: ident) => {{
        let brane = $crate::get_brane($config.brane.as_ref(), $config.lightlikepoint.as_ref())?;
        let cred_provider = $crate::CredentialsProvider::new(
            $config.access_key.as_ref(),
            $config.secret_access_key.as_ref(),
        )?;
        <$client>::new_with($dispatcher, cred_provider, brane)
    }};
}
...
pub fn get_brane(brane: &str, lightlikepoint: &str) -> io::Result<Brane> {
    if !lightlikepoint.is_empty() {
        Ok(Brane::Custom {
            name: brane.to_owned(),
            lightlikepoint: lightlikepoint.to_owned(),
        })
    } else if !brane.is_empty() {
        brane.parse::<Brane>().map_err(|e| {
            Error::new(
                ErrorKind::InvalidInput,
                format!("invalid aws brane format {}: {}", brane, e),
            )
        })
    } else {
        Ok(Brane::default())
