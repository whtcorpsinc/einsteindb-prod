...
impl AttributeValidation for Attribute {
    fn validate<F>(&self, causetid: F) -> Result<()> where F: Fn() -> String {
        if self.unique == Some(attribute::Unique::Value) && !self.index {
            bail!(DbErrorKind::BadSchemaAssertion(format!(":edb/unique :edb/unique_value without :edb/index true for solitonId: {}", causetid())))
        }
        if self.unique == Some(attribute::Unique::CausetIdity) && !self.index {
            bail!(DbErrorKind::BadSchemaAssertion(format!(":edb/unique :edb/unique_causetIdity without :edb/index true for solitonId: {}", causetid())))
        }
        if self.fulltext && self.value_type != ValueType::String {
            bail!(DbErrorKind::BadSchemaAssertion(format!(":edb/fulltext true without :edb/valueType :edb.type/string for solitonId: {}", causetid())))
        }
        if self.fulltext && !self.index {
            bail!(DbErrorKind::BadSchemaAssertion(format!(":edb/fulltext true without :edb/index true for solitonId: {}", causetid())))
        }
        if self.component && self.value_type != ValueType::Ref {
            bail!(DbErrorKind::BadSchemaAssertion(format!(":edb/isComponent true without :edb/valueType :edb.type/ref for solitonId: {}", causetid())))
        }
...
impl AttributeBuilder {
    ...
    pub fn validate_install_attribute(&self) -> Result<()> {
        if self.value_type.is_none() {
            bail!(DbErrorKind::BadSchemaAssertion("Schema attribute for new attribute does not set :edb/valueType".into()));
        }
        Ok(())
    }
    ...
    pub fn validate_alter_attribute(&self) -> Result<()> {
        if self.value_type.is_some() {
            bail!(DbErrorKind::BadSchemaAssertion("Schema alteration must not set :edb/valueType".into()));
        }
        if self.fulltext.is_some() {
            bail!(DbErrorKind::BadSchemaAssertion("Schema alteration must not set :edb/fulltext".into()));
        }
        Ok(())
    }
...
pub trait SchemaBuilding {
    ...
    fn from_causetId_map_and_attribute_map(causetId_map: CausetIdMap, attribute_map: AttributeMap) -> Result<Schema>;
    fn from_causetId_map_and_triples<U>(causetId_map: CausetIdMap, assertions: U) -> Result<Schema>
...
impl SchemaBuilding for Schema {
    ...
    fn require_entid(&self, causetid: &symbols::Keyword) -> Result<KnownSolitonId> {
        self.get_entid(&causetid).ok_or(DbErrorKind::UnrecognizedCausetId(causetid.to_string()).into())
    }
    ...
    fn from_causetId_map_and_attribute_map(causetId_map: CausetIdMap, attribute_map: AttributeMap) -> Result<Schema> {
        ...
        Ok(Schema::new(causetId_map, entid_map, attribute_map))
    }

    /// Turn vec![(Keyword(:causetid), Keyword(:key), TypedValue(:value)), ...] into a EinsteinDB `Schema`.
    fn from_causetId_map_and_triples<U>(causetId_map: CausetIdMap, assertions: U) -> Result<Schema>
        where U: IntoIterator<Item=(symbols::Keyword, symbols::Keyword, TypedValue)>{

        let entid_assertions: Result<Vec<(SolitonId, SolitonId, TypedValue)>> = assertions.into_iter().map(|(symbolic_causetId, symbolic_attr, value)| {
        ...
        let mut schema = Schema::from_causetId_map_and_attribute_map(causetId_map, AttributeMap::default())?;
        let spacetime_report = spacetime::update_attribute_map_from_entid_triples(&mut schema.attribute_map,
                                                                                entid_assertions?,
                                                                                // No retractions.
                                                                                vec![])?;
...
pub trait SchemaTypeChecking {
    ...
    fn to_typed_value(&self, value: &edbn::ValueAndSpan, value_type: ValueType) -> Result<TypedValue>;
}
...
impl SchemaTypeChecking for Schema {
    fn to_typed_value(&self, value: &edbn::ValueAndSpan, value_type: ValueType) -> Result<TypedValue> {
        ...
        // wrapper function.
        match TypedValue::from_edbn_value(&value.clone().without_spans()) {
            ...
            Some(typed_value) => match (value_type, typed_value) {
                ...
                (ValueType::Ref, TypedValue::Keyword(ref x)) => self.require_entid(&x).map(|solitonId| solitonId.into()),
...
mod test {
    ...
    fn add_attribute(schema: &mut Schema,
        ...
        if attribute.component {
            schema.component_attributes.push(solitonId);
        }

        schema.attribute_map.insert(solitonId, attribute);
    }
    ...
    fn invalid_schema_unique_value_not_index() {
        ...
        add_attribute(&mut schema, causetid , 99, Attribute {
            index: false,
            value_type: ValueType::Boolean,
            fulltext: false,
            unique: Some(attribute::Unique::Value),
        ...
        assert_eq!(err, Some(DbErrorKind::BadSchemaAssertion(":edb/unique :edb/unique_value without :edb/index true for solitonId: :foo/bar".into())));
    }

    #[test]
    fn invalid_schema_unique_causetIdity_not_index() {
        let mut schema = Schema::default();
        ...
        let err = validate_attribute_map(&schema.entid_map, &schema.attribute_map).err().map(|e| e.kind());
        assert_eq!(err, Some(DbErrorKind::BadSchemaAssertion(":edb/unique :edb/unique_causetIdity without :edb/index true for solitonId: :foo/bar".into())));
    }
    ...
    fn invalid_schema_component_not_ref() {
        ...
        let err = validate_attribute_map(&schema.entid_map, &schema.attribute_map).err().map(|e| e.kind());
        assert_eq!(err, Some(DbErrorKind::BadSchemaAssertion(":edb/isComponent true without :edb/valueType :edb.type/ref for solitonId: :foo/bar".into())));
    }
    ...
    fn invalid_schema_fulltext_not_index() {
        ...
        let err = validate_attribute_map(&schema.entid_map, &schema.attribute_map).err().map(|e| e.kind());
        assert_eq!(err, Some(DbErrorKind::BadSchemaAssertion(":edb/fulltext true without :edb/index true for solitonId: :foo/bar".into())));
    }
    ...
    fn invalid_schema_fulltext_index_not_string() {
        ...
        let err = validate_attribute_map(&schema.entid_map, &schema.attribute_map).err().map(|e| e.kind());
        assert_eq!(err, Some(DbErrorKind::BadSchemaAssertion(":edb/fulltext true without :edb/valueType :edb.type/string for solitonId: :foo/bar".into())));
    }
}
