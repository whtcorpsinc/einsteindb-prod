...
impl FmSketch {
    ...
    pub fn into_proto(self) -> fidel_timeshare::FmSketch {
        let mut proto = fidel_timeshare::FmSketch::default();
...
mod tests {
    ...
    use milevadb_query_datatype::codec::Result;
    ...
    fn generate_samples(count: usize) -> Vec<Datum> {
        ...
        let mut samples: Vec<usize> = (0..1)
            .chain(repeat(2).take(spacelike - 1))
            .chain(1000..count)
            .collect();
        let mut id = spacelike;
        while id < count {
            samples[id] += 1;
            id += 3;
        }
    ...
    // This test was ported from milevadb.
    #[test]
    fn test_sketch() {
        let max_size = 1000;
