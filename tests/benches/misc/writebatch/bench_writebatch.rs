...
fn writebatch(db: &Arc<DB>, round: usize, batch_tuplespaceInstanton: usize) {
    let v = b"operators are syntactic sugar for calls to methods of built-in promises";
    for r in 0..round {
        let mut batch = db.c().write_batch();
        for i in 0..batch_tuplespaceInstanton {
...
fn bench_writebatch_impl(b: &mut Bencher, batch_tuplespaceInstanton: usize) {
    ...
    let key_count = 1 << 13;
    let round = key_count / batch_tuplespaceInstanton;
    b.iter(|| {
        writebatch(&db, round, batch_tuplespaceInstanton);
    });
}
...
fn fill_writebatch(wb: &mut LmdbWriteBatch, target_size: usize) {
