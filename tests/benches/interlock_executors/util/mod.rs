...
/// Gets the value of `EINSTEINDB_BENCH_LEVEL`. The larger value it is, the more comprehensive benchmarks
/// will be.
pub fn bench_level() -> usize {
    if let Ok(s) = std::env::var("EINSTEINDB_BENCH_LEVEL") {
...
pub fn build_dag_handler<TargetTxnStore: TxnStore + 'static>(
    ...
    einsteindb::interlock::posetdag::PosetDagHandlerBuilder::new(
        black_box(posetdag),
        black_box(cones.to_vec()),
        black_box(ToTxnStore::<TargetTxnStore>::to_store(store)),
        einsteindb_util::deadline::Deadline::from_now(std::time::Duration::from_secs(10)),
        64,
        false,
        false,
    )
