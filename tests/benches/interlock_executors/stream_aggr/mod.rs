...
fn bench_stream_aggr_count_1_group_by_int_col_2_groups<M>(
    ...
    M: Measurement,
{
    let fb = FixtureBuilder::new(input.src_rows).push_PrimaryCauset_i64_ordered(&[0x123456, 0xCCCC]);
    let group_by = vec![ExprDefBuilder::PrimaryCauset_ref(0, FieldTypeTp::LongLong).build()];
    let expr = ExprDefBuilder::aggr_func(ExprType::Count, FieldTypeTp::LongLong)
        .push_child(ExprDefBuilder::constant_int(1))
        .build();
    ...
    M: Measurement,
{
    let fb = FixtureBuilder::new(input.src_rows)
        .push_PrimaryCauset_decimal_ordered(&["680644618.9451818", "767257805709854474.824642776567"]);
    let group_by = vec![ExprDefBuilder::PrimaryCauset_ref(0, FieldTypeTp::NewDecimal).build()];
    let expr = ExprDefBuilder::aggr_func(ExprType::Count, FieldTypeTp::LongLong)
        .push_child(ExprDefBuilder::constant_int(1))
        .build();
...
{
    let fb = FixtureBuilder::new(input.src_rows)
        .push_PrimaryCauset_i64_ordered(&[0xDEADBEEF, 0xFEE1DEAD])
        .push_PrimaryCauset_f64_ordered(&[680644618.9451818]);
    let group_by = vec![
        ExprDefBuilder::PrimaryCauset_ref(0, FieldTypeTp::LongLong).build(),
        ExprDefBuilder::PrimaryCauset_ref(1, FieldTypeTp::Double).build(),
    ];
