...
fn create_tmp_engine(dir: &TempDir) -> Engines<LmdbEngine, LmdbEngine> {
    ...
    Engines::new(LmdbEngine::from_db(db), LmdbEngine::from_db(raft_db))
}
...
fn spacelike_violetabftstore(
    ...
    let (raft_router, mut system) = create_raft_batch_system(&causetg.raft_store);
    ...
    let causetg_track = Arc::new(VersionTrack::new(causetg.raft_store.clone()));
    ...
    system
        ...
        .unwrap();
    (causetg_controller, raft_router, system.apply_router(), system)
}
...
fn test_ufidelate_violetabftstore_config() {
    ...
    raft_store.raft_log_gc_memory_barrier = 54321;
    validate_store(&router, move |causetg: &Config| {
        assert_eq!(causetg, &raft_store);
    });
