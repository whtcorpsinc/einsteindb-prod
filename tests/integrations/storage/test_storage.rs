...
fn test_store_resolve_with_illegal_tso() {
    ...
    store.resolve_lock_with_illegal_tso(spacelike_ts, commit_ts);
}
...
fn test_txn_store_gc2_without_key() {
    test_txn_store_gc_multiple_tuplespaceInstanton(1, 0);
}

#[test]
fn test_txn_store_gc2_with_less_tuplespaceInstanton() {
    test_txn_store_gc_multiple_tuplespaceInstanton(1, 3);
}

#[test]
fn test_txn_store_gc2_with_many_tuplespaceInstanton() {
    test_txn_store_gc_multiple_tuplespaceInstanton(1, DEFAULT_GC_BATCH_KEYS + 1);
}
...
fn test_txn_store_write_conflict() {
    ...
    let key = b"key";
    let primary = b"key";
    let conflict_spacelike_ts = 5;
    let conflict_commit_ts = 10;
    store.put_ok(key, primary, conflict_spacelike_ts, conflict_commit_ts);
    let spacelike_ts2 = 6;
...
fn inc<E: Engine>(store: &SyncTestStorage<E>, oracle: &Oracle, key: &[u8]) -> Result<i32, ()> {
    ...
    for i in 0..INC_MAX_RETRY {
        let spacelike_ts = oracle.get_ts();
        let number: i32 = match store.get(Context::default(), &key_address, spacelike_ts) {
...
fn inc_multi<E: Engine>(store: &SyncTestStorage<E>, oracle: &Oracle, n: usize) -> bool {
    'retry: for i in 0..INC_MAX_RETRY {
        let spacelike_ts = oracle.get_ts();
        let tuplespaceInstanton: Vec<Key> = (0..n).map(format_key).map(|x| Key::from_raw(&x)).collect();
        let mut mutations = vec![];
        for key in tuplespaceInstanton.iter().take(n) {
            let number = match store.get(Context::default(), key, spacelike_ts) {
        ...
        if store
            .prewrite(Context::default(), mutations, b"k0".to_vec(), spacelike_ts)
            .is_err()
        {
            backoff(i);
            continue;
        ...
        let commit_ts = oracle.get_ts();
        if store
            .commit(Context::default(), tuplespaceInstanton, spacelike_ts, commit_ts)
            .is_err()
        {
            backoff(i);
            continue;
