...
pub fn get_raft_client(pool: &tokio::runtime::Runtime) -> VioletaBftClient<VioletaBftStoreBlackHole> {
...
fn test_batch_raft_fallback() {
    ...
    (0..100).for_each(|_| {
        raft_client.slightlike(1, &addr, VioletaBftMessage::default()).unwrap();
...
fn test_raft_client_reconnect() {
    ...
    let addr = format!("localhost:{}", port);
    (0..50).for_each(|_| raft_client.slightlike(1, &addr, VioletaBftMessage::default()).unwrap());
    raft_client.flush();
    ...
    let slightlike = |_| {
        thread::sleep(time::Duration::from_millis(10));
        raft_client.slightlike(1, &addr, VioletaBftMessage::default())
    };
...
fn test_batch_size_limit() {
    ...
    for _ in 0..10 {
        ...
        let mut raft_m = VioletaBftMessage::default();
        for _ in 0..(5 * 1024) {
            let mut e = Entry::default();
            e.set_data(vec![b'a'; 1024]);
            raft_m.mut_message().mut_entries().push(e);
        }
        raft_client.slightlike(1, &addr, raft_m).unwrap();
    }
    raft_client.flush();
