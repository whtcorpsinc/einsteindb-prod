...
fn test_read_hibernated_brane() {
    ...
    cluster.causetg.raft_store.raft_store_max_leader_lease = ReadableDuration(Duration::from_millis(1));
    cluster.fidel_client.disable_default_operator();
    let r1 = cluster.run_conf_change();
    let p2 = new_peer(2, 2);
    ...
    router.slightlike_raft_message(wake_up_msg).unwrap();
...
fn test_read_index_out_of_order() {
    ...
    cluster.causetg.raft_store.raft_store_max_leader_lease =
