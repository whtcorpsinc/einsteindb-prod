    ...
    C: FnOnce(&mut Cluster<NodeCluster>),
{
    ...
    let mut snaps = vec![];
    snaps.push((1, LmdbSnapshot::new(cluster.get_raft_engine(1))));
    if mode == DataLost::AllLost {
        cluster.wait_last_index(1, 2, last_index + 1, Duration::from_secs(3));
        snaps.push((2, LmdbSnapshot::new(cluster.get_raft_engine(2))));
        cluster.wait_last_index(1, 3, last_index + 1, Duration::from_secs(3));
        snaps.push((3, LmdbSnapshot::new(cluster.get_raft_engine(3))));
    }
...
fn test_ufidelate_internal_apply_index() {
    ...
    for i in 1..3 {
        ...
        snaps.push((i, LmdbSnapshot::new(cluster.get_raft_engine(1))));
    }
    cluster.clear_slightlike_filters();
