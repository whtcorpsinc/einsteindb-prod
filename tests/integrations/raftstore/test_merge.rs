...
use ekvproto::raft_serverpb::{PeerState, BraneLocalState};
use violetabft::eraftpb::MessageType;

use engine_lmdb::Compat;
use engine_promises::Peekable;
use engine_promises::{CAUSET_RAFT, CAUSET_WRITE};
...
fn test_node_merge_with_slow_learner() {
    ...
    configure_for_merge(&mut cluster);
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 40;
    cluster.causetg.raft_store.raft_log_gc_count_limit = 40;
    cluster.causetg.raft_store.merge_max_log_gap = 15;
...
fn test_node_merge_slow_split(is_right_derive: bool) {
    ...
    let fidel_client = Arc::clone(&cluster.fidel_client);
    fidel_client.disable_default_operator();
    cluster.causetg.raft_store.right_derive_when_split = is_right_derive;
...
fn test_merge_isloated_stale_learner() {
    ...
    cluster.causetg.raft_store.max_leader_missing_duration = ReadableDuration::hours(2);
    cluster.causetg.raft_store.abnormal_leader_missing_duration = ReadableDuration::minutes(10);
    cluster.causetg.raft_store.peer_stale_state_check_interval = ReadableDuration::minutes(5);
    let fidel_client = Arc::clone(&cluster.fidel_client);
    fidel_client.disable_default_operator();
