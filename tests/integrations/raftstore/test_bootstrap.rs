...
use ekvproto::metapb;
use ekvproto::raft_serverpb::BraneLocalState;

use concurrency_manager::ConcurrencyManager;
use engine_lmdb::{Compat, LmdbEngine};
use engine_promises::{Engines, Peekable, ALL_CAUSETS, CAUSET_RAFT};
...
fn test_node_bootstrap_with_prepared_data() {
    ...
    let (_, system) = fsm::create_raft_batch_system(&causetg.raft_store);
    ...
    let tmp_path_raft = tmp_path.path().join(Path::new("violetabft"));
    let raft_engine = Arc::new(
        engine_lmdb::raw_util::new_engine(tmp_path_raft.to_str().unwrap(), None, &[], None)
            .unwrap(),
    );
    let engines = Engines::new(
        LmdbEngine::from_db(Arc::clone(&engine)),
        LmdbEngine::from_db(Arc::clone(&raft_engine)),
    );
    ...
    let mut node = Node::new(
        system,
        &causetg.server,
        Arc::new(VersionTrack::new(causetg.raft_store.clone())),
