...
use ekvproto::raft_serverpb::{VioletaBftApplyState, VioletaBftTruncatedState};

use engine_lmdb::LmdbEngine;
use engine_promises::{Engines, Peekable, CAUSET_RAFT};
...
fn get_raft_msg_or_default<M: protobuf::Message + Default>(
    ...
    engines
        .kv
        .get_msg_causet(CAUSET_RAFT, key)
        .unwrap()
        .unwrap_or_default()
}
...
fn test_compact_log<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    for (&id, engines) in &cluster.engines {
        let mut state: VioletaBftApplyState =
            get_raft_msg_or_default(&engines, &tuplespaceInstanton::apply_state_key(1));
...
fn check_compacted(
    ...
    for (id, engines) in all_engines {
        for i in 0..compacted_idx[id] {
            let key = tuplespaceInstanton::raft_log_key(1, i);
...
fn test_compact_count_limit<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.causetg.raft_store.raft_log_gc_count_limit = 100;
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 500;
...
fn test_compact_many_times<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.raft_log_gc_count_limit = gc_limit;
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 500;
...
fn test_compact_size_limit<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.causetg.raft_store.raft_log_gc_count_limit = 100000;
    cluster.causetg.raft_store.raft_log_gc_size_limit = ReadableSize::mb(2);
    cluster.run();
    cluster.stop_node(1);
...
fn test_compact_reserve_max_ticks<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 500;
    cluster.causetg.raft_store.raft_log_gc_size_limit = ReadableSize::mb(20);
    cluster.causetg.raft_store.raft_log_reserve_max_ticks = 2;
    cluster.run();
    ...
    for (&id, engines) in &cluster.engines {
        must_get_equal(&engines.kv.as_inner(), b"k1", b"v1");
        let mut state: VioletaBftApplyState = get_raft_msg_or_default(&engines, &apply_key);
        let state = state.take_truncated_state();
        // compact should not spacelike
        assert_eq!(RAFT_INIT_LOG_INDEX, state.get_index());
        assert_eq!(RAFT_INIT_LOG_TERM, state.get_term());
