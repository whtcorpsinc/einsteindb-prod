...
use ekvproto::{debugpb, metapb, raft_serverpb};
...
use engine_promises::{MiscExt, SyncMutable, CAUSET_DEFAULT, CAUSET_DAGGER, CAUSET_RAFT, CAUSET_WRITE};
...
fn test_mvcc_rollback_and_cleanup() {
        ...
        .zip(vec![k.clone(), k2.clone()])
    {
        ...
        assert_eq!(lock.lock_version, prewrite_spacelike_version2);
    }
...
fn test_debug_brane_info() {
    ...
    let mut raft_state = raft_serverpb::VioletaBftLocalState::default();
    ...
    assert_eq!(
        raft_engine
            .c()
            .get_msg::<raft_serverpb::VioletaBftLocalState>(&raft_state_key)
            .unwrap()
            .unwrap(),
        raft_state
    );
    ...
    let mut apply_state = raft_serverpb::VioletaBftApplyState::default();
    ...
    assert_eq!(
        kv_engine
            .c()
            .get_msg_causet::<raft_serverpb::VioletaBftApplyState>(CAUSET_RAFT, &apply_state_key)
            .unwrap()
            .unwrap(),
        apply_state
    );
    ...
    let mut brane_state = raft_serverpb::BraneLocalState::default();
    brane_state.set_state(raft_serverpb::PeerState::Tombstone);
    kv_engine
        .c()
    ...
    assert_eq!(
        kv_engine
            .c()
            .get_msg_causet::<raft_serverpb::BraneLocalState>(CAUSET_RAFT, &brane_state_key)
            .unwrap()
            .unwrap(),
        brane_state
    );
