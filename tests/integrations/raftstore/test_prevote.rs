...
fn test_prevote<T: Simulator>(
    ...
    cluster.causetg.raft_store.prevote = true;
...
fn test_prevote_reboot_majority_followers() {
    let mut cluster = new_node_cluster(0, 5);
    // A prevote round will spacelike, but nothing will succeed.
    test_prevote(
        &mut cluster,
...
fn test_prevote_reboot_minority_followers() {
    let mut cluster = new_node_cluster(0, 5);
    // A prevote round will spacelike, but nothing will succeed until recovery.
    test_prevote(
        &mut cluster,
...
fn test_pair_isolated<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    // Then, add a policy to FIDel that it should ask the VioletaBft leader to remove the peer from the group.
    fidel_client.must_remove_peer(brane, new_peer(4, 4));
    fidel_client.must_remove_peer(brane, new_peer(5, 5));

    // Verify the nodes have self removed.
    cluster.must_remove_brane(4, brane);
    cluster.must_remove_brane(5, brane);
}
...
fn test_isolated_follower_leader_does_not_change<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.must_put(b"k1", b"v1");
    let brane_status = new_status_request(1, new_peer(1, 1), new_brane_leader_cmd());
    let resp = cluster
        .call_command(brane_status.clone(), Duration::from_secs(5))
        .unwrap();
    let term = resp.get_header().get_current_term();
    // Isolate peer5.
    cluster.partition(vec![1, 2, 3, 4], vec![5]);
    let election_timeout = cluster.causetg.raft_store.raft_base_tick_interval.0
        * cluster.causetg.raft_store.raft_election_timeout_ticks as u32;
    ...
    let leader = cluster.leader_of_brane(1);
