...
fn test_auto_split_brane<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.causetg.raft_store.split_brane_check_tick_interval = ReadableDuration::millis(100);
    ...
    let check_size_diff = cluster.causetg.raft_store.brane_split_check_diff.0;
    let mut cone = 1..;
...
fn test_delay_split_brane() {
    ...
    cluster.causetg.raft_store.raft_log_gc_count_limit = 500;
    cluster.causetg.raft_store.merge_max_log_gap = 100;
...
fn test_apply_new_version_snapshot<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 5;
...
fn test_split_brane_diff_check<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.split_brane_check_tick_interval = ReadableDuration::millis(100);
    cluster.causetg.raft_store.brane_split_check_diff = ReadableSize(10);
    cluster.causetg.raft_store.raft_log_gc_tick_interval = ReadableDuration::secs(20);
...
fn test_quick_election_after_split<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let reserved_time =
        Duration::from_millis(cluster.causetg.raft_store.raft_base_tick_interval.as_millis() * 2);
