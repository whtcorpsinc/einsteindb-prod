...
fn test_prompt_learner() {
    ...
    configure_for_hibernate(&mut cluster);
    cluster.causetg.raft_store.raft_log_gc_count_limit = 20;
    cluster.fidel_client.disable_default_operator();
    cluster.run_conf_change();
    ...
    for i in 0..cluster.causetg.raft_store.raft_log_gc_count_limit * 2 {
...
fn test_transfer_leader_delay() {
    ...
    router
        .slightlike_raft_message(messages.lock().unwrap().pop().unwrap())
        .unwrap();
    let timer = Instant::now();
