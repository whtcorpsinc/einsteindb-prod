...
fn test_compact_lock_causet<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.lock_causet_compact_interval = ReadableDuration::millis(interval);
    // Set lock_causet_compact_bytes_memory_barrier.
    cluster.causetg.raft_store.lock_causet_compact_bytes_memory_barrier = ReadableSize(100);
    cluster.run();
