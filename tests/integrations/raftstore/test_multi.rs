...
fn test_multi_base_after_bootstrap<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let prev_last_index = cluster.raft_local_state(brane_id, 1).get_last_index();
    ...
    let last_index = cluster.raft_local_state(brane_id, 1).get_last_index();
...
fn test_leader_change_with_uncommitted_log<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.causetg.raft_store.raft_election_timeout_ticks = 50;
...
fn test_consistency_check<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    // disable compact log to make test more stable.
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 1000;
    cluster.causetg.raft_store.consistency_check_interval = ReadableDuration::secs(1);
    // We use three peers([1, 2, 3]) for this test.
    cluster.run();
...
fn test_node_catch_up_logs() {
    let mut cluster = new_node_cluster(0, 3);
    cluster.causetg.raft_store.raft_base_tick_interval = ReadableDuration::millis(500);
    cluster.causetg.raft_store.raft_max_size_per_msg = ReadableSize(5);
    cluster.causetg.raft_store.raft_election_timeout_ticks = 50;
    cluster.causetg.raft_store.max_leader_missing_duration = ReadableDuration::hours(1);
    cluster.causetg.raft_store.peer_stale_state_check_interval = ReadableDuration::minutes(30);
    cluster.causetg.raft_store.abnormal_leader_missing_duration = ReadableDuration::hours(1);
    // disable compact log to make test more stable.
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 3000;
