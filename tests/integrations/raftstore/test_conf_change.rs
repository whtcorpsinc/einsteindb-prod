...
use ekvproto::metapb;
use ekvproto::raft_cmdpb::{VioletaBftCmdResponse, VioletaBftResponseHeader};
...
fn test_after_remove_itself<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.raft_log_gc_memory_barrier = 10000;
    ...
    for _ in 0..250 {
        let brane: BraneLocalState = engine1
            .c()
            .get_msg_causet(CAUSET_RAFT, &tuplespaceInstanton::brane_state_key(r1))
            .unwrap()
            .unwrap();
...
fn test_split_brain<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let base_tick = cluster.causetg.raft_store.raft_base_tick_interval.0;
    let election_timeout = base_tick * cluster.causetg.raft_store.raft_election_timeout_ticks as u32;
...
fn test_transfer_leader_safe<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    thread::sleep(causetg.raft_store.raft_reject_transfer_leader_duration.into());
    assert_eq!(
        cluster.leader_of_brane(brane_id).unwrap().get_id(),
        leader_id
    );
...
fn test_conf_change_remove_leader() {
    ...
    cluster.causetg.raft_store.allow_remove_leader = false;
    let fidel_client = Arc::clone(&cluster.fidel_client);
    fidel_client.disable_default_operator();
    let r1 = cluster.run_conf_change();
    fidel_client.must_add_peer(r1, new_peer(2, 2));
