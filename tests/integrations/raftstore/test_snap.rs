...
use violetabft::eraftpb::{Message, MessageType};
...
fn test_huge_snapshot<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.causetg.raft_store.snap_apply_batch_size = ReadableSize(500);
...
fn test_server_snap_gc() {
    ...
    cluster.causetg.raft_store.snap_gc_timeout = ReadableDuration::millis(300);
                ...
                && m.get_to_peer().get_store_id() == 3
            {
                if self.inner.first_snap.rl().is_none() {
                    *self.inner.first_snap.wl() = Some(m.take_message());
                    ...
                    let _ = self.inner.sent.lock().unwrap().slightlike(());
                }
            }
            res.push(m);
        }
    ...
    fn before(&self, msgs: &mut Vec<VioletaBftMessage>) -> Result<()> {
        ...
        for msg in msgs.drain(..) {
            ...
            if should_collect {
                ...
                self.notifier.lock().unwrap().slightlike(()).unwrap();
            } else {
                if stale {
...
fn test_request_snapshot_apply_repeatedly() {
    ...
    let raft_key = tuplespaceInstanton::raft_state_key(brane_id);
    let raft_state: VioletaBftLocalState = engine.c().get_msg(&raft_key).unwrap().unwrap();
    assert!(
