...
use ekvproto::raft_serverpb::{PeerState, VioletaBftMessage, BraneLocalState, StoreIdent};
...
use engine_promises::{SyncMutable, CAUSET_RAFT};
...
fn test_tombstone<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let mut raft_msg = VioletaBftMessage::default();

    raft_msg.set_brane_id(r1);
    // Use an invalid from peer to ignore gc peer message.
    raft_msg.set_from_peer(new_peer(0, 0));
    raft_msg.set_to_peer(new_peer(2, 2));
    raft_msg.mut_brane_epoch().set_conf_ver(0);
    raft_msg.mut_brane_epoch().set_version(0);

    cluster.slightlike_raft_msg(raft_msg).unwrap();
...
fn test_fast_destroy<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    let state: BraneLocalState = engine_3.c().get_msg_causet(CAUSET_RAFT, &key).unwrap().unwrap();
    assert_eq!(state.get_state(), PeerState::Tombstone);
...
fn test_readd_peer<T: Simulator>(cluster: &mut Cluster<T>) {
    ...
    cluster.slightlike_raft_msg(gc_msg).unwrap();
...
fn test_server_stale_meta() {
    ...
    let mut state: BraneLocalState = engine_3
        .c()
        .get_msg_causet(CAUSET_RAFT, &tuplespaceInstanton::brane_state_key(1))
        .unwrap()
        .unwrap();
    ...
    engine_3
        .c()
        .put_msg_causet(CAUSET_RAFT, &tuplespaceInstanton::brane_state_key(1), &state)
        .unwrap();
    cluster.clear_slightlike_filters();
...
fn test_safe_tombstone_gc() {
    ...
    let tick = cluster.causetg.raft_store.raft_election_timeout_ticks;
    ...
    cluster.causetg.raft_store.peer_stale_state_check_interval = ReadableDuration(check_interval);
    cluster.causetg.raft_store.abnormal_leader_missing_duration = ReadableDuration(check_interval * 2);
    cluster.causetg.raft_store.max_leader_missing_duration = ReadableDuration(check_interval * 2);
    ...
    while timer.elapsed() < Duration::from_secs(5) {
        state = cluster.get_engine(4).c().get_msg_causet(CAUSET_RAFT, &key).unwrap();
        if state.is_some() {
            break;
        }
