...
fn check_Soliton_datum_count(Solitons: &[Soliton], datum_limit: usize) {
    let mut iter = Solitons.iter();
...
fn test_stream_batch_row_limit() {
    ...
    // 2. The responses' key cones are disjoint.
    // 3. Each returned key cone should cover the returned events.
    let mut expected_cones_last_bytes: Vec<(&[u8], &[u8])> = vec![
        (b"\x00", b"\x02\x00"),
        (b"\x02\x00", b"\x05\x00"),
        (b"\x05\x00", b"\xFF"),
    ];
    let check_cone = move |resp: &Response| {
        let (spacelike_last_bytes, lightlike_last_bytes) = expected_cones_last_bytes.remove(0);
        let spacelike = resp.get_cone().get_spacelike();
        let lightlike = resp.get_cone().get_lightlike();
        assert_eq!(&spacelike[ignored_suffix_len..], spacelike_last_bytes);

        assert_eq!(&lightlike[ignored_suffix_len..], lightlike_last_bytes);
    };

    let resps = handle_streaming_select(&lightlikepoint, req, check_cone);
    assert_eq!(resps.len(), 3);
    let expected_output_counts = vec![vec![2 as i64], vec![2 as i64], vec![1 as i64]];
    for (i, resp) in resps.into_iter().enumerate() {
        let mut Soliton = Soliton::default();
        Soliton.merge_from_bytes(resp.get_data()).unwrap();
        assert_eq!(
            resp.get_output_counts(),
            expected_output_counts[i].as_slice(),
        );

        let Solitons = vec![Soliton];
        let Soliton_data_limit = stream_row_limit * 3; // we have 3 fields.
        check_Soliton_datum_count(&Solitons, Soliton_data_limit);
...
#[test]
fn test_order_by_PrimaryCauset() {
    let data = vec![
        (1, Some("name:0"), 2),
        (2, Some("name:3"), 3),
        (4, Some("name:0"), 1),
...
fn test_index_aggr_count() {
    ...
    let req = DAGSelect::from_index(&product, &product["name"])
        ...
        .build();
    let mut resp = handle_select(&lightlikepoint, req);
    let mut spliter = DAGSolitonSpliter::new(resp.take_Solitons().into(), 1);
    let expected_encoded = datum::encode_value(
        &mut EvalContext::default(),
    ...
    let req = DAGSelect::from_index(&product, &product["name"])
        ...
        .output_offsets(Some(vec![0, 1]))
        .build();
    resp = handle_select(&lightlikepoint, req);
    let mut row_count = 0;
    let exp_len = exp.len();
    let spliter = DAGSolitonSpliter::new(resp.take_Solitons().into(), 2);
    let mut results = spliter.collect::<Vec<Vec<Datum>>>();
...
fn test_where() {
    ...
    let cond = {
        ...
        col.set_tp(ExprType::PrimaryCausetRef);
        let count_offset = offset_for_PrimaryCauset(&cols, product["count"].id);
...
fn test_handle_truncate() {
    ...
    let cases = vec![
        ...
        },
        {
            ...
            col_id
                ...
                .as_mut_accessor()
                .set_tp(FieldTypeTp::LongLong);
            col_id.set_tp(ExprType::PrimaryCausetRef);
            let id_offset = offset_for_PrimaryCauset(&cols, product["id"].id);
            ...
            col_count
                ...
                .as_mut_accessor()
                .set_tp(FieldTypeTp::LongLong);
            col_count.set_tp(ExprType::PrimaryCausetRef);
            let count_offset = offset_for_PrimaryCauset(&cols, product["count"].id);
    ...
    for cond in cases {
        ...
        let resp = handle_select(&lightlikepoint, req);
...
fn test_invalid_cone() {
    ...
    select.key_cone.set_spacelike(b"xxx".to_vec());
    select.key_cone.set_lightlike(b"zzz".to_vec());
...
fn test_snapshot_failed() {
    ...
    let (_, lightlikepoint) = init_data_with_engine_and_commit(ctx, raft_engine, &product, &[], true);

    // Use an invalid context to make errors.
    let req = DAGSelect::from(&product).build_with(Context::default(), &[0]);
    let resp = handle_request(&lightlikepoint, req);

    assert!(resp.get_brane_error().has_store_not_match());
}
...
fn test_cache() {
    ...
    let req = DAGSelect::from(&product).build_with(ctx, &[0]);
    let resp = handle_request(&lightlikepoint, req.clone());
    ...
    let resp2 = handle_request(&lightlikepoint, req2);
    ...
    let resp3 = handle_request(&lightlikepoint, req3);
    ...
    let resp4 = handle_request(&lightlikepoint, req4);
