...
fn stale_read_during_splitting(right_derive: bool) {
    ...
    // Get the first brane.
    let brane_left = cluster.get_brane(key1);
    let brane_right = cluster.get_brane(key2);
    assert_eq!(brane_left, brane_right);
    let brane1 = brane_left;
    assert_eq!(brane1.get_id(), 1);
    let peer3 = brane1
        .get_peers()
        .iter()
        .find(|p| p.get_id() == 3)
        .unwrap()
        .clone();
    cluster.must_transfer_leader(brane1.get_id(), peer3.clone());
    ...
    // of the new brane.
    //           EinsteinDB A  EinsteinDB B  EinsteinDB C
    // Brane 1    L       F       F
    // Brane 2    X       L       F
    ...
    // A key that is covered by the old brane and the new brane.
    let stale_key = if right_derive { key1 } else { key2 };
    // Get the new brane.
    let brane2 = cluster.get_brane_with(stale_key, |brane| brane != &brane1);

    // Get the leader of the new brane.
    let leader2 = cluster.leader_of_brane(brane2.get_id()).unwrap();
...
fn must_not_stale_read(
    ...
    let mut request = new_request(
        ...
        vec![new_put_causet_cmd("default", stale_key, v3)],
        false,
    );
...
fn test_stale_read_during_merging() {
    ...
    fidel_client.must_add_peer(brane.get_id(), new_peer(2, 4));
    fidel_client.must_add_peer(brane.get_id(), new_peer(3, 5));

    cluster.must_split(&brane, b"k2");

    let mut brane1 = cluster.get_brane(key1);
    let mut brane1000 = cluster.get_brane(key2);
    assert_ne!(brane1, brane1000);
    assert_eq!(brane1.get_id(), 1); // requires disable right_derive.
    let leader1 = brane1
        .get_peers()
        .iter()
        .find(|p| p.get_id() == 4)
        .unwrap()
        .clone();
    cluster.must_transfer_leader(brane1.get_id(), leader1.clone());

    let leader1000 = brane1000
        .get_peers()
        .iter()
        .find(|p| p.get_store_id() != leader1.get_store_id())
        .unwrap()
        .clone();
    cluster.must_transfer_leader(brane1000.get_id(), leader1000.clone());
    ...
    //             merge into
    // brane1000 ------------> brane1
    cluster.must_try_merge(brane1000.get_id(), brane1.get_id());
    ...
    // We can not use `get_brane_with` to get the latest info of reigon 1000,
    // because leader1 is not paused, it executes commit merge very fast
    // and reports fidel, its cone covers brane1000.
    //
    // brane1000 does prepare merge, it increases ver and conf_ver by 1.
    debug!("before merge: {:?} | {:?}", brane1000, brane1);
    let brane1000_version = brane1000.get_brane_epoch().get_version() + 1;
    brane1000
        .mut_brane_epoch()
        .set_version(brane1000_version);
    let brane1000_conf_version = brane1000.get_brane_epoch().get_conf_ver() + 1;
    brane1000
        .mut_brane_epoch()
        .set_conf_ver(brane1000_conf_version);

    // Epoch changed by commit merge.
    brane1 = cluster.get_brane_with(key1, |brane| brane != &brane1);
    debug!("after merge: {:?} | {:?}", brane1000, brane1);

    // A key that is covered by brane 1000 and brane 1.
    let stale_key = key2;
...
fn test_read_index_when_transfer_leader_2() {
    ...
    let filter = Box::new(
        BranePacketFilter::new(r1.get_id(), old_leader.get_store_id())
...
fn test_read_after_peer_destroyed() {
    ...
    let mut request = new_request(
        r1,
        cluster.fidel_client.get_brane_epoch(r1),
        vec![new_get_cmd(b"k1")],
        false,
    );
    ...
    cluster
        ...
        .unwrap();
    // Wait for violetabftstore receives the read request.
    sleep_ms(200);
