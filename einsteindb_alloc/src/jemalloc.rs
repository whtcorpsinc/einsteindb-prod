...
use einsteindb_jemalloc_ctl::{epoch, stats, Error};
use einsteindb_jemalloc_sys::malloc_stats_print;

pub type Allocator = einsteindb_jemallocator::Jemalloc;
pub const fn allocator() -> Allocator {
    einsteindb_jemallocator::Jemalloc
}
...
mod profiling {
    ...
    pub fn activate_prof() -> ProfResult<()> {
        ...
        unsafe {
            if let Err(e) = einsteindb_jemalloc_ctl::raw::ufidelate(PROF_ACTIVE, true) {
    ...
    pub fn deactivate_prof() -> ProfResult<()> {
        info!("stop profiler");
        unsafe {
            if let Err(e) = einsteindb_jemalloc_ctl::raw::ufidelate(PROF_ACTIVE, false) {
    ...
    pub fn dump_prof(path: &str) -> ProfResult<()> {
        ...
        let res = unsafe { einsteindb_jemalloc_ctl::raw::ufidelate(PROF_DUMP, ptr) };
        match res {
            Err(e) => {
    ...
    mod tests {
        ...
        fn is_profiling_on() -> bool {
            match unsafe { einsteindb_jemalloc_ctl::raw::read(OPT_PROF) } {
                Err(e) => {
