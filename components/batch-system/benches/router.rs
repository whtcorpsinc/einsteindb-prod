...
fn bench_slightlike(c: &mut Criterion) {
    ...
    c.bench_function("router::slightlike", |b| {
        b.iter(|| {
            router.slightlike(1, Message::Loop(0)).unwrap();
        })
    });
    system.shutdown();
}

criterion_group!(benches, bench_slightlike);
