...
//! This crate controls the global allocator used by EinsteinDB.
//!
//! As of now EinsteinDB always turns on jemalloc on Unix, though libraries
...
//!
//! Writing `extern crate einsteindb_alloc;` will link it to jemalloc when
//! appropriate. The EinsteinDB library itself links to `einsteindb_alloc` to
//! ensure that any binary linking to it will use jemalloc.
//!
//! With few exceptions, _every binary and project in the EinsteinDB workspace
//! should link (perhaps transitively) to einsteindb_alloc_. This is to ensure
//! that tests and benchmarks run with the production allocator. In other
//! words, binaries and projects that don't link to `einsteindb` should link
//! to `einsteindb_alloc`.
//!
...
//! platforms Lmdb is using the system malloc. On Linux C malloc is
...
//!   capability
//!
//! - jemalloc - compiles einsteindb-jemallocator (default)
//!
//! - tcmalloc - compiles tcmalloc
//!
//! causetg `fuzzing` is defined by `run_libfuzzer` in `fuzz/cli.rs` and
//! is passed to rustc directly with `--causetg`; in other words it's not
...
//! cargo test --features mem-profiling -p einsteindb_alloc -- --ignored
//! ```
//!
...
//! and rustc, which are both configured without profiling. EinsteinDB's
//! jemalloc is configured for profiling if you pass
//! `--features=mem-profiling` to cargo for eather `einsteindb_alloc` or
//! `einsteindb`.
