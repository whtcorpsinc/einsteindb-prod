...
pub fn brane_raft_prefix(brane_id: u64) -> [u8; 10] {
    let mut key = [0; 10];
...
pub fn brane_raft_prefix_len() -> usize {
    // REGION_RAFT_PREFIX_KEY + brane_id + suffix
    REGION_RAFT_PREFIX_KEY.len() + mem::size_of::<u64>() + 1
}

pub fn raft_log_key(brane_id: u64, log_index: u64) -> [u8; 19] {
    make_brane_key(brane_id, RAFT_LOG_SUFFIX, log_index)
}

pub fn raft_state_key(brane_id: u64) -> [u8; 11] {
    make_brane_prefix(brane_id, RAFT_STATE_SUFFIX)
}

pub fn snapshot_raft_state_key(brane_id: u64) -> [u8; 11] {
    make_brane_prefix(brane_id, SNAPSHOT_RAFT_STATE_SUFFIX)
}
...
/// Get the log index from violetabft log key generated by `raft_log_key`.
...
/// Get the brane id and index from violetabft log key generated by `raft_log_key`.
pub fn decode_raft_log_key(key: &[u8]) -> Result<(u64, u64)> {
    ...
    if key.len() != expect_key_len
        || !key.spacelikes_with(REGION_RAFT_PREFIX_KEY)
        || key[suffix_idx] != RAFT_LOG_SUFFIX
    {
        return Err(Error::InvalidVioletaBftLogKey(key.to_owned()));
    }
    let brane_id = BigEndian::read_u64(&key[REGION_RAFT_PREFIX_KEY.len()..suffix_idx]);
    let index = BigEndian::read_u64(&key[suffix_idx + mem::size_of::<u8>()..]);
    Ok((brane_id, index))
}

pub fn raft_log_prefix(brane_id: u64) -> [u8; 11] {
    make_brane_prefix(brane_id, RAFT_LOG_SUFFIX)
}

/// Decode brane violetabft key, return the brane id and violetabft suffix type.
pub fn decode_brane_raft_key(key: &[u8]) -> Result<(u64, u8)> {
    decode_brane_key(REGION_RAFT_PREFIX_KEY, key, "violetabft")
}
...
mod tests {
    ...
    fn test_brane_id_key() {
        ...
        for brane_id in brane_ids {
            let prefix = brane_raft_prefix(brane_id);

            assert!(raft_log_prefix(brane_id).spacelikes_with(&prefix));
            assert!(raft_log_key(brane_id, 1).spacelikes_with(&prefix));
            assert!(raft_state_key(brane_id).spacelikes_with(&prefix));
        ...
        for (lid, rid, order) in tbls {
            let lhs = brane_raft_prefix(lid);
            let rhs = brane_raft_prefix(rid);
    ...
    fn test_brane_key() {
        ...
        for id in ids {
            ...
            let raft_prefix = brane_raft_prefix(id);
            let raft_apply_key = apply_state_key(id);
            assert!(raft_apply_key.spacelikes_with(&raft_prefix));

            assert_eq!(
                decode_brane_raft_key(&raft_apply_key).unwrap(),
    ...
    fn test_raft_log_key() {
        for brane_id in 1..10 {
            for idx_id in 1..10 {
                let key = raft_log_key(brane_id, idx_id);
                assert_eq!(idx_id, raft_log_index(&key).unwrap());
                assert_eq!((brane_id, idx_id), decode_raft_log_key(&key).unwrap());
            }
        }
        ...
        // invalid prefix
        assert!(decode_raft_log_key(&brane_state_key).is_err());
    }
