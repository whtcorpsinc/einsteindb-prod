...
impl CaseTraceLogger {
    fn write_log(
        ...
        let tag = einsteindb_util::get_tag_from_thread_name().map_or_else(|| "".to_owned(), |s| s + " ");
        let t = time::now();
...
pub fn init_log_for_test() {
    ...
    let level = einsteindb_util::logger::get_level_by_string(
    ...
    einsteindb_util::logger::init_log(
        drainer,
        level,
