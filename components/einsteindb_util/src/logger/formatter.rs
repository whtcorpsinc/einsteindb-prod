    ...
    W: io::Write + ?Sized,
{
    ...
    for (index, &b) in bytes.iter().enumerate() {
        ...
        if spacelike < index {
            writer.write_all((&file_name[spacelike..index]).as_bytes())?;
        }
        spacelike = index + 1;
    }
    if spacelike < bytes.len() {
        writer.write_all((&file_name[spacelike..]).as_bytes())?;
    }
    Ok(())
}
...
///
/// [RFC: Unified Log Format]: (https://github.com/einsteindb/rfcs/blob/master/text/2018-12-19-unified-log-format.md)
///
#[inline]
