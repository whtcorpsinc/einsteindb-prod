...
pub trait BufferReader {
    /// Returns a slice spacelikeing at current position.
    ///
...
impl BufferWriter for Vec<u8> {
    unsafe fn bytes_mut(&mut self, size: usize) -> &mut [u8] {
        // Return a slice spacelikeing from `self.len()` position and
    ...
    #[inline]
    fn write_bytes(&mut self, values: &[u8]) -> Result<()> {
        self.extlightlike_from_slice(values);
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_vec_reallocate() {
        ...
        // beyond the length of the vector). It also deplightlikes on behavior
        ...
        // on platforms where EinsteinDB uses jemalloc.
