...
use num::promises::{Zero, One};
use ordered_float::OrderedFloat;

use edbn::parse::{self, ParseError};
use edbn::types::{
    Value,
    ValueAndSpan,
    Span,
...
// Helpers for parsing strings and converting them into edbn::Value.
...
fn test_text() {
    ...
    let raw_edbn = r#""This string contains a \" and a \\""#;
    let raw_string = r#"This string contains a " and a \"#;
    assert_eq!(parse::value(raw_edbn).unwrap(),
               ValueAndSpan {
                   inner: SpannedValue::Text(raw_string.to_string()),
                   span: Span(0, raw_edbn.len() as u32)
               });
}
...
fn test_set() {
    ...
    // These tests assume the impleeinsteindbion of Ord for Value, (specifically the sort order which
...
/// The `test_causetq_*` functions contain the queries taken from the old Clojure impleeinsteindbion of EinsteinDB.
...
/// Secondly, see note in `test_causetq_starred_pages` on the use of '
#[test]
fn test_causetq_active_sessions() {
    use self::Value::*;
    ...
    let test = "[
        ...
        :in $
        :where
            [?id :session/startReason ?reason ?causetx]
            [?causetx :edb/causecausetxInstant ?ts]
...
#[test]
fn test_causetq_ended_sessions() {
    use self::Value::*;
    ...
    let test = "[
        ...
        :in $
        :where
            [?id :session/endReason ?endReason ?causetx]
            [?causetx :edb/causecausetxInstant ?ts]
    ]";
...
#[test]
fn test_causetq_starred_pages() {
    use self::Value::*;

    // TODO: The original causetq had added "'" like `:find '[?url` and `since '[$ ?since] '[$]`
    let test = "[
...
#[test]
fn test_causetq_pages_matching_string_1() {
    use self::Value::*;
...
#[test]
fn test_causetq_pages_matching_string_2() {
    use self::Value::*;
...
macro_rules! def_test_as_value_type {
    ($value: causetid, $method: causetid, $is_some: expr, $expected: expr) => {
        if $is_some {
