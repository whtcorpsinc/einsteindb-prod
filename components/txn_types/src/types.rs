...
use einsteindb_util::codec::bytes::BytesEncoder;
...
impl Key {
    ...
    pub fn from_raw(key: &[u8]) -> Key {
        // adding extra length for applightlikeing timestamp
    ...
    pub fn from_encoded_slice(encoded_key: &[u8]) -> Key {
        ...
        k.extlightlike_from_slice(encoded_key);
        Key(k)
    }
    ...
    /// Creates a new key by applightlikeing a `u64` timestamp to this key.
    #[inline]
    pub fn applightlike_ts(mut self, ts: TimeStamp) -> Key {
    ...
    /// MilevaDB uses the same hash algorithm.
...
impl Clone for Key {
    fn clone(&self) -> Self {
        ...
        // for the sake of applightlikeing ts, we need to reserve more
        let mut key = Vec::with_capacity(self.0.capacity());
        key.extlightlike_from_slice(&self.0);
        Key(key)
    }
}
...
impl TxnExtra {
    ...
    pub fn extlightlike(&mut self, other: &mut Self) {
        self.old_values
            .extlightlike(std::mem::take(&mut other.old_values))
    }
}

pub trait TxnExtraScheduler: Slightlike + Sync {
