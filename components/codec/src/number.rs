...
impl NumberCodec {
    ...
    /// which is memory-comparable in asclightlikeing order.
    ///
    /// # Panics
    ///
    ...
    /// Encodes an unsigned 64 bit integer `v` to `buf`,
    /// which is memory-comparable in desclightlikeing order.
    ///
    /// # Panics
    ///
    /// Panics when `buf.len() < 8`.
    ...
    /// Encodes an unsigned 16 bit integer `v` to `buf` in little lightlikeian,
    ...
    /// Decodes an unsigned 16 bit integer from `buf` in little lightlikeian,
    ...
    /// Encodes a signed 16 bit integer `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes a signed 16 bit integer from `buf` in little lightlikeian,
    ...
    /// Encodes a 32 bit float number `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes a 32 bit float number `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Encodes an unsigned 32 bit integer `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes an unsigned 32 bit integer from `buf` in little lightlikeian,
    ...
    /// Encodes a signed 32 bit integer `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes a signed 32 bit integer from `buf` in little lightlikeian,
    ...
    /// Encodes an unsigned 64 bit integer `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes an unsigned 64 bit integer from `buf` in little lightlikeian,
    ...
    /// Encodes a signed 64 bit integer `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes a signed 64 bit integer from `buf` in little lightlikeian,
    ...
    /// Encodes a 64 bit float number `v` to `buf` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Panics
    ///
    ...
    /// Decodes a 64 bit float number from `buf` in little lightlikeian,
    ...
    pub fn try_decode_var_u64(buf: &[u8]) -> Result<(u64, usize)> {
        ...
        unsafe {
            ...
            if likely(len >= MAX_VARINT64_LENGTH) {
                ...
                let ptr_lightlike = buf.as_ptr().add(len);
                // Slow path
                let mut shift = 0;
                while ptr != ptr_lightlike && *ptr >= 0x80 {
                    val |= ((*ptr & 0x7f) as u64) << shift;
                    shift += 7;
                    ptr = ptr.add(1);
                }
                if unlikely(ptr == ptr_lightlike) {
    ...
    pub fn get_first_encoded_var_int_len(buf: &[u8]) -> usize {
        unsafe {
            ...
            if likely(len >= MAX_VARINT64_LENGTH) {
                ...
                // When we got here, we are either `ptr == ptr_lightlike`, or `*ptr < 0x80`.
                // For `ptr == ptr_lightlike` case, it means we are expecting a value < 0x80
...
pub trait NumberDecoder: BufferReader {
    ...
    /// Reads an unsigned 16 bit integer in little lightlikeian,
    ...
    /// Reads a signed 16 bit integer in little lightlikeian,
    ...
    /// Reads an unsigned 32 bit integer in little lightlikeian,
    ...
    /// Reads a signed 32 bit integer in little lightlikeian,
    ...
    /// Reads a 32 bit float  in little lightlikeian,
    ...
    /// Reads an unsigned 64 bit integer in little lightlikeian,
    ...
    /// Reads a signed 64 bit integer in little lightlikeian,
    ...
    /// Reads a 64 bit float number in little lightlikeian,
...
pub trait NumberEncoder: BufferWriter {
    ...
    /// Writes an unsigned 16 bit integer `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes an unsigned 16 bit integer `v` in big lightlikeian,
    ...
    /// Writes a signed 16 bit integer `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes an unsigned 32 bit integer `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes an unsigned 32 bit integer `v` in big lightlikeian,
    /// which is memory-comparable.
    ///
    ...
    /// Writes a signed 32 bit integer `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes a 32 bit float number `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes an unsigned 64 bit integer `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes a signed 64 bit integer `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
    ...
    /// Writes a 64 bit float number `v` in little lightlikeian,
    /// which is not memory-comparable.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn get_u32_samples() -> Vec<u32> {
        ...
        samples.extlightlike(get_u16_samples().into_iter().map(|v| v as u32));
        samples
    }
    ...
    fn get_u64_samples() -> Vec<u64> {
        ...
        samples.extlightlike(get_u32_samples().into_iter().map(|v| v as u64));
        samples
    }
    ...
    macro_rules! test_codec {
        ($samples:expr, $enc:ident, $dec:ident, $buf_enc:ident, $buf_dec:ident,) => {
            for sample in $samples {
                ...
                // Encode to a `Cursor` (backed by Vec) with existing data at beginning and lightlike
                ...
                // Decode from a `Cursor` with existing data at beginning and lightlike
                for buf_len in len..len + 20 {
                    let mut payload: Vec<u8> = Vec::with_capacity(buf_len);
                    for _ in 0..buf_len {
                        payload.push(rand::random::<u8>());
...
mod benches {
    ...
    /// Encode u64 little lightlikeian using `NumberCodec` and store position in extra variable.
    #[bench]
    ...
    /// Encode u64 little lightlikeian using `byteorder::WriteBytesExt` over a `Cursor<&mut [u8]>`.
    #[bench]
    ...
    /// Encode u64 little lightlikeian using `NumberEncoder` over a `Cursor<&mut [u8]>`.
    #[bench]
    ...
    /// Encode u64 little lightlikeian using `NumberEncoder` over a `Vec<u8>`.
    #[bench]
    ...
    /// Decode u64 little lightlikeian using `NumberCodec` and store position in extra variable.
    #[bench]
    ...
    /// Decode u64 little lightlikeian using `NumberCodec` and store position via slice index.
    #[bench]
    ...
    /// Decode u64 little lightlikeian using `byteorder::ReadBytesExt` over a `Cursor<&[u8]>`.
    #[bench]
    ...
    /// The original implementation in EinsteinDB
    ...
    /// Decode u64 little lightlikeian using `bytes::Buf` over a `Cursor<&[u8]>`.
    #[bench]
    ...
    /// Decode u64 little lightlikeian using `NumberDecoder` over a `Cursor<&[u8]>`.
    #[bench]
    ...
    /// Decode u64 little lightlikeian using the original implementation in EinsteinDB.
    #[bench]
    ...
    /// Encode u64 in VarInt using original EinsteinDB implementation.
    #[bench]
    ...
    pub fn decode_var_u64_original(data: &mut &[u8]) -> super::Result<u64> {
        if !data.is_empty() {
            // process with value < 127 indeplightlikeently at first
            ...
            // process with data's len >=10 or data lightlikes with var u64
        ...
        // process data's len < 10 && data not lightlike with var u64.
        let mut res = 0;
        for i in 0..data.len() {
            let b = data[i];
            res |= (u64::from(b) & 0x7f) << (i * 7);
    ...
    /// Decode u64 < 128 in VarInt using original EinsteinDB implementation.
    #[bench]
    ...
    /// Decode normal u64 in VarInt using original EinsteinDB implementation.
    /// The buffer size >= 10.
    #[bench]
