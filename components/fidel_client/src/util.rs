    ...
    F: Fn(&FidelClientStub) -> GrpcResult<R>,
{
    ...
    for _ in 0..retry {
        let ret = {
            ...
            // which may hold the read lock and wait for FIDel client thread completing the request
            // and the FIDel client thread which may block on acquiring the write lock.
