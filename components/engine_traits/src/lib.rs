...
//! A generic EinsteinDB persistence engine
//!
//! This is a work-in-progress attempt to abstract all the features needed by
//! EinsteinDB to persist its data, so that persistence engines other than Lmdb may be
...
//! ## 1) Migrating the `engine` abstractions
//!
//! The engine crate was an earlier attempt to abstract the persistence engine. Much
...
pub use crate::cone::*;
mod raft_engine;
pub use raft_engine::{CacheStats, VioletaBftEngine, VioletaBftLogBatch};
