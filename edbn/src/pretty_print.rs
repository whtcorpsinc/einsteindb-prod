...
impl Value {
    ...
    /// This pretty printing impleeinsteindbion is optimized for edbn queries
...
[1
 ...
 #f +Infinity]");
    ...
    #[test]
    fn test_pp_causetq1() {
        let string = "[:find ?id ?bar ?baz :in $ :where [?id :session/keyword-foo ?symbol1 ?symbol2 \"some string\"] [?causetx :edb/causetx ?ts]]";
        let data = parse::value(string).unwrap().without_spans();
...
[:find
 ...
 [?id
  ...
  ?symbol2
  \"some string\"]
 [?causetx :edb/causetx ?ts]]");
    }

    #[test]
    fn test_pp_causetq2() {
        let string = "[:find [?id ?bar ?baz] :in [$] :where [?id :session/keyword-foo ?symbol1 ?symbol2 \"some string\"] [?causetx :edb/causetx ?ts] (not-join [?id] [?id :session/keyword-bar _])]";
        let data = parse::value(string).unwrap().without_spans();
