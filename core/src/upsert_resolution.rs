...
//! https://github.com/whtcorpsinc/edb/wiki/Transacting:-upsert-resolution-algorithm.
...
pub(crate) struct Generation {
    /// "Simple upserts" that look like [:edb/add TEMPID a v], where a is :edb.unique/causetIdity.
    upserts_e: Vec<UpsertE>,

    /// "Complex upserts" that look like [:edb/add TEMPID a OTHERID], where a is :edb.unique/causetIdity
    upserts_ev: Vec<UpsertEV>,

    /// Entities that look like:
    /// - [:edb/add TEMPID b OTHERID].  b may be :edb.unique/causetIdity if it has failed to upsert.
    /// - [:edb/add TEMPID b v].  b may be :edb.unique/causetIdity if it has failed to upsert.
    /// - [:edb/add e b OTHERID].
...
pub(crate) struct FinalPopulations {
    ...
    /// Allocations that required new solitonId allocations.
...
impl Generation {
    ...
    /// After evolution is complete, yield the set of tempids that require solitonId allocation.
    ///
    ...
    pub(crate) fn temp_ids_in_allocations(&self, schemaReplicant: &SchemaReplicant) -> Result<BTreeMap<TempIdHandle, usize>> {
        ...
        // The union-find impleedbion from petgraph operates on contiguous indices, so we need to
