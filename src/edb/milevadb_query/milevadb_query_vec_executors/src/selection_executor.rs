...
pub struct BatchSelectionFreeDaemon<Src: BatchFreeDaemon> {
    context: EvalContext,
    src: Src,
...
impl BatchSelectionFreeDaemon<Box<dyn BatchFreeDaemon<StorageStats = ()>>> {
...
impl<Src: BatchFreeDaemon> BatchSelectionFreeDaemon<Src> {
    ...
    fn handle_src_result(&mut self, src_result: &mut BatchExecuteResult) -> Result<()> {
        // When there are errors in `src_result`, it means that the first several events do not
        ...
        while condition_index < self.conditions.len() && !src_result.logical_rows.is_empty() {
            ...
            src_logical_rows_copy.extlightlike_from_slice(&src_result.logical_rows);
...
fn fidelio_logical_rows_by_vector_value<'a, TT: EvaluableRef<'a>, T: 'a + SolitonRef<'a, TT>>(
    ...
    ctx: &mut EvalContext,
    eval_result: T,
    eval_result_logical_rows: LogicalEvents,
...
impl<Src: BatchFreeDaemon> BatchFreeDaemon for BatchSelectionFreeDaemon<Src> {
    ...
    fn next_batch(&mut self, scan_rows: usize) -> BatchExecuteResult {
        ...
        if let Err(e) = self.handle_src_result(&mut src_result) {
            // TODO: Events before we meeting an evaluation error are innocent.
            src_result.is_drained = src_result.is_drained.and(Err(e));
            src_result.logical_rows.clear();
        } else {
            // Only applightlike warnings when there is no error during filtering because
...
mod tests {
    ...
    fn test_empty_rows() {
        ...
        fn foo() -> Result<Option<i64>> {
            // This function should never be called because we filter no events
            unreachable!()
        }
        ...
        let mut exec = BatchSelectionFreeDaemon::new_for_test(
            src_exec,
        ...
        // When source executor returns empty events, selection executor should process correctly.
    ...
    /// NULL            NULL
    /// (drained)
    fn make_src_executor_using_fixture_1() -> MockFreeDaemon {
        MockFreeDaemon::new(
            vec![FieldTypeTp::LongLong.into(), FieldTypeTp::Double.into()],
            vec![
                BatchExecuteResult {
    ...
    fn test_no_predicate_or_predicate_always_true() {
        ...
        let exec_no_predicate =
            |src_exec: MockFreeDaemon| BatchSelectionFreeDaemon::new_for_test(src_exec, vec![]);

        // Build a selection executor with a predicate that always returns true.
        let exec_predicate_true = |src_exec: MockFreeDaemon| {
            let predicate = RpnExpressionBuilder::new_for_test()
                .push_constant_for_test(1i64)
                .build_for_test();
            BatchSelectionFreeDaemon::new_for_test(src_exec, vec![predicate])
        };
    ...
    fn test_predicate_always_false() {
        ...
        let predicate = RpnExpressionBuilder::new_for_test()
            ...
            .build_for_test();
        let mut exec = BatchSelectionFreeDaemon::new_for_test(src_exec, vec![predicate]);

        // The selection executor should always return empty events.
    ...
    /// NULL            NULL            2
    /// (drained)
    fn make_src_executor_using_fixture_2() -> MockFreeDaemon {
        MockFreeDaemon::new(
            vec![
    ...
    /// Tests the scenario that the predicate returns both true and false. Events that predicate
    /// returns false should be removed from the result.
    #[test]
    fn test_predicate_1() {
        let src_exec = make_src_executor_using_fixture_2();

        // Use FnIsEven(PrimaryCauset[0]) as the predicate.
    ...
    fn test_predicate_2() {
        ...
        // Use is_even(PrimaryCauset[1]) as the predicate.
    ...
    fn test_multiple_predicate_1() {
        // Use [is_even(PrimaryCauset[0]), is_even(PrimaryCauset[1])] as the predicate.

        let predicate: Vec<_> = (0..=1)
            .map(|offset| {
                move || {
                    RpnExpressionBuilder::new_for_test()
                        .push_PrimaryCauset_ref_for_test(offset)
                        .push_fn_call_for_test(is_even_fn_meta(), 1, FieldTypeTp::LongLong)
                        .build_for_test()
                }
            })
