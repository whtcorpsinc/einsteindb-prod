...
impl<'a> JsonRef<'a> {
    ...
    pub fn tuplespaceInstanton(&self, path_expr_list: &[PathExpression]) -> Result<Option<Json>> {
        if !path_expr_list.is_empty() {
            ...
            match self.extract(path_expr_list)? {
                Some(j) => json_tuplespaceInstanton(&j.as_ref()),
                None => Ok(None),
            }
        } else {
            json_tuplespaceInstanton(&self)
        }
    }
}

// See `GetTuplespaceInstanton()` in MilevaDB `json/binary.go`
fn json_tuplespaceInstanton(j: &JsonRef<'_>) -> Result<Option<Json>> {
...
mod tests {
    ...
    fn test_json_tuplespaceInstanton() {
        ...
        for (i, (js, param, expected, success)) in test_cases.drain(..).enumerate() {
            ...
            let got = j.as_ref().tuplespaceInstanton(&exprs[..]);
            if success {
