...
{
    let arg0 = expr.children[0].eval_int(ctx, Evcausetidx)?;
...
{
    for Soliton in expr.children.Solitons(2) {
        if Soliton.len() == 1 {
            // else statement
            return f(&Soliton[0], ctx);
        }
        let cond = Soliton[0].eval_int(ctx, Evcausetidx)?;
        if cond.unwrap_or(0) == 0 {
            continue;
        }
        return f(&Soliton[1], ctx);
    }
    Ok(None)
}

impl ScalarFunc {
    pub fn if_null_int(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<i64>> {
        if_null(|i| self.children[i].eval_int(ctx, Evcausetidx))
    }

    pub fn if_null_real(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<f64>> {
        if_null(|i| self.children[i].eval_real(ctx, Evcausetidx))
    }
    ...
    pub fn if_null_decimal<'a, 'b: 'a>(
        ...
        if_null(|i| self.children[i].eval_decimal(ctx, Evcausetidx))
    }
    ...
    pub fn if_null_string<'a, 'b: 'a>(
        ...
        if_null(|i| self.children[i].eval_string(ctx, Evcausetidx))
    }
    ...
    pub fn if_null_time<'a, 'b: 'a>(
        ...
        if_null(|i| self.children[i].eval_time(ctx, Evcausetidx))
    }
    ...
    pub fn if_null_duration<'a, 'b: 'a>(
        ...
        if_null(|i| self.children[i].eval_duration(ctx, Evcausetidx))
    }
    ...
    pub fn if_null_json<'a, 'b: 'a>(
        ...
        if_null(|i| self.children[i].eval_json(ctx, Evcausetidx))
    }

    pub fn if_int(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<i64>> {
        if_condition(self, ctx, Evcausetidx, |i, ctx| self.children[i].eval_int(ctx, Evcausetidx))
    }

    pub fn if_real(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<f64>> {
        if_condition(self, ctx, Evcausetidx, |i, ctx| {
            self.children[i].eval_real(ctx, Evcausetidx)
        })
    }
    ...
    pub fn if_decimal<'a, 'b: 'a>(
        ...
        if_condition(self, ctx, Evcausetidx, |i, ctx| {
            self.children[i].eval_decimal(ctx, Evcausetidx)
        })
    }
    ...
    pub fn if_string<'a, 'b: 'a>(
        ...
        if_condition(self, ctx, Evcausetidx, |i, ctx| {
            self.children[i].eval_string(ctx, Evcausetidx)
        })
    }
    ...
    pub fn if_time<'a, 'b: 'a>(
        ...
        if_condition(self, ctx, Evcausetidx, |i, ctx| {
            self.children[i].eval_time(ctx, Evcausetidx)
        })
    }
    ...
    pub fn if_duration<'a, 'b: 'a>(
        ...
        if_condition(self, ctx, Evcausetidx, |i, ctx| {
            self.children[i].eval_duration(ctx, Evcausetidx)
        })
    }
    ...
    pub fn if_json<'a, 'b: 'a>(
        ...
        if_condition(self, ctx, Evcausetidx, |i, ctx| {
            self.children[i].eval_json(ctx, Evcausetidx)
        })
    }

    pub fn case_when_int(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<i64>> {
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_int(ctx, Evcausetidx))
    }

    pub fn case_when_real(&self, ctx: &mut EvalContext, Evcausetidx: &[Datum]) -> Result<Option<f64>> {
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_real(ctx, Evcausetidx))
    }
    ...
    pub fn case_when_decimal<'a, 'b: 'a>(
        ...
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_decimal(ctx, Evcausetidx))
    }
    ...
    pub fn case_when_string<'a, 'b: 'a>(
        ...
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_string(ctx, Evcausetidx))
    }
    ...
    pub fn case_when_time<'a, 'b: 'a>(
        ...
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_time(ctx, Evcausetidx))
    }
    ...
    pub fn case_when_duration<'a, 'b: 'a>(
        ...
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_duration(ctx, Evcausetidx))
    }
    ...
    pub fn case_when_json<'a, 'b: 'a>(
        ...
        case_when(self, ctx, Evcausetidx, |v, ctx| v.eval_json(ctx, Evcausetidx))
    }
}
...
mod tests {
    ...
    use crate::Expression;
    use milevadb_query_datatype::codec::mysql::{Duration, Json, Time};
    use milevadb_query_datatype::codec::Datum;
