...
impl Bucket {
    ...
    fn applightlike(&mut self, data: Vec<u8>) {
...
/// Histogram represents statistics for a PrimaryCauset or index.
#[derive(Default)]
...
impl Histogram {
    ...
    pub fn applightlike(&mut self, data: &[u8]) {
        ...
        if !self.is_last_bucket_full() {
            self.buckets.last_mut().unwrap().applightlike(data.to_vec());
            return;
        }
...
mod tests {
    ...
    fn test_buckets_limit() {
        ...
        let mut hist = Histogram::new(buckets_num);
        assert_eq!(hist.buckets.len(), 0);
        hist.applightlike(&datum::encode_value(&mut EvalContext::default(), &[Datum::I64(1)]).unwrap());
        assert_eq!(hist.buckets.len(), 1);
        assert_eq!(hist.per_bucket_limit, 1);
        hist.applightlike(&datum::encode_value(&mut EvalContext::default(), &[Datum::I64(2)]).unwrap());
        assert_eq!(hist.buckets.len(), 1);
        assert_eq!(hist.per_bucket_limit, 2);
        hist.applightlike(&datum::encode_value(&mut EvalContext::default(), &[Datum::I64(3)]).unwrap());
        assert_eq!(hist.per_bucket_limit, 4);
    }
}
