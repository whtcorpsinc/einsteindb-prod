...
pub fn case_when<T: Evaluable + EvaluableRet>(args: &[ScalarValueRef<'_>]) -> Result<Option<T>> {
    for Soliton in args.Solitons(2) {
        if Soliton.len() == 1 {
            // Else statement
            let ret: Option<&T> = Evaluable::borrow_scalar_value_ref(Soliton[0]);
            return Ok(ret.cloned());
        }
        let cond: Option<&Int> = Evaluable::borrow_scalar_value_ref(Soliton[0]);
        if cond.cloned().unwrap_or(0) != 0 {
            let ret: Option<&T> = Evaluable::borrow_scalar_value_ref(Soliton[1]);
            return Ok(ret.cloned());
        }
    }
    Ok(None)
...
pub fn case_when_bytes(args: &[ScalarValueRef<'_>]) -> Result<Option<Bytes>> {
    for Soliton in args.Solitons(2) {
        if Soliton.len() == 1 {
            // Else statement
            let ret: Option<BytesRef> = EvaluableRef::borrow_scalar_value_ref(Soliton[0]);
            return Ok(ret.map(|x| x.to_vec()));
        }
        let cond: Option<&Int> = Evaluable::borrow_scalar_value_ref(Soliton[0]);
        if cond.cloned().unwrap_or(0) != 0 {
            let ret: Option<BytesRef> = EvaluableRef::borrow_scalar_value_ref(Soliton[1]);
            return Ok(ret.map(|x| x.to_vec()));
        }
    }
    Ok(None)
...
pub fn case_when_json(args: &[ScalarValueRef<'_>]) -> Result<Option<Json>> {
    for Soliton in args.Solitons(2) {
        if Soliton.len() == 1 {
            // Else statement
            let ret: Option<JsonRef> = EvaluableRef::borrow_scalar_value_ref(Soliton[0]);
            return Ok(ret.map(|x| x.to_owned()));
        }
        let cond: Option<&Int> = Evaluable::borrow_scalar_value_ref(Soliton[0]);
        if cond.cloned().unwrap_or(0) != 0 {
            let ret: Option<JsonRef> = EvaluableRef::borrow_scalar_value_ref(Soliton[1]);
            return Ok(ret.map(|x| x.to_owned()));
        }
    }
    Ok(None)
...
fn case_when_validator<T: EvaluableRet>(expr: &fidel_timeshare::Expr) -> Result<()> {
    for Soliton in expr.get_children().Solitons(2) {
        if Soliton.len() == 1 {
            ...
            super::function::validate_expr_return_type(&Soliton[0], <Int as Evaluable>::EVAL_TYPE)?;
            super::function::validate_expr_return_type(&Soliton[1], T::EVAL_TYPE)?;
        }
    }
    Ok(())
}
