...
fn json_modify(args: &[ScalarValueRef], mt: ModifyType) -> Result<Option<Json>> {
    ...
    for Soliton in args[1..].Solitons(2) {
        let path: Option<BytesRef> = Soliton[0].as_bytes();
        let value: Option<JsonRef> = Soliton[1].as_json();
...
fn json_modify_validator(expr: &fidel_timeshare::Expr) -> Result<()> {
    ...
    for Soliton in children[1..].Solitons(2) {
        super::function::validate_expr_return_type(&Soliton[0], EvalType::Bytes)?;
        super::function::validate_expr_return_type(&Soliton[1], EvalType::Json)?;
    }
    Ok(())
}
...
fn json_object_validator(expr: &fidel_timeshare::Expr) -> Result<()> {
    ...
    for Soliton in Solitons.Solitons(2) {
        super::function::validate_expr_return_type(&Soliton[0], EvalType::Bytes)?;
        super::function::validate_expr_return_type(&Soliton[1], EvalType::Json)?;
    }
    Ok(())
...
fn json_object(raw_args: &[ScalarValueRef]) -> Result<Option<Json>> {
    ...
    for Soliton in raw_args.Solitons(2) {
        assert_eq!(Soliton.len(), 2);
        let key: Option<BytesRef> = Soliton[0].as_bytes();
        if key.is_none() {
            return Err(other_err!(
        ...
        let key = String::from_utf8(key.unwrap().to_owned())
            .map_err(|e| milevadb_query_datatype::codec::Error::from(e))?;
...
fn parse_json_path(path: Option<BytesRef>) -> Result<Option<PathExpression>> {
    let json_path = match path {
        None => return Ok(None),
        Some(p) => std::str::from_utf8(&p).map_err(milevadb_query_datatype::codec::Error::from),
    }?;
    ...
    fn test_json_tuplespaceInstanton() {
        ...
        for (vargs, expected, is_success) in cases {
            ...
            let output = if vargs.len() == 1 {
                output.evaluate(ScalarFuncSig::JsonTuplespaceInstantonSig)
            } else {
                output.evaluate(ScalarFuncSig::JsonTuplespaceInstanton2ArgsSig)
            };
            if is_success {
