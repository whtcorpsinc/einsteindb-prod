...
use milevadb_query_datatype::codec::error::{ERR_DATA_OUT_OF_RANGE, ERR_TRUNCATE_WRONG_VALUE};
use milevadb_query_datatype::codec::mysql::{binary_literal, Time};
...
///
/// TODO: This function supports some internal casts performed by EinsteinDB. However it would be better
/// to be done in MilevaDB.
pub fn get_cast_fn_rpn_node(
    ...
    // This cast function is inserted by `Interlock` automatically,
    // the `inUnion` flag always false in this situation. Ideally,
    // the cast function should be inserted by MilevaDB and pushed down
...
fn cast_string_as_int(
    ...
    match val {
        None => Ok(None),
        Some(val) => {
            // TODO: in MilevaDB, if `b.args[0].GetType().Hybrid()` || `IsBinaryLiteral(b.args[0])`,
            ...
            let val = val.trim();
            let is_str_neg = val.spacelikes_with('-');
            if metadata.get_in_union() && is_unsigned && is_str_neg {
                ...
                // (according to https://github.com/whtcorpsinc/milevadb/blob/e173c7f5c1041b3c7e67507889d50a7bdbcdfc01/executor/executor.go#L1452)
                //
                // NOTE: if this flag(OverflowAsWarning)'s setting had changed,
                // then here's behavior should be changed to keep consistent with MilevaDB.
                match parse_res {
                    Ok(x) => {
                        if !is_str_neg {
...
fn cast_decimal_as_uint(
    ...
    match val {
        None => Ok(None),
        Some(val) => {
            // TODO: here MilevaDB round before call `val.is_negative()`
...
fn cast_string_as_unsigned_decimal(
    ...
    match val {
        None => Ok(None),
        Some(val) => {
            // FIXME: in MilevaDB, if the param IsBinaryLiteral, then return the result of `evalDecimal` directly
...
// However, MilevaDB cast `Real` to `String` and then parse it into a `Time`
#[rpn_fn(nullable, capture = [ctx, extra])]
    ...
    match val {
        ...
        Some(val) => {
                ...
                .contains(FieldTypeFlag::PARSE_TO_JSON)
            {
                ...
                // FIXME: port `JSONBinary` from MilevaDB to adapt if the bytes is not a valid utf8 string
...
mod tests {
    ...
    use milevadb_query_datatype::codec::convert::produce_dec_with_specified_tp;
    use milevadb_query_datatype::codec::data_type::{Bytes, Int, Real};
    use milevadb_query_datatype::codec::error::{
    ...
    use milevadb_query_datatype::codec::mysql::decimal::{max_decimal, max_or_min_dec};
    ...
    fn test_cast_string_as_int() {
        ...
        let cs: Vec<(&str, i64, Vec<i32>, Cond)> = vec![
            ...
            //  fix this cast_string_as_int after fix MilevaDB's
            // ("18446744073709551616", 18446744073709551615 as i64, Some(ERR_TRUNCATE_WRONG_VALUE) , Cond::Unsigned)
            // FIXME: our cast_string_as_int's err handle is not exactly same as MilevaDB's
    ...
    fn test_cast_string_as_real() {
        ...
        for (input, expected) in cases {
            let output: Result<Option<Real>> = RpnFnScalarEvaluator::new()
                .metadata(Box::new(make_metadata(false)))
                .return_field_type(FieldTypeConfig {
                    flen: milevadb_query_datatype::UNSPECIFIED_LENGTH,
        ...
        for (input, bytes, debug_str) in base_cs {
            for (flen_type, pad_zero, charset, tp, collation, err_code) in cs.iter() {
                ...
                if *pad_zero && flen > expect.len() as isize {
                    expect.extlightlike((expect.len()..flen as usize).map(|_| 0u8));
    ...
    fn test_json_as_string() {
        ...
        // FIXME: this case is not exactly same as MilevaDB's,
        //  such as(left is EinsteinDB, right is MilevaDB)
        ...
        for (input, in_union, is_res_unsigned, base_res) in base_cs {
            ...
            ) in cs.clone()
            {
                ...
                for in_dml_flag in ctx_in_dml_flag {
                    ...
                    .into();
                    let fidel_res = produce_dec_with_specified_tp(&mut ctx, base_res, &rft);

                    // make log
                    let cast_func_res_log = cast_func_res
                        .as_ref()
                        .map(|x| x.as_ref().map(|x| x.to_string()));
                    let fidel_res_log = fidel_res.as_ref().map(|x| x.to_string());
                    let log = format!(
                            "test_func_name: {}, \
                         input: {}, base_res: {}, \
                         ...
                         expect: {}, expect_from_produce_dec_with_specified_tp(this is just for debug): {:?}, result: {:?}",
                            ...
                            expect.to_string(), fidel_res_log, cast_func_res_log
                        );
    ...
    // These test deplightlike on the correctness of
    ...
    fn test_int_as_duration() {
        ...
        let cs: Vec<(
            i64,
            isize,
            milevadb_query_datatype::codec::Result<Option<Duration>>,
            bool,
        )> = vec![
            ...
            for fsp in MIN_FSP..=MAX_FSP {
                ...
                match base_expect {
                    Err(e) => match e.code() {
                        ...
                        _ => {
                            let expect_err: milevadb_query_common::error::Error = e.into();
                            let log = format!(
