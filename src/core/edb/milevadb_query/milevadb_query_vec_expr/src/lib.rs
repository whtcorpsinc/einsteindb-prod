...
fn map_string_compare_sig<Cmp: CmpOp>(ret_field_type: &FieldType) -> Result<RpnFnMeta> {
    Ok(match_template_collator! {
        TT, match ret_field_type.as_accessor().collation().map_err(milevadb_query_datatype::codec::Error::from)? {
...
{
    // FIXME: The signature for different signed / unsigned int should be inferred at MilevaDB side.
    if children.len() != 2 {
...
fn map_expr_node_to_rpn_func(expr: &Expr) -> Result<RpnFnMeta> {
    ...
    Ok(match value {
        ...
        ScalarFuncSig::JsonTuplespaceInstantonSig => json_tuplespaceInstanton_fn_meta(),
        ScalarFuncSig::JsonTuplespaceInstanton2ArgsSig => json_tuplespaceInstanton_fn_meta(),
        // impl_like
