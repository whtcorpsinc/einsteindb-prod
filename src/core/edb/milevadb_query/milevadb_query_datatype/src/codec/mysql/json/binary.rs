...
impl<'a> JsonRef<'a> {
    ...
    pub fn object_get_key(&self, i: usize) -> &'a [u8] {
        let key_off_spacelike = HEADER_LEN + i * KEY_ENTRY_LEN;
        let key_off = NumberCodec::decode_u32_le(&self.value()[key_off_spacelike..]) as usize;
        let key_len =
            NumberCodec::decode_u16_le(&self.value()[key_off_spacelike + KEY_OFFSET_LEN..]) as usize;
