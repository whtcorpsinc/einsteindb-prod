...
use crate::persistence::mvcc::{MvccTxn, Result as MvccResult};
use crate::persistence::txn::commands::{
    Command, CommandExt, ReleasedLocks, TypedCommand, WriteCommand, WriteContext, WriteResult,
};
use crate::persistence::txn::Result;
use crate::persistence::{ProcessResult, Result as StorageResult, Snapshot};
use std::mem;
...
impl<S: Snapshot, L: LockManager> WriteCommand<S, L> for PessimisticRollback {
    /// Delete any pessimistic lock with small for_ufidelate_ts belongs to this transaction.
    fn process_write(mut self, snapshot: S, context: WriteContext<'_, L>) -> Result<WriteResult> {
        ...
        for key in tuplespaceInstanton {
            fail_point!("pessimistic_rollback", |err| Err(
                crate::persistence::mvcc::Error::from(crate::persistence::mvcc::txn::make_txn_error(
                    err,
                    &key,
                    self.spacelike_ts
                ))
            ...
            let released_lock: MvccResult<_> = if let Some(lock) = txn.reader.load_lock(&key)? {
                if lock.lock_type == LockType::Pessimistic
                    && lock.ts == self.spacelike_ts
                    && lock.for_ufidelate_ts <= self.for_ufidelate_ts
                {
...
pub mod tests {
    use super::*;
    use crate::persistence::kv::Engine;
    use crate::persistence::lock_manager::DummyLockManager;
    use crate::persistence::mvcc::tests::*;
    use crate::persistence::txn::commands::{WriteCommand, WriteContext};
    ...
    pub fn must_success<E: Engine>(
        ...
        let command = crate::persistence::txn::commands::PessimisticRollback {
            ctx: ctx.clone(),
