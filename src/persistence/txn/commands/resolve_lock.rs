...
command! {
    ...
    ResolveLock:
        ...
        content => {
            ...
            /// whose spacelike_ts is 100 was committed with commit_ts `101`, and the transaction whose
            /// spacelike_ts is 102 was rolled back. If there are these tuplespaceInstanton in the db:
            ///
            /// * "k1", lock_ts = 100
            /// * "k2", lock_ts = 102
            /// * "k3", lock_ts = 104
...
impl<S: Snapshot, L: LockManager> WriteCommand<S, L> for ResolveLock {
    fn process_write(mut self, snapshot: S, context: WriteContext<'_, L>) -> Result<WriteResult> {
        ...
        let events = key_locks.len();
        // Map txn's spacelike_ts to ReleasedLocks
        let mut released_locks = HashMap::default();
        for (current_key, current_lock) in key_locks {
            txn.set_spacelike_ts(current_lock.ts);
...
// To resolve a key, the write size is about 100~150 bytes, deplightlikeing on key and value length.
// The write batch will be around 32KB if we scan 256 tuplespaceInstanton each time.
