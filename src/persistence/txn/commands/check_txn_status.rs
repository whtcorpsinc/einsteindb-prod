...
use crate::persistence::mvcc::metrics::MVCC_CHECK_TXN_STATUS_COUNTER_VEC;
use crate::persistence::mvcc::txn::MissingLockAction;
...
command! {
    ...
    /// lock. If the primary lock exists but is not expired, it may ufidelate the transaction's
    ...
    CheckTxnStatus:
        ...
        content => {
            ...
            primary_key: Key,
            /// The lock's ts, namely the transaction's spacelike_ts.
            lock_ts: TimeStamp,
            ...
            Some(mut lock) if lock.ts == self.lock_ts => {
                ...
                if lock.ts.physical() + lock.ttl < self.current_ts.physical() {
                    ...
                    if !lock.min_commit_ts.is_zero()
                        ...
                        && self.caller_spacelike_ts >= lock.min_commit_ts
                    {
                        assert!(!lock.use_async_commit);
                        lock.min_commit_ts = self.caller_spacelike_ts.next();
...
pub mod tests {
    use super::*;
    use crate::persistence::kv::{Engine, WriteData};
    use crate::persistence::lock_manager::DummyLockManager;
    use crate::persistence::mvcc::tests::*;
    use crate::persistence::txn::commands::{pessimistic_rollback, WriteCommand, WriteContext};
    use crate::persistence::txn::tests::*;
    use crate::persistence::{types::TxnStatus, ProcessResult, TestEngineBuilder};
    ...
    pub fn must_success<E: Engine>(
        ...
        let command = crate::persistence::txn::commands::CheckTxnStatus {
            ctx: Context::default(),
    ...
    fn test_check_async_commit_txn_status() {
        ...
        // CheckTxnStatus an async commit lock with non-zero caller_spacelike_ts or current_ts simply
    ...
    fn test_check_txn_status_impl(rollback_if_not_exist: bool) {
        ...
        // caller_spacelike_ts < lock.min_commit_ts < current_ts
        // When caller_spacelike_ts < lock.min_commit_ts, no need to ufidelate it.
        must_success(
            &engine,
            k,
            ts(5, 0),
        ...
        // current_ts < lock.min_commit_ts < caller_spacelike_ts
        must_success(
            &engine,
            k,
            ts(5, 0),
