...
use crate::persistence::lock_manager::{self, LockManager, WaitTimeout};
use crate::persistence::mvcc::{Lock as MvccLock, MvccReader, ReleasedLock};
use crate::persistence::txn::latch::{self, Latches};
use crate::persistence::txn::{ProcessResult, Result};
use crate::persistence::types::{
    MvccInfo, PessimisticLockRes, PrewriteResult, SecondaryLocksStatus, StorageCallbackType,
    TxnStatus,
};
use crate::persistence::{metrics, Result as StorageResult, Snapshot, Statistics};
...
///
/// These are typically scheduled and used through the [`CausetStorage`](crate::persistence::CausetStorage) with functions like
/// [`prewrite`](prewrite::Prewrite) trait and are executed asynchronously.
// Logic related to these can be found in the `src/persistence/txn/proccess.rs::process_write_impl` function.
pub enum Command {
    Prewrite(Prewrite),
...
impl From<PrewriteRequest> for TypedCommand<PrewriteResult> {
    fn from(mut req: PrewriteRequest) -> Self {
        let for_ufidelate_ts = req.get_for_ufidelate_ts();
        let secondary_tuplespaceInstanton = if req.get_use_async_commit() {
            Some(req.get_secondaries().into())
        } else {
            None
        };
        if for_ufidelate_ts == 0 {
            Prewrite::new(
...
impl From<BatchRollbackRequest> for TypedCommand<()> {
    fn from(mut req: BatchRollbackRequest) -> Self {
        let tuplespaceInstanton = req.get_tuplespaceInstanton().iter().map(|x| Key::from_raw(x)).collect();
        Rollback::new(tuplespaceInstanton, req.get_spacelike_version().into(), req.take_context())
    }
}
...
impl From<ScanLockRequest> for TypedCommand<Vec<LockInfo>> {
    fn from(mut req: ScanLockRequest) -> Self {
        let spacelike_key = if req.get_spacelike_key().is_empty() {
            None
        } else {
            Some(Key::from_raw(req.get_spacelike_key()))
        };
...
impl From<ResolveLockRequest> for TypedCommand<()> {
    fn from(mut req: ResolveLockRequest) -> Self {
        let resolve_tuplespaceInstanton: Vec<Key> = req
            .get_tuplespaceInstanton()
            .iter()
            .map(|key| Key::from_raw(key))
            .collect();
        let txn_status = if req.get_spacelike_version() > 0 {
        ...
        if resolve_tuplespaceInstanton.is_empty() {
            ResolveLockReadPhase::new(txn_status, None, req.take_context())
        } else {
            let spacelike_ts: TimeStamp = req.get_spacelike_version().into();
            assert!(!spacelike_ts.is_zero());
            let commit_ts = req.get_commit_version().into();
            ResolveLockLite::new(spacelike_ts, commit_ts, resolve_tuplespaceInstanton, req.take_context())
        }
    }
}
...
impl From<MvccGetByStartTsRequest> for TypedCommand<Option<(Key, MvccInfo)>> {
    fn from(mut req: MvccGetByStartTsRequest) -> Self {
        MvccByStartTs::new(req.get_spacelike_ts().into(), req.take_context())
    }
}
...
impl ReleasedLocks {
    pub fn new(spacelike_ts: TimeStamp, commit_ts: TimeStamp) -> Self {
        Self {
            spacelike_ts,
            commit_ts,
            ..Default::default()
