...
use einsteindb_util::yatp_pool::{FuturePool, PoolTicker, YatpPoolBuilder};

use crate::persistence::kv::{destroy_tls_engine, set_tls_engine, Engine, Statistics};
...
impl SchedPool {
    pub fn new<E: Engine>(engine: E, pool_size: usize, name_prefix: &str) -> Self {
        ...
        let pool = YatpPoolBuilder::new(SchedTicker {})
            ...
            // Safety: by setting `after_spacelike` and `before_stop`, `FuturePool` ensures
...
pub fn tls_flush() {
    TLS_SCHED_METRICS.with(|m| {
        ...
        for (cmd, stat) in m.local_scan_details.drain() {
            for (causet, causet_details) in stat.details().iter() {
                for (tag, count) in causet_details.iter() {
                    KV_COMMAND_SCAN_DETAILS
                        .with_label_values(&[cmd, *causet, *tag])
                        .inc_by(*count as i64);
                }
            }
        }
...
pub fn tls_collect_read_duration(cmd: &str, duration: Duration) {
    TLS_SCHED_METRICS.with(|m| {
        m.borrow_mut()
            ...
            .observe(einsteindb_util::time::duration_to_sec(duration))
    });
}
