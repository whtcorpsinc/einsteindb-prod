...
use crate::persistence::kv::{Snapshot, Statistics};
use crate::persistence::metrics::*;
use crate::persistence::mvcc::{
...
pub trait CausetStore: Slightlike {
    ...
    /// Fetch the provided set of tuplespaceInstanton.
    fn batch_get(
        &self,
...
///
/// Commonly they are obtained as a result of a [`scanner`](CausetStore::scanner) operation.
pub trait Scanner: Slightlike {
    ...
    /// If `sample_step` is greater than 0, skips `sample_step - 1` number of tuplespaceInstanton after each returned key.
...
pub trait TxnEntryStore: Slightlike {
    /// The scanner type returned by `scanner()`.
...
pub trait TxnEntryScanner: Slightlike {
...
/// A transaction entry in underlying persistence.
...
impl EntryBatch {
    ...
    pub fn iter(&self) -> impl Iteron<Item = &TxnEntry> {
        self.entries.iter()
    }
...
impl<S: Snapshot> CausetStore for SnapshotStore<S> {
    ...
    fn incremental_get(&mut self, key: &Key) -> Result<Option<Value>> {
        if self.point_getter_cache.is_none() {
            self.point_getter_cache = Some(
                PointGetterBuilder::new(self.snapshot.clone(), self.spacelike_ts)
                    .fill_cache(self.fill_cache)
                    .isolation_level(self.isolation_level)
                    .multi(true)
                    .bypass_locks(self.bypass_locks.clone())
    ...
    fn batch_get(
        ...
        if tuplespaceInstanton.len() == 1 {
            return Ok(vec![self.get(&tuplespaceInstanton[0], statistics)]);
        }
        ...
        let mut values: Vec<MaybeUninit<Element>> = Vec::with_capacity(tuplespaceInstanton.len());
        for _ in 0..tuplespaceInstanton.len() {
            values.push(MaybeUninit::uninit());
        }
        for (original_order, key) in order_and_tuplespaceInstanton {
    ...
    fn scanner(
        ...
        self.verify_cone(&lower_bound, &upper_bound)?;
        let scanner = ScannerBuilder::new(self.snapshot.clone(), self.spacelike_ts, desc)
            .cone(lower_bound, upper_bound)
            .omit_value(key_only)
            .fill_cache(self.fill_cache)
            .isolation_level(self.isolation_level)
            .bypass_locks(self.bypass_locks.clone())
...
impl<S: Snapshot> TxnEntryStore for SnapshotStore<S> {
    ...
    fn entry_scanner(
        ...
        let (min_ts, max_ts) = if after_ts == TimeStamp::new(0) {
            ...
            // Scan ts in (after_ts, spacelike_ts].
            (Some(after_ts.next()), Some(self.spacelike_ts))
        };
        let scanner =
            ScannerBuilder::new(self.snapshot.clone(), self.spacelike_ts, false /* desc */)
                .cone(lower_bound, upper_bound)
                .omit_value(false)
                .fill_cache(self.fill_cache)
                .isolation_level(self.isolation_level)
...
impl<S: Snapshot> SnapshotStore<S> {
    ...
    fn verify_cone(&self, lower_bound: &Option<Key>, upper_bound: &Option<Key>) -> Result<()> {
        if let Some(ref l) = lower_bound {
            if let Some(b) = self.snapshot.lower_bound() {
                if !b.is_empty() && l.as_encoded().as_slice() < b {
                    ...
                    return Err(Error::from(ErrorInner::InvalidReqCone {
                        spacelike: Some(l.as_encoded().clone()),
                        lightlike: upper_bound.as_ref().map(|ref b| b.as_encoded().clone()),
        ...
        if let Some(ref u) = upper_bound {
            if let Some(b) = self.snapshot.upper_bound() {
                if !b.is_empty() && (u.as_encoded().as_slice() > b || u.as_encoded().is_empty()) {
                    REQUEST_EXCEED_BOUND.inc();
                    return Err(Error::from(ErrorInner::InvalidReqCone {
                        spacelike: lower_bound.as_ref().map(|ref b| b.as_encoded().clone()),
                        lightlike: Some(u.as_encoded().clone()),
...
/// A CausetStore that reads on fixtures.
...
impl CausetStore for FixtureStore {
    ...
    fn batch_get(
        ...
        Ok(tuplespaceInstanton.iter().map(|key| self.get(key, statistics)).collect())
    }
...
mod tests {
    ...
    use crate::persistence::mvcc::{Mutation, MvccTxn};
    use crate::persistence::txn::commit;
    ...
    struct TestStore {
        tuplespaceInstanton: Vec<String>,
        snapshot: Arc<LmdbSnapshot>,
        ctx: Context,
        engine: LmdbEngine,
    }
    ...
    impl TestStore {
        fn new(key_num: u64) -> TestStore {
            ...
            let tuplespaceInstanton: Vec<String> = (START_ID..START_ID + key_num)
        ...
        fn store(&self) -> SnapshotStore<Arc<LmdbSnapshot>> {
            SnapshotStore::new(
                self.snapshot.clone(),
                COMMIT_TS.next(),
                IsolationLevel::Si,
    ...
    // Snapshot with bound
    #[derive(Clone)]
    struct MockConeSnapshot {
        spacelike: Vec<u8>,
        lightlike: Vec<u8>,
    }

    #[derive(Default)]
    struct MockConeSnapshotIter {}

    impl Iteron for MockConeSnapshotIter {
    ...
    impl MockConeSnapshot {
        fn new(spacelike: Vec<u8>, lightlike: Vec<u8>) -> Self {
            Self { spacelike, lightlike }
        }
    }

    impl Snapshot for MockConeSnapshot {
        type Iter = MockConeSnapshotIter;

        fn get(&self, _: &Key) -> EngineResult<Option<Value>> {
            Ok(None)
        }
        fn get_causet(&self, _: CfName, _: &Key) -> EngineResult<Option<Value>> {
            Ok(None)
        }
        fn get_causet_opt(&self, _: ReadOptions, _: CfName, _: &Key) -> EngineResult<Option<Value>> {
            Ok(None)
        }
        fn iter(&self, _: IterOptions, _: ScanMode) -> EngineResult<Cursor<Self::Iter>> {
            Ok(Cursor::new(
                MockConeSnapshotIter::default(),
                ScanMode::Forward,
            ))
        }
        fn iter_causet(
        ...
        fn lower_bound(&self) -> Option<&[u8]> {
            Some(self.spacelike.as_slice())
        }
        fn upper_bound(&self) -> Option<&[u8]> {
            Some(self.lightlike.as_slice())
        }
    }
    ...
    fn test_snapshot_store_get() {
        ...
        for key in &store.tuplespaceInstanton {
            let key = key.as_bytes();
    ...
    fn test_snapshot_store_batch_get() {
        ...
        let snapshot_store = store.store();
        let mut statistics = Statistics::default();
        let mut tuplespaceInstanton_list = Vec::new();
        for key in &store.tuplespaceInstanton {
            tuplespaceInstanton_list.push(Key::from_raw(key.as_bytes()));
        }
        let data = snapshot_store
            .batch_get(&tuplespaceInstanton_list, &mut statistics)
            .unwrap();
        for item in data {
    ...
    fn test_snapshot_store_scan() {
        ...
        let spacelike_key = Key::from_raw(key.as_bytes());
        let mut scanner = snapshot_store
            .scanner(false, false, false, Some(spacelike_key), None)
            .unwrap();

        let half = (key_num / 2) as usize;
        let expect = &store.tuplespaceInstanton[0..half];
    ...
    fn test_snapshot_store_reverse_scan() {
        ...
        let spacelike_key = Key::from_raw(key.as_bytes());
        let expect = &store.tuplespaceInstanton[0..half - 1];
        let mut scanner = snapshot_store
            .scanner(true, false, false, None, Some(spacelike_key))
            .unwrap();
    ...
    fn test_scan_with_bound() {
        ...
        // Collect all scanned tuplespaceInstanton
    ...
    fn test_scanner_verify_bound() {
        // CausetStore with a limited cone
        let snap = MockConeSnapshot::new(b"b".to_vec(), b"c".to_vec());
        ...
        // CausetStore with whole cone
        let snap2 = MockConeSnapshot::new(b"".to_vec(), b"".to_vec());
    ...
    fn gen_fixture_store() -> FixtureStore {
        ...
        data.insert(
            ...
            Err(Error::from(ErrorInner::Mvcc(MvccError::from(
                MvccErrorInner::KeyIsLocked(ekvproto::kvrpcpb::LockInfo::default()),
            )))),
        );
...
mod benches {
    ...
    fn bench_fixture_batch_get(b: &mut test::Bencher) {
        let mut batch_get_tuplespaceInstanton = vec![];
        let mut data = BTreeMap::default();
        for _ in 0..100 {
            let user_key = gen_payload(64);
            let key = Key::from_raw(&user_key);
            batch_get_tuplespaceInstanton.push(key.clone());
        ...
        b.iter(|| {
            let store = test::black_box(&store);
            let mut statistics = Statistics::default();
            let value = store.batch_get(test::black_box(&batch_get_tuplespaceInstanton), &mut statistics);
