...
quick_error! {
    ...
    pub enum ErrorInner {
        ...
        Mvcc(err: crate::persistence::mvcc::Error) {
            from()
            cause(err)
            display("{}", err)
        }
...
impl ErrorCodeExt for Error {
    fn error_code(&self) -> ErrorCode {
        match self.0.as_ref() {
            ...
            ErrorInner::ProtoBuf(_) => error_code::persistence::PROTOBUF,
            ErrorInner::Mvcc(e) => e.error_code(),
            ErrorInner::Other(_) => error_code::persistence::UNKNOWN,
            ErrorInner::Io(_) => error_code::persistence::IO,
            ErrorInner::InvalidTxnTso { .. } => error_code::persistence::INVALID_TXN_TSO,
            ErrorInner::InvalidReqCone { .. } => error_code::persistence::INVALID_REQ_RANGE,
            ErrorInner::MaxTimestampNotSynced { .. } => {
                error_code::persistence::MAX_TIMESTAMP_NOT_SYNCED
            }
        }
    }
}
