...
//! Scheduler which schedules the execution of `persistence::Command`s.
//!
//! There is one scheduler for each store. It receives commands from clients, executes them against
//! the MVCC layer persistence engine.
//!
...
use crate::persistence::txn::commands::{WriteContext, WriteResult};
use crate::persistence::txn::{
    commands::Command,
    latch::{Latches, Lock},
...
impl<E: Engine, L: LockManager> Scheduler<E, L> {
    /// Creates a scheduler.
    pub(in crate::persistence) fn new(
        engine: E,
        lock_mgr: L,
    ...
    pub(in crate::persistence) fn run_cmd(&self, cmd: Command, callback: StorageCallback) {
        // write flow control
    ...
    /// Initiates an async operation to get a snapshot from the persistence engine, then posts a
    ...
    fn process_write(self, engine: &E, snapshot: E::Snap, task: Task, statistics: &mut Statistics) {
        ...
        match task.cmd.process_write(snapshot, context) {
            // Initiates an async write operation on the persistence engine, there'll be a `WriteFinished`
            ...
            Ok(WriteResult {
                ...
                if let Some((lock, is_first_lock, wait_timeout)) = lock_info {
                    ...
                    // The callback to receive async results of write prepare from the persistence engine.
...
mod tests {
    use super::*;
    use crate::persistence::mvcc::{self, Mutation};
    use crate::persistence::txn::{commands, latch::*};
