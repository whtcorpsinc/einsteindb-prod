...
quick_error! {
    #[derive(Debug)]
    pub enum ErrorInner {
        Engine(err: crate::persistence::kv::Error) {
            from()
            cause(err)
            display("{}", err)
        }
        ...
        PessimisticLockRolledBack { spacelike_ts: TimeStamp, key: Vec<u8> } {
            display("pessimistic lock already rollbacked, spacelike_ts:{}, key:{}", spacelike_ts, hex::encode_upper(key))
        }
        TxnLockNotFound { spacelike_ts: TimeStamp, commit_ts: TimeStamp, key: Vec<u8> } {
            display("txn lock not found {}-{} key:{}", spacelike_ts, commit_ts, hex::encode_upper(key))
        }
        ...
        LockTypeNotMatch { spacelike_ts: TimeStamp, key: Vec<u8>, pessimistic: bool } {
            display("lock type not match, spacelike_ts:{}, key:{}, pessimistic:{}", spacelike_ts, hex::encode_upper(key), pessimistic)
        }
        ...
        PessimisticLockNotFound { spacelike_ts: TimeStamp, key: Vec<u8> } {
            display("pessimistic lock not found, spacelike_ts:{}, key:{}", spacelike_ts, hex::encode_upper(key))
        }
...
impl ErrorCodeExt for Error {
    fn error_code(&self) -> ErrorCode {
        match self.0.as_ref() {
            ...
            ErrorInner::Committed { .. } => error_code::persistence::COMMITTED,
            ErrorInner::PessimisticLockRolledBack { .. } => {
                error_code::persistence::PESSIMISTIC_LOCK_ROLLED_BACK
            }
            ErrorInner::TxnLockNotFound { .. } => error_code::persistence::TXN_LOCK_NOT_FOUND,
            ErrorInner::TxnNotFound { .. } => error_code::persistence::TXN_NOT_FOUND,
            ErrorInner::LockTypeNotMatch { .. } => error_code::persistence::LOCK_TYPE_NOT_MATCH,
            ErrorInner::WriteConflict { .. } => error_code::persistence::WRITE_CONFLICT,
            ErrorInner::Deadlock { .. } => error_code::persistence::DEADLOCK,
            ErrorInner::AlreadyExist { .. } => error_code::persistence::ALREADY_EXIST,
            ErrorInner::DefaultNotFound { .. } => error_code::persistence::DEFAULT_NOT_FOUND,
            ErrorInner::CommitTsExpired { .. } => error_code::persistence::COMMIT_TS_EXPIRED,
            ErrorInner::KeyVersion => error_code::persistence::KEY_VERSION,
            ErrorInner::PessimisticLockNotFound { .. } => {
                error_code::persistence::PESSIMISTIC_LOCK_NOT_FOUND
            }
...
pub mod tests {
    use super::*;
    use crate::persistence::kv::{Engine, Modify, ScanMode, Snapshot, WriteData};
    ...
    pub fn must_locked<E: Engine>(engine: &E, key: &[u8], spacelike_ts: impl Into<TimeStamp>) -> Lock {
        let snapshot = engine.snapshot(&Context::default()).unwrap();
        let mut reader = MvccReader::new(snapshot, None, true, IsolationLevel::Si);
        let lock = reader.load_lock(&Key::from_raw(key)).unwrap().unwrap();
        assert_eq!(lock.ts, spacelike_ts.into());
    ...
    pub fn must_pessimistic_locked<E: Engine>(
        ...
        let lock = reader.load_lock(&Key::from_raw(key)).unwrap().unwrap();
        assert_eq!(lock.ts, spacelike_ts.into());
        assert_eq!(lock.for_ufidelate_ts, for_ufidelate_ts.into());
        assert_eq!(lock.lock_type, LockType::Pessimistic);
    }
