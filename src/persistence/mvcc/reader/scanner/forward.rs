...
use crate::persistence::mvcc::{NewerTsCheckState, Result};
use crate::persistence::txn::{Result as TxnResult, TxnEntry, TxnEntryScanner};
use crate::persistence::{Cursor, Snapshot, Statistics};
...
impl<S: Snapshot, P: ScanPolicy<S>> ForwardScanner<S, P> {
    ...
    pub fn read_next(&mut self) -> Result<Option<P::Output>> {
        ...
        // TODO: We don't need to seek lock CAUSET if isolation level is RC.
...
fn scan_latest_handle_lock<S: Snapshot, T>(
    ...
    // calls.
    if result.is_err() {
        statistics.lock.processed_tuplespaceInstanton += 1;
        cursors.move_write_cursor_to_next_user_key(&current_user_key, statistics)?;
    }
    result
...
impl<S: Snapshot> ScanPolicy<S> for DeltaEntryPolicy {
    ...
    fn handle_lock(
        ...
        let result = if lock.ts > causetg.ts {
                ...
                && (lock.lock_type == LockType::Put || lock.lock_type == LockType::Delete)
            {
                ...
                super::seek_for_valid_value(
                    ...
                    cursors.default.as_mut().unwrap(),
                    &current_user_key,
                    std::cmp::max(lock.ts, lock.for_ufidelate_ts),
                    statistics,
                )?
            } else {
                None
...
pub mod test_util {
    use super::*;
    use crate::persistence::mvcc::Write;
...
mod latest_entry_tests {
    ...
    use crate::persistence::{Engine, TestEngineBuilder};
    use ekvproto::kvrpcpb::Context;
