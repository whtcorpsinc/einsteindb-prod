...
use engine_promises::{CfName, IterOptions, CAUSET_DEFAULT, CAUSET_DAGGER, CAUSET_WRITE};
...
use crate::persistence::mvcc::{default_not_found_error, NewerTsCheckState, Result};
use crate::persistence::txn::{Result as TxnResult, Scanner as StoreScanner};
...
impl<S: Snapshot> ScannerBuilder<S> {
    ...
    /// Limit the cone to `[lower_bound, upper_bound)` in which the `ForwardKvScanner` should scan.
    /// `None` means unbounded.
    ///
    /// Default is `(None, None)`.
    #[inline]
    pub fn cone(mut self, lower_bound: Option<Key>, upper_bound: Option<Key>) -> Self {
        self.0.lower_bound = lower_bound;
        self.0.upper_bound = upper_bound;
        self
    }

    /// Set locks that the scanner can bypass. Locks with spacelike_ts in the specified set will be
    ...
    pub fn build(mut self) -> Result<Scanner<S>> {
        let lock_cursor = self.0.create_causet_cursor(CAUSET_DAGGER)?;
        let write_cursor = self.0.create_causet_cursor(CAUSET_WRITE)?;
        if self.0.desc {
    ...
    pub fn build_entry_scanner(
        ...
        let lock_cursor = self.0.create_causet_cursor(CAUSET_DAGGER)?;
        let write_cursor = self.0.create_causet_cursor(CAUSET_WRITE)?;
        // Note: Create a default causet cursor will take key cone, so we need to
        //       ensure the default cursor is created after lock and write.
        let default_cursor = self.0.create_causet_cursor(CAUSET_DEFAULT)?;
    ...
    pub fn build_delta_scanner(
        ...
        let default_cursor = self
            .0
            .create_causet_cursor_with_scan_mode(CAUSET_DEFAULT, ScanMode::Mixed)?;
        Ok(ForwardScanner::new(
            self.0,
            lock_cursor,
            write_cursor,
...
impl<S: Snapshot> ScannerConfig<S> {
    ...
    /// Create the cursor.
    #[inline]
    fn create_causet_cursor(&mut self, causet: CfName) -> Result<Cursor<S::Iter>> {
        self.create_causet_cursor_with_scan_mode(causet, self.scan_mode())
    }

    /// Create the cursor with specified scan_mode, instead of inferring scan_mode from the config.
    #[inline]
    fn create_causet_cursor_with_scan_mode(
        &mut self,
        causet: CfName,
        scan_mode: ScanMode,
    ) -> Result<Cursor<S::Iter>> {
        let (lower, upper) = if causet == CAUSET_DEFAULT {
            (self.lower_bound.take(), self.upper_bound.take())
        } else {
            (self.lower_bound.clone(), self.upper_bound.clone())
        };
        // FIXME: Try to find out how to filter default CAUSET SSTs by spacelike ts
        let (hint_min_ts, hint_max_ts) = if causet == CAUSET_WRITE {
            (self.hint_min_ts, self.hint_max_ts)
        } else {
            (None, None)
        };
        let cursor = CursorBuilder::new(&self.snapshot, causet)
            .cone(lower, upper)
...
/// Reads user key's value in default CAUSET according to the given write CAUSET value
/// (`write`).
///
...
///
/// Panics if key in default CAUSET does not exist. This means there is a data corruption.
fn near_load_data_by_write<I>(
    default_cursor: &mut Cursor<I>, // TODO: make it `ForwardCursor`.
    user_key: &Key,
    write_spacelike_ts: TimeStamp,
    statistics: &mut Statistics,
) -> Result<Value>
where
    I: Iteron,
{
    let seek_key = user_key.clone().applightlike_ts(write_spacelike_ts);
    ...
    statistics.data.processed_tuplespaceInstanton += 1;
...
pub fn has_data_in_cone<S: Snapshot>(
    snapshot: S,
    causet: CfName,
    left: &Key,
    right: &Key,
    ...
    let mut iter = snapshot.iter_causet(causet, iter_opt, ScanMode::Forward)?;
    ...
    if let Some(write) = seek_for_valid_write(write_cursor, user_key, after_ts, statistics)? {
        if write.write_type == WriteType::Put {
            let value = if let Some(v) = write.short_value {
                ...
                near_load_data_by_write(default_cursor, user_key, write.spacelike_ts, statistics)?
            };
...
mod tests {
    use super::*;
    use crate::persistence::kv::SEEK_BOUND;
    use crate::persistence::kv::{Engine, LmdbEngine, TestEngineBuilder};
    use crate::persistence::mvcc::tests::*;
    use crate::persistence::mvcc::{Error as MvccError, ErrorInner as MvccErrorInner};
    use crate::persistence::txn::tests::*;
    ...
    fn test_scan_with_lock_and_write_impl(desc: bool) {
        ...
        let test_scanner_result =
            move |engine: &LmdbEngine, expected_result: Vec<(Vec<u8>, Option<Vec<u8>>)>| {
        ...
        // Lock before write in different tuplespaceInstanton
        let engine = new_engine();
        add_lock_at_ts(POST_TS, &engine, b"a");
    ...
    fn must_met_newer_ts_data<E: Engine>(
        ...
        .cone(Some(Key::from_raw(key)), None)
