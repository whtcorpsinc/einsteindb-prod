...
use crate::persistence::kv::{Cursor, CursorBuilder, ScanMode, Snapshot, Statistics};
...
impl<S: Snapshot> PointGetterBuilder<S> {
    ...
    /// Set whether or not to get multiple tuplespaceInstanton.
    ///
    /// Defaults to `true`.
    #[inline]
    ...
    pub fn build(self) -> Result<PointGetter<S>> {
        ...
        let write_cursor = CursorBuilder::new(&self.snapshot, CAUSET_WRITE)
...
/// This struct can be used to get the value of user tuplespaceInstanton. Internally, rollbacks are ignored and
...
impl<S: Snapshot> PointGetter<S> {
    ...
    /// Get a lock of a user key in the lock CAUSET. If lock exists, it will be checked to
    /// see whether it conflicts with the given `ts`.
    ///
    /// In common cases we expect to get nothing in lock causet. Using a `get_causet` instead of `seek`
    /// is fast in such cases due to no need for Lmdb to continue move and skip deleted entries
    /// until find a user key.
    fn load_and_check_lock(&mut self, user_key: &Key) -> Result<()> {
        self.statistics.lock.get += 1;
        let lock_value = self.snapshot.get_causet(CAUSET_DAGGER, user_key)?;
        ...
        if let Some(ref lock_value) = lock_value {
            ...
            {
                self.statistics.lock.processed_tuplespaceInstanton += 1;
                Err(e.into())
            } else {
                Ok(())
            }
    ...
    /// Load the value.
    ///
    /// First, a correct version info in the Write CAUSET will be sought. Then, value will be loaded
    /// from Default CAUSET if necessary.
    fn load_data(&mut self, user_key: &Key) -> Result<Option<Value>> {
        let mut use_near_seek = false;
        let mut seek_key = user_key.clone();

        if self.met_newer_ts_data == NewerTsCheckState::NotMetYet {
            seek_key = seek_key.applightlike_ts(TimeStamp::max());
            if !self
                .write_cursor
        ...
        seek_key = seek_key.applightlike_ts(self.ts);
                ...
                WriteType::Put => {
                    ...
                    match write.short_value {
                        ...
                        None => {
                            ...
                            return Ok(Some(self.load_data_from_default_causet(spacelike_ts, user_key)?));
                        }
                    }
                }
                WriteType::Delete => {
    ...
    /// Load the value from default CAUSET.
    ///
    /// We assume that mostly the tuplespaceInstanton given to batch get tuplespaceInstanton are not very close to each other.
    /// `near_seek` will likely fall back to `seek` in such scenario, which takes 2x time
    /// compared to `get_causet`. Thus we use `get_causet` directly here.
    fn load_data_from_default_causet(
        &mut self,
        ...
        let value = self
            .snapshot
            .get_causet(CAUSET_DEFAULT, &user_key.clone().applightlike_ts(write_spacelike_ts))?;
...
mod tests {
    ...
    use crate::persistence::kv::{CfStatistics, Engine, LmdbEngine, TestEngineBuilder};
    ...
    fn must_get_value<S: Snapshot>(point_getter: &mut PointGetter<S>, key: &[u8], prefix: &[u8]) {
        ...
        assert!(val.spacelikes_with(prefix));
    }
    ...
    fn new_sample_engine() -> LmdbEngine {
    ...
    /// PUT     foo1    -> foo1vv...    (commit at 3)
    /// PUT     foo2    -> foo2vv...    (spacelike at 4)
    fn new_sample_engine_2() -> LmdbEngine {
        let suffix = "v".repeat(SHORT_VALUE_MAX_LEN + 1);
        let engine = TestEngineBuilder::new().build().unwrap();
        must_prewrite_put(
            &engine,
    ...
    /// There are some locks in the Lock CAUSET.
    #[test]
    ...
    fn test_get_latest_value() {
        ...
        //
        // write.spacelike_ts(10) < primary_lock.spacelike_ts(15) < write.commit_ts(20)
