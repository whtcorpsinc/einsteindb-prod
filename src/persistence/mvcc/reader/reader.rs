...
use crate::persistence::kv::{Cursor, ScanMode, Snapshot, Statistics};
use crate::persistence::mvcc::{default_not_found_error, Result};
...
impl<S: Snapshot> MvccReader<S> {
    ...
    /// Scan locks that satisfies `filter(lock)` returns true, from the given spacelike key `spacelike`.
        ...
        F: Fn(&Lock) -> bool,
    {
        ...
        self.statistics.lock.processed_tuplespaceInstanton += locks.len();
...
mod tests {
    ...
    use crate::persistence::kv::Modify;
    use crate::persistence::mvcc::{MvccReader, MvccTxn};
