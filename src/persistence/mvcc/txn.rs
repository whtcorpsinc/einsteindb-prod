...
use crate::persistence::kv::{Modify, ScanMode, Snapshot, Statistics, WriteData};
use crate::persistence::mvcc::{
    metrics::*, reader::MvccReader, reader::TxnCommitRecord, ErrorInner, Result,
};
use crate::persistence::types::TxnStatus;
...
impl<S: Snapshot> MvccTxn<S> {
    ...
    pub(crate) fn put_lock(&mut self, key: Key, lock: &Lock) {
        let write = Modify::Put(CAUSET_DAGGER, key, lock.to_bytes());
    ...
    fn prewrite_key_value(
        ...
        if let Some(secondary_tuplespaceInstanton) = secondary_tuplespaceInstanton {
            ...
            lock.secondaries = secondary_tuplespaceInstanton.to_owned();
    ...
    pub(crate) fn mark_rollback_on_mismatching_lock(
        ...
        assert_ne!(lock.ts, self.spacelike_ts);
        ...
        if self.spacelike_ts < lock.min_commit_ts {
        ...
        lock.rollback_ts.push(self.spacelike_ts);
    ...
    fn handle_non_pessimistic_lock_conflict(&self, key: Key, lock: Lock) -> Result<()> {
        ...
        // Set ttl to 0 so MilevaDB will resolve lock immediately.
        info.set_lock_ttl(0);
    ...
    pub fn acquire_pessimistic_lock(
        ...
        if let Some(lock) = self.reader.load_lock(&key)? {
            ...
            // Overwrite the lock with small for_ufidelate_ts
            if for_ufidelate_ts > lock.for_ufidelate_ts {
    ...
    fn amlightlike_pessimistic_lock(
        ...
        if let Some((commit_ts, _)) = self.reader.seek_write(key, TimeStamp::max())? {
            ...
            //   1. lock's for_ufidelate_ts >= key's latest commit_ts
            //   2. lock's for_ufidelate_ts >= txn's spacelike_ts
            //   3. If the data is changed after acquiring the pessimistic lock, key's new commit_ts > lock's for_ufidelate_ts
            //
            // So, if the key's latest commit_ts is still less than or equal to lock's for_ufidelate_ts, the data is not changed.
            // However, we can't get lock's for_ufidelate_ts in current implementation (txn's for_ufidelate_ts is ufidelated for each DML),
            // we can only use txn's spacelike_ts to check -- If the key's commit_ts is less than txn's spacelike_ts, it's less than
            // lock's for_ufidelate_ts too.
            if commit_ts >= self.spacelike_ts {
                warn!(
                    "prewrite failed (pessimistic lock not found)";
                    "spacelike_ts" => self.spacelike_ts,
        ...
        // Luckily no other txn modified this lock, amlightlike it by treat it as optimistic txn.
        MVCC_CONFLICT_COUNTER
    ...
    pub fn cleanup(
        ...
        match self.reader.load_lock(&key)? {
            Some(ref lock) if lock.ts == self.spacelike_ts => {
                ...
                let is_pessimistic_txn = !lock.for_ufidelate_ts.is_zero();
    ...
    fn check_extra_op(
        ...
        use crate::persistence::mvcc::reader::seek_for_valid_write;
...
mod tests {
    ...
    fn test_mvcc_txn_read_imp(k1: &[u8], k2: &[u8], v: &[u8]) {
        ...
        // even if lock.spacelike_ts(23) < latest write.commit_ts(27)
        must_get(&engine, k1, u64::max_value(), v);
    ...
    fn test_pessimistic_lock() {
        ...
        // Acquire lock when there is lock with different for_ufidelate_ts.
    ...
    fn test_constraint_check_with_overlapping_txn() {
        ...
        // spacelike_ts = 5,  commit_ts = 15, Lock
    ...
    fn test_non_pessimistic_lock_conflict_with_pessismitic_txn() {
        ...
        // Write a non-pessimistic lock with for_ufidelate_ts 20.
    ...
    fn test_amlightlike_pessimistic_lock() {
        fn fail_to_write_pessimistic_lock<E: Engine>(
            ...
            // Delete the pessimistic lock to pretlightlike write failure.
