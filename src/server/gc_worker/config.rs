...
use einsteindb_util::config::{ReadableSize, VersionTrack};
...
pub struct GcConfig {
    pub ratio_memory_barrier: f64,
    pub batch_tuplespaceInstanton: usize,
...
impl Default for GcConfig {
    fn default() -> GcConfig {
        GcConfig {
            ratio_memory_barrier: DEFAULT_GC_RATIO_THRESHOLD,
            batch_tuplespaceInstanton: DEFAULT_GC_BATCH_KEYS,
...
impl GcConfig {
    pub fn validate(&self) -> std::result::Result<(), Box<dyn std::error::Error>> {
        if self.batch_tuplespaceInstanton == 0 {
            return Err("gc.batch_tuplespaceInstanton should not be 0".into());
        }
        Ok(())
    }
}
...
impl ConfigManager for GcWorkerConfigManager {
    fn dispatch(
    ...
    ) -> std::result::Result<(), Box<dyn std::error::Error>> {
        {
            ...
            self.0.ufidelate(move |causetg: &mut GcConfig| causetg.ufidelate(change));
        }
        info!(
