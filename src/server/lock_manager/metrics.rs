...
make_static_metric! {
    pub struct WaitBlockStatusGauge: IntGauge {
        "type" => {
            locks,
            txns,
        },
...
lazy_static! {
    ...
    pub static ref DETECT_DURATION_HISTOGRAM: Histogram = register_histogram!(
        "einsteindb_lock_manager_detect_duration",
        "Duration of handling detect requests",
        exponential_buckets(0.0001, 2.0, 20).unwrap() // 0.1ms ~ 104s
    )
    .unwrap();
    pub static ref WAIT_Block_STATUS_GAUGE: WaitBlockStatusGauge = register_static_int_gauge_vec!(
        WaitBlockStatusGauge,
        "einsteindb_lock_manager_wait_Block_status",
