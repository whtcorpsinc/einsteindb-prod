...
impl Display for Task {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Task::WaitFor { spacelike_ts, lock, .. } => {
                write!(f, "txn:{} waiting for {}:{}", spacelike_ts, lock.ts, lock.hash)
            }
...
impl WaiterManager {
    ...
    fn handle_wait_for(&mut self, waiter: Waiter) {
        let (waiter_ts, lock) = (waiter.spacelike_ts, waiter.lock);
        ...
        let f = waiter.on_timeout(move || {
            if let Some(waiter) = wait_table.borrow_mut().remove_waiter(lock, waiter_ts) {
                detector_scheduler.clean_up_wait_for(waiter.spacelike_ts, waiter.lock);
                waiter.notify();
            }
        });
        ...
        for hash in hashes {
            ...
            if let Some((mut oldest, others)) = wait_table.remove_oldest_waiter(lock) {
                ...
                self.detector_scheduler
                    .clean_up_wait_for(oldest.spacelike_ts, oldest.lock);
    ...
    fn run(&mut self, task: Task) {
        match task {
            Task::WaitFor {
                ...
                let waiter = Waiter::new(spacelike_ts, cb, pr, lock, self.normalize_deadline(timeout));
            ...
            Task::Deadlock {
                ...
                self.handle_deadlock(spacelike_ts, lock, deadlock_key_hash);
            }
