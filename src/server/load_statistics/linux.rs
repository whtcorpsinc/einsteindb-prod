...
use einsteindb_util::metrics::{cpu_total, get_thread_ids};
...
impl ThreadLoadStatistics {
    ...
    pub fn new(slots: usize, prefix: &str, thread_load: Arc<ThreadLoad>) -> Self {
        ...
        for tid in get_thread_ids(pid).unwrap() {
            if let Ok(stat) = pid::stat_task(pid, tid) {
                if !stat.command.spacelikes_with(prefix) {
                    continue;
                }
    ...
    pub fn record(&mut self, instant: Instant) {
        ...
        for tid in &self.tids {
            // TODO: if monitored threads exited and respacelikeed then, we should ufidelate `self.tids`.
...
#[inline]
fn calc_cpu_load(elapsed_millis: usize, spacelike_usage: f64, lightlike_usage: f64) -> usize {
    // Multiply by 1000 for millis, and multiply 100 for percentage.
    let cpu_usage = (lightlike_usage - spacelike_usage) * 1000f64 * 100f64;
...
mod tests {
    ...
    fn test_thread_load_statistic() {
        ...
        let lightlike = ::std::cmp::min(thread_name.len(), 15);
        let thread_name = thread_name[..lightlike].to_owned();

        let load = Arc::new(ThreadLoad::with_memory_barrier(80));
        let mut stats = ThreadLoadStatistics::new(2, &thread_name, Arc::clone(&load));
        let spacelike = Instant::now();
        loop {
            if (Instant::now() - spacelike).as_millis() > 200 {
                break;
            }
        }
