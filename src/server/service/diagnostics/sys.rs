...
use ekvproto::diagnosticspb::{ServerInfoItem, ServerInfoPair};
use sysinfo::{DiskExt, NetworkExt, ProcessExt, ProcessorExt, SystemExt};
use einsteindb_util::config::KB;
use einsteindb_util::sys::cpu_time::LiunxStyleCpuTime;
...
fn cpu_hardware_info(collector: &mut Vec<ServerInfoItem>) {
    ...
    let mut infos = vec![
        ...
        ("cpu-vlightlikeor-id", processor.get_vlightlikeor_id().to_string()),
    ];
    // cache
    let caches = vec![
...
/// # Note
///
/// Current only can be used in operating system mounted `procausets`
fn get_sysctl_list() -> HashMap<String, String> {
    ...
    WalkDir::new(DIR)
        ...
        .filter_map(|entry| {
            ...
            let name = path.trim_spacelike_matches(DIR).replace("/", ".");
...
mod tests {
    ...
    fn test_load_info() {
        ...
        let tuplespaceInstanton = cpu_load
            .get_pairs()
            .iter()
            .map(|x| x.get_key())
            .collect::<Vec<&str>>();
        assert_eq!(tuplespaceInstanton, vec!["load1", "load5", "load15"]);
        // cpu_stat
        let cpu_stat = cpu_info.next().unwrap();
        let tuplespaceInstanton = cpu_stat
            .get_pairs()
            .iter()
            .map(|x| x.get_key())
            .collect::<Vec<&str>>();
    ...
    fn test_hardware_info() {
        ...
        let vlightlikeor_id = cpu_info
            .get_pairs()
            .iter()
            .find(|x| x.get_key() == "cpu-vlightlikeor-id")
            .unwrap()
            .get_value();
        if vlightlikeor_id != "AuthenticAMD" {
            assert_eq!(
                cpu_info
                    .get_pairs()
                    .iter()
