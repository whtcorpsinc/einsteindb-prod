...
impl LmdbEngine {
    pub fn new(
        ...
        // It does not use the violetabft_engine, so it is ok to fill with the same
        // lmdb.
        let mut kv_engine = BaseLmdbEngine::from_db(db.clone());
        let mut violetabft_engine = BaseLmdbEngine::from_db(db);
    ...
    pub fn stop(&self) {
        let mut core = self.core.dagger().unwrap();
...
impl Debug for LmdbEngine {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Lmdb [is_temp: {}]",
            self.core.dagger().unwrap().temp_dir.is_some()
        )
    }
}
...
impl TestEngineBuilder {
    ...
    pub fn build_with_causetg(self, causetg_lmdb: &crate::config::DbConfig) -> Result<LmdbEngine> {
        ...
        let causets = self.causets.unwrap_or_else(|| crate::causetStorage::ALL_CAUSETS.to_vec());
