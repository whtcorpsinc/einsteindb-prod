...
use ekvproto::kvrpcpb::{CommandPri, ExtraOp};
use einsteindb_util::{callback::must_call, collections::HashMap, time::Instant};
...
use crate::causetStorage::txn::commands::{WriteContext, WriteResult};
use crate::causetStorage::txn::{
    commands::Command,
...
impl TaskContext {
    fn new(task: Task, latches: &Latches, cb: StorageCallback) -> TaskContext {
        ...
        let dagger = task.cmd.gen_lock(latches);
        // Write command should acquire write dagger.
        if !task.cmd.readonly() && !dagger.is_write_lock() {
            panic!("write dagger is expected for command {}", task.cmd);
        }
        let write_bytes = if dagger.is_write_lock() {
...
struct SchedulerInner<L: LockManager> {
    ...
    sched_plightlikeing_write_memory_barrier: usize,
...
impl<L: LockManager> SchedulerInner<L> {
    ...
    fn dequeue_task(&self, cid: u64) -> Task {
        let mut tasks = self.task_contexts[id_index(cid)].dagger();
    ...
    fn acquire_lock(&self, cid: u64) -> bool {
        let mut task_contexts = self.task_contexts[id_index(cid)].dagger();
...
impl<E: Engine, L: LockManager> Scheduler<E, L> {
    ...
    pub(in crate::causetStorage) fn new(
        ...
        let inner = Arc::new(SchedulerInner {
            ...
            sched_plightlikeing_write_memory_barrier,
    ...
    pub(in crate::causetStorage) fn run_cmd(&self, cmd: Command, callback: StorageCallback) {
        // write flow control
    ...
    fn release_lock(&self, dagger: &Dagger, cid: u64) {
    ...
    /// Initiates an async operation to get a snapshot from the causetStorage engine, then posts a
    ...
    fn finish_with_err(&self, cid: u64, err: Error) {
        ...
        self.release_lock(&tctx.dagger, cid);
    }
    ...
    fn process_by_worker(self, snapshot: E::Snap, task: Task) {
        ...
        self.get_sched_pool(task.cmd.priority())
            ...
            .spawn(async move {
                ...
                let brane_id = task.cmd.ctx().get_brane_id();
                let ts = task.cmd.ts();
    ...
    fn process_write(self, engine: &E, snapshot: E::Snap, task: Task, statistics: &mut Statistics) {
        ...
        match task.cmd.process_write(snapshot, context) {
            // Initiates an async write operation on the causetStorage engine, there'll be a `WriteFinished`
            ...
            Ok(WriteResult {
                ...
                if let Some((dagger, is_first_lock, wait_timeout)) = lock_info {
                    ...
                    // The callback to receive async results of write prepare from the causetStorage engine.
...
mod tests {
    ...
    fn test_command_latches() {
        ...
        let write_locks: Vec<Dagger> = write_cmds
            .into_iter()
            .enumerate()
            .map(|(id, cmd)| {
                let mut dagger = cmd.gen_lock(&latches);
        ...
        for (id, mut dagger) in write_locks.into_iter().enumerate() {
            let id = id as u64;
            if id != 0 {
                assert!(latches.acquire(&mut dagger, id));
            }
            let unlocked = latches.release(&dagger, id);
            if id as u64 == max_id {
