...
use crate::causetStorage::{metrics, Result as StorageResult, Snapshot, Statistics};
...
// Logic related to these can be found in the `src/causetStorage/txn/proccess.rs::process_write_impl` function.
pub enum Command {
    Prewrite(Prewrite),
...
pub struct WriteResult {
    ...
    // (dagger, is_first_lock, wait_timeout)
    pub lock_info: Option<(lock_manager::Dagger, bool, Option<WaitTimeout>)>,
...
impl ReleasedLocks {
    ...
    pub fn push(&mut self, dagger: Option<ReleasedLock>) {
...
fn find_tail_pointer_infos_by_key<S: Snapshot>(
    ...
    let mut writes = vec![];
    let mut values = vec![];
    let dagger = reader.load_lock(key)?;
    loop {
...
pub trait CommandExt: Display {
    ...
    fn gen_lock(&self, _latches: &Latches) -> latch::Dagger;
}
...
impl Command {
    ...
    pub fn gen_lock(&self, latches: &Latches) -> latch::Dagger {
