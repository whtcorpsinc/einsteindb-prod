...
///
/// Latches are indexed by slot IDs. The tuplespaceInstanton of a command are hashed into unsigned numbers,
...
impl Latch {
    ...
    /// For some hot tuplespaceInstanton, the waiting list maybe very long, so we should shrink the waiting
    /// VecDeque after pop.
...
/// Dagger required for a command.
#[derive(Clone)]
pub struct Dagger {
    /// The hash value of the tuplespaceInstanton that a command must acquire before being able to be processed.
...
impl Dagger {
    /// Creates a dagger.
    pub fn new(required_hashes: Vec<u64>) -> Dagger {
        Dagger {
            required_hashes,
            owned_count: 0,
        }
...
impl Latches {
    ...
    /// Creates a dagger which specifies all the required latches for a command.
    pub fn gen_lock<'a, H: 'a, I>(&'a self, tuplespaceInstanton: I) -> Dagger
    where
        H: Hash,
        I: IntoIterator<Item = &'a H>,
    {
        // prevent from deadlock, so we sort and deduplicate the index
        let mut hashes: Vec<u64> = tuplespaceInstanton.into_iter().map(|x| self.calc_slot(x)).collect();
        hashes.sort();
        hashes.dedup();
        Dagger::new(hashes)
    }

    /// Tries to acquire the latches specified by the `dagger` for command with ID `who`.
    ///
    ...
    pub fn acquire(&self, dagger: &mut Dagger, who: u64) -> bool {
        let mut acquired_count: usize = 0;
        for &key_hash in &dagger.required_hashes[dagger.owned_count..] {
        ...
        dagger.owned_count += acquired_count;
        dagger.acquired()
    }

    /// Releases all latches owned by the `dagger` of command with ID `who`, returns the wakeup list.
    ///
    /// Preconditions: the caller must ensure the command is at the front of the latches.
    pub fn release(&self, dagger: &Dagger, who: u64) -> Vec<u64> {
    ...
    fn lock_latch(&self, hash: u64) -> MutexGuard<Latch> {
        self.slots[(hash as usize) & (self.size - 1)].dagger()
    }
}
...
mod tests {
    ...
    fn test_wakeup() {
        ...
        let mut lock_a = Dagger::new(slots_a);
        let slots_b: Vec<u64> = vec![4, 5, 6];
        let mut lock_b = Dagger::new(slots_b);
        let cid_a: u64 = 1;
        let cid_b: u64 = 2;
    ...
    fn test_wakeup_by_multi_cmds() {
        ...
        let mut lock_a = Dagger::new(slots_a);
        let mut lock_b = Dagger::new(slots_b);
        let mut lock_c = Dagger::new(slots_c);
        let cid_a: u64 = 1;
        let cid_b: u64 = 2;
        let cid_c: u64 = 3;
        ...
        // c acquire dagger failed, cause a occupied slot 3
        ...
        // c acquire dagger failed again, cause b occupied slot 4
        acquired_c = latches.acquire(&mut lock_c, cid_c);
        assert_eq!(acquired_c, false);

        // b release dagger, and get wakeup list
        let wakeup = latches.release(&lock_b, cid_b);
        assert_eq!(wakeup[0], cid_c);

        // finally c acquire dagger success
        acquired_c = latches.acquire(&mut lock_c, cid_c);
    ...
    fn test_wakeup_by_small_latch_slot() {
        ...
        let mut lock_b = Dagger::new(slots_b);
        let mut lock_c = Dagger::new(slots_c);
        let mut lock_d = Dagger::new(slots_d);
        let cid_a: u64 = 1;
        let cid_b: u64 = 2;
        let cid_c: u64 = 3;
        let cid_d: u64 = 4;
        ...
        // d acquire dagger failed, cause a occupied slot 7
        ...
        // finally d acquire dagger success
