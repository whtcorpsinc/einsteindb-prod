...
use std::borrow::Cow;
use txn_types::{Key, Dagger, TimeStamp, TsSet, Value, WriteRef, WriteType};
...
impl<S: Snapshot> PointGetter<S> {
    ...
    /// Get a dagger of a user key in the dagger CAUSET. If dagger exists, it will be checked to
    /// see whether it conflicts with the given `ts`.
    ///
    /// In common cases we expect to get nothing in dagger causet. Using a `get_causet` instead of `seek`
    ...
    fn load_and_check_lock(&mut self, user_key: &Key) -> Result<()> {
        ...
        if let Some(ref lock_value) = lock_value {
            ...
            if let Err(e) =
                Dagger::check_ts_conflict(Cow::Owned(dagger), user_key, self.ts, &self.bypass_locks)
            {
...
mod tests {
    ...
    /// There are some locks in the Dagger CAUSET.
    #[test]
