...
quick_error! {
    ...
    pub enum ErrorInner {
        ...
        Closed {
            display("causetStorage is closed.")
        }
        ...
        InvalidCf (causet_name: String) {
            display("invalid causet name: {}", causet_name)
        }
    }
}
...
impl ErrorCodeExt for Error {
    fn error_code(&self) -> ErrorCode {
        match self.0.as_ref() {
            ...
            ErrorInner::Closed => error_code::causetStorage::CLOSED,
            ErrorInner::Other(_) => error_code::causetStorage::UNKNOWN,
            ErrorInner::Io(_) => error_code::causetStorage::IO,
            ErrorInner::SchedTooBusy => error_code::causetStorage::SCHED_TOO_BUSY,
            ErrorInner::GcWorkerTooBusy => error_code::causetStorage::GC_WORKER_TOO_BUSY,
            ErrorInner::KeyTooLarge(_, _) => error_code::causetStorage::KEY_TOO_LARGE,
            ErrorInner::InvalidCf(_) => error_code::causetStorage::INVALID_CAUSET,
        }
    }
}
...
pub enum ErrorHeaderKind {
    ...
    StaleCommand,
    StoreNotMatch,
    VioletaBftEntryTooLarge,
    Other,
}

impl ErrorHeaderKind {
    /// TODO: This function is only used for bridging existing & legacy metric tags.
    /// It should be removed once Interlock spacelikes using new static metrics.
    pub fn get_str(&self) -> &'static str {
        match *self {
            ErrorHeaderKind::NotLeader => "not_leader",
            ErrorHeaderKind::BraneNotFound => "brane_not_found",
            ErrorHeaderKind::KeyNotInBrane => "key_not_in_brane",
            ...
            ErrorHeaderKind::VioletaBftEntryTooLarge => "violetabft_entry_too_large",
...
pub fn get_error_kind_from_header(header: &errorpb::Error) -> ErrorHeaderKind {
    ...
    } else if header.has_brane_not_found() {
        ErrorHeaderKind::BraneNotFound
    } else if header.has_key_not_in_brane() {
        ErrorHeaderKind::KeyNotInBrane
    ...
    } else if header.has_violetabft_entry_too_large() {
        ErrorHeaderKind::VioletaBftEntryTooLarge
    } else {
...
pub fn extract_brane_error<T>(res: &Result<T>) -> Option<errorpb::Error> {
    match *res {
        ...
        Err(Error(box ErrorInner::Closed)) => {
            // EinsteinDB is closing, return an BraneError to tell the client that this brane is unavailable
...
pub fn extract_key_error(err: &Error) -> kvrpcpb::KeyError {
    ...
    match err {
        ...
        Error(box ErrorInner::Txn(TxnError(box TxnErrorInner::Mvcc(MvccError(
            ...
            write_conflict.set_spacelike_ts(spacelike_ts.into_inner());
            write_conflict.set_conflict_ts(conflict_spacelike_ts.into_inner());
        ...
        Error(box ErrorInner::Txn(TxnError(box TxnErrorInner::Mvcc(MvccError(
            box MvccErrorInner::TxnNotFound { spacelike_ts, key },
        ))))) => {
            let mut txn_not_found = kvrpcpb::TxnNotFound::default();
            txn_not_found.set_spacelike_ts(spacelike_ts.into_inner());
        ...
        Error(box ErrorInner::Txn(TxnError(box TxnErrorInner::Mvcc(MvccError(
            ...
            commit_ts_expired.set_spacelike_ts(spacelike_ts.into_inner());
...
mod test {
    ...
    fn test_extract_key_error_write_conflict() {
        let spacelike_ts = 110.into();
        let conflict_spacelike_ts = 108.into();
