...
//! Interact with persistent causetStorage.
//!
//! The [`CausetStorage`](CausetStorage) structure provides raw and transactional APIs on top of
//! a lower-level [`Engine`](kv::Engine).
//!
//! There are multiple [`Engine`](kv::Engine) implementations, [`VioletaBftKv`](crate::server::violetabft::VioletaBftKv)
...
use crate::causetStorage::metrics::CommandKind;
...
use txn_types::{Key, KvPair, Dagger, TimeStamp, TsSet, Value};
...
/// When a EinsteinDB server is running, a [`VioletaBftKv`](crate::server::violetabft::VioletaBftKv) will be the
...
/// underlying engine is [`VioletaBftKv`](crate::server::violetabft::VioletaBftKv),
...
pub struct CausetStorage<E: Engine, L: LockManager> {
    ...
    // Fields below are causetStorage configurations.
    max_key_size: usize,
}
...
impl<E: Engine, L: LockManager> CausetStorage<E, L> {
            ...
            async move {
                    ...
                    Self::with_tls_engine(|engine| Self::snapshot(engine, None, &ctx)).await?;
                {
                    ...
                    let result = snap_store
                        ...
                        // map causetStorage::txn::Error -> causetStorage::Error
                        .map_err(Error::from)
                        .map(|r| {
            ...
            async move {
                ...
                if enable_async_commit {
                    ...
                    if ctx.get_isolation_level() == IsolationLevel::Si {
                        concurrency_manager
                            .read_cone_check(Some(&spacelike_key), lightlike_key.as_ref(), |key, dagger| {
    ...
    /// Write a raw key to the causetStorage.
    pub fn raw_put(
        &self,
        ctx: Context,
        causet: String,
    ...
    /// Write some tuplespaceInstanton to the causetStorage in a batch.
    ...
    /// Delete a raw key from the causetStorage.
    pub fn raw_delete(
        &self,
        ctx: Context,
        causet: String,
    ...
    /// The CAUSET name can be one of `"default"`, `"write"` and `"dagger"`. If given `causet` is empty,
...
mod tests {
    ...
    use engine_promises::{CAUSET_DAGGER, CAUSET_VIOLETABFT, CAUSET_WRITE};
    ...
    fn test_get_put() {
        let causetStorage = TestStorageBuilder::new(DummyLockManager {})
            ...
            .unwrap();
        let (tx, rx) = channel();
        expect_none(block_on(causetStorage.get(
            Context::default(),
            Key::from_raw(b"x"),
            100.into(),
        )));
        ...
        expect_error(
            ...
            block_on(causetStorage.get(Context::default(), Key::from_raw(b"x"), 101.into())),
        );
        causetStorage
            .sched_txn_command(
                commands::Commit::new(
    ...
    fn test_causet_error() {
        ...
        expect_error(
            ...
            block_on(causetStorage.get(Context::default(), Key::from_raw(b"x"), 1.into())),
        );
        expect_error(
            |e| match e {
                Error(box ErrorInner::Txn(TxnError(box TxnErrorInner::Mvcc(tail_pointer::Error(
        ...
        expect_error(
            ...
            block_on(causetStorage.batch_get(
                Context::default(),
                vec![Key::from_raw(b"c"), Key::from_raw(b"d")],
                1.into(),
            )),
        );
        let x = block_on(causetStorage.batch_get_command(vec![
    ...
    fn test_scan_with_key_only() {
        let mut titan_db_config = NoetherDBConfig::default();
    ...
    fn test_batch_get_command() {
        ...
        causetStorage
            ...
            .unwrap();
        rx.recv().unwrap();
        let mut x = block_on(causetStorage.batch_get_command(vec![
        ...
        causetStorage
            ...
            .unwrap();
        rx.recv().unwrap();
        let x: Vec<Option<Vec<u8>>> = block_on(causetStorage.batch_get_command(vec![
    ...
    fn test_txn() {
        ...
        expect_value(
            b"100".to_vec(),
            block_on(causetStorage.get(Context::default(), Key::from_raw(b"x"), 120.into())),
        );
        expect_value(
            b"101".to_vec(),
            block_on(causetStorage.get(Context::default(), Key::from_raw(b"y"), 120.into())),
        );
        causetStorage
            .sched_txn_command(
                commands::Prewrite::with_defaults(
    ...
    fn test_cleanup() {
        let causetStorage = TestStorageBuilder::new(DummyLockManager {})
            .build()
            .unwrap();
        let cm = causetStorage.concurrency_manager.clone();
        let (tx, rx) = channel();
        causetStorage
            .sched_txn_command(
                commands::Prewrite::with_defaults(
    ...
    fn test_high_priority_get_put() {
        ...
        expect_none(block_on(causetStorage.get(ctx, Key::from_raw(b"x"), 100.into())));
        let mut ctx = Context::default();
        ctx.set_priority(CommandPri::High);
        causetStorage
            .sched_txn_command(
        ...
        expect_value(
            b"100".to_vec(),
            block_on(causetStorage.get(ctx, Key::from_raw(b"x"), 101.into())),
        );
    }
    ...
    fn test_delete_cone() {
        ...
        expect_value(
            b"100".to_vec(),
            block_on(causetStorage.get(Context::default(), Key::from_raw(b"y"), 101.into())),
        );
        expect_value(
            b"100".to_vec(),
            block_on(causetStorage.get(Context::default(), Key::from_raw(b"z"), 101.into())),
        );
    ...
    fn test_raw_delete_cone() {
        ...
        expect_value(
            b"004".to_vec(),
            block_on(causetStorage.raw_get(Context::default(), "".to_string(), b"d".to_vec())),
        );
        ...
        expect_value(
            b"003".to_vec(),
            block_on(causetStorage.raw_get(Context::default(), "".to_string(), b"c".to_vec())),
        );
        expect_none(block_on(causetStorage.raw_get(
            Context::default(),
            "".to_string(),
            b"d".to_vec(),
        )));
        expect_value(
            b"005".to_vec(),
            block_on(causetStorage.raw_get(Context::default(), "".to_string(), b"e".to_vec())),
        );
        ...
        expect_value(
            b"001".to_vec(),
            block_on(causetStorage.raw_get(Context::default(), "".to_string(), b"a".to_vec())),
        );
        expect_value(
            b"002".to_vec(),
            block_on(causetStorage.raw_get(Context::default(), "".to_string(), b"b".to_vec())),
        );
    ...
    fn test_raw_batch_put() {
        ...
        for (key, val) in test_data {
            expect_value(
                val,
                block_on(causetStorage.raw_get(Context::default(), "".to_string(), key)),
            );
        }
    }
    ...
    fn test_raw_batch_get() {
        ...
        expect_multi_values(
            results,
            block_on(causetStorage.raw_batch_get(Context::default(), "".to_string(), tuplespaceInstanton)),
        );
    }
    ...
    fn test_batch_raw_get() {
        ...
        let x: Vec<Option<Vec<u8>>> = block_on(causetStorage.raw_batch_get_command(cmds))
            .unwrap()
            .into_iter()
            .map(|x| x.unwrap())
            .collect();
    ...
    fn test_raw_scan() {
        ...
        expect_multi_values(
            results.clone(),
            block_on(causetStorage.raw_scan(
                Context::default(),
                "".to_string(),
                vec![],
                None,
        ...
        .into_iter()
        .map(|(k, v)| Some((k, v)));
        let engine = causetStorage.get_engine();
        expect_multi_values(
    ...
    fn test_raw_batch_scan() {
        ...
        expect_multi_values(
            results,
            block_on(causetStorage.raw_batch_scan(
                Context::default(),
                "".to_string(),
                cones.clone(),
                5,
        ...
        let (lock_a, lock_b, lock_c, lock_x, lock_y, lock_z) = (
            {
                ...
                dagger.set_primary_lock(b"c".to_vec());
                dagger.set_lock_version(101);
                dagger.set_key(b"a".to_vec());
                dagger.set_lock_ttl(123);
                dagger.set_txn_size(3);
                dagger
            },
            {
                let mut dagger = LockInfo::default();
            ...
            {
                let mut dagger = LockInfo::default();
                dagger.set_primary_lock(b"x".to_vec());
                dagger.set_lock_version(100);
    ...
    fn test_resolve_lock() {
        use crate::causetStorage::txn::RESOLVE_LOCK_BATCH_SIZE;
    ...
    fn test_txn_heart_beat() {
        ...
        // `advise_ttl` = 110, which is greater than current ttl. The dagger's ttl will be ufidelated to
        // 110.
        causetStorage
            .sched_txn_command(
    ...
    fn test_check_secondary_locks() {
        ...
        causetStorage
            .sched_txn_command(
                commands::Prewrite::new(
                    vec![Mutation::Dagger(k1.clone()), Mutation::Dagger(k2.clone())],
                    b"k".to_vec(),
                    10.into(),
                    100,
                    false,
    ...
    fn test_pessimistic_lock_impl(pipelined_pessimistic_lock: bool) {
        ...
        for &return_values in &[false, true] {
            ...
            delete_pessimistic_lock(&causetStorage, key.clone(), 10, 10);
        }
        ...
        for &return_values in &[false, true] {
            ...
            delete_pessimistic_lock(&causetStorage, key.clone(), 30, 30);
        }
    }
    ...
    // It's used to check whether we slightlike right messages to dagger manager.
    #[derive(Clone)]
    ...
    // Test whether `CausetStorage` slightlikes right wait-for-dagger msgs to `LockManager`.
    #[test]
    fn validate_wait_for_lock_msg() {
        let (msg_tx, msg_rx) = channel();
        let causetStorage = TestStorageBuilder::from_engine_and_lock_mgr(
        ...
        causetStorage
            .sched_txn_command(
                ...
                expect_value_callback(
                    tx.clone(),
                    0,
                    TxnStatus::uncommitted(txn_types::Dagger::new(
                        LockType::Put,
                        b"k".to_vec(),
                        spacelike_ts,
                        100,
    ...
    fn test_check_memory_locks() {
        let causetStorage = TestStorageBuilder::new(DummyLockManager {})
            .build()
            .unwrap();
        let cm = causetStorage.get_concurrency_manager();
